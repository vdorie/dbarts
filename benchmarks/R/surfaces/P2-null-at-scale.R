...
# shipped default mixture switches representation freely (78.5 switches per
# chain) and a no-swap move set leaves a minority of chains stuck at an x3
# root for the whole chain. A single tree is the setting where a stuck root
# is visible at all; an ensemble can self-average a stuck tree's label away
# without any chain ever "mixing" it. This cell asks whether the x3-root
# representation survives at production tree counts or is washed out by the
# ensemble.
#
# Same duplicate-column design, same matched-seed idiom, two of P2's three
# move-set arms (default and no-swap only - birth/death is P2's own
# contrast, not part of the tree-count question), n.trees in {50, 200}.
#
...
cat("P2-confounded-step.R, m = 1: default 0 stuck-on-x3 chains of 40, ")
