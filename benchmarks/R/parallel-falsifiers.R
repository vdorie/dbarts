...
run_e2 <- function() {
  ...
  for (i in seq_len(nrow(e2))) {
    ...
    cat(sprintf(
      "             roll=%.2e nodeAvg=%.2e fit=%.2e rebuild=%.2e resp=%.2e book=%.2e | partRd=%.2e partWr=%.2e leafStat=%.2e\n",
      r$residualRoll / r$sweeps,
      r$nodeAverages / r$sweeps,
      r$fitWrite / r$sweeps,
      r$totalRebuild / r$sweeps,
      r$response / r$sweeps,
      r$bookkeep / r$sweeps,
      r$scanPartRd / r$sweeps,
      r$scanPartWr / r$sweeps,
      r$scanLeafStats / r$sweeps
    ))
  }
  invisible(e2)
}
...
run_e1 <- function() {
  ...
  for (i in seq_len(nrow(agg))) {
    r <- agg[i, ]
    cat(sprintf(
      "%-12s n=%-7g b=%-2d  atoms=%.0f  atoms/n=%.3f  occ=n/atoms=%.1f\n",
      r$tag,
      r$n,
      r$b,
      r$atoms,
...
run_e3 <- function() {
  ...
  for (tg in unique(e3$tag)) {
    ...
    cat(sprintf(
      "-- %s (proposed=%g, degenerate=%g)\n",
      tg,
      tot,
      sum(s$degenerate)
    ))
    cat(sprintf(
      "   OVERALL agree=%.3f survivor=%.3f trueAccept=%.3f\n",
      sum(s$agree) / tot,
      sum(s$survivor) / tot,
      sum(s$trueAccept) / tot
    ))
    for (i in seq_len(nrow(s))) {
      ...
      cat(sprintf(
        "   %-7s prop=%-8g agree=%.3f survivor=%.3f trueAcc=%.3f\n",
        r$move,
        r$proposed,
        r$agree / r$proposed,
        r$survivor / r$proposed,
...
if (which %in% c("e2", "all")) {
  run_e2()
}
if (which %in% c("e1", "all")) {
  run_e1()
}
if (which %in% c("e3", "all")) {
  run_e3()
}
