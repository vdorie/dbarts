...
rtnorm <- function(mu, lo, hi) {
  ...
  flo <- ifelse(is.finite(lo), lo, mu - 8)
  fhi <- ifelse(is.finite(hi), hi, mu + 8)
  bad <- !is.finite(z)
  z[bad] <- 0.5 * (flo[bad] + fhi[bad])
  pmin(pmax(z, flo), fhi)
}

## ---- log marginal category likelihood sum over a subset of rows
loglik_rows <- function(idx, y, mu, g) {
  # g = full cutpoints incl -Inf,0,..,Inf
  sum(log(pmax(
    pnorm(g[y[idx] + 1L] - mu[idx]) -
      pnorm(g[y[idx]] - mu[idx]),
    1e-300
  )))
}
...
run <- function(dat, method, iters = 6000L, burn = 1500L) {
  ...
  X <- cbind(1, x)
  XtX <- crossprod(X)
  prior <- diag(c(1e-2, 1e-2))
  Vbeta <- solve(XtX + prior)
  Lb <- chol(Vbeta)
    ...
    if (method == "AC") {
      ...
      for (j in seq_along(free)) {
        ...
        if (is.finite(la) && log(runif(1)) < la) {
          g[k] <- prop
          acc[j] <- acc[j] + 1
          if (it > burn) accPost[j] <- accPost[j] + 1
        }
      }
      z <- rtnorm(mu, g[y], g[y + 1L])
    } else if (method == "LOGGAP") {
      d <- diff(g[2:K])
      delta <- log(d) # K-2 gaps (gamma_1..gamma_{K-1})
      step <- tau * rnorm(K - 2L)
      dp <- exp(delta + step)
      gp <- g
      gp[2:K] <- c(0, cumsum(dp))
      la <- loglik_rows(seq_len(n), y, mu, gp) -
        loglik_rows(seq_len(n), y, mu, g) +
        sum(step) # Jacobian: sum log(d'/d)
      if (is.finite(la) && log(runif(1)) < la) {
        g <- gp
        acc <- acc + 1
        if (it > burn) accPost <- accPost + 1
      }
      z <- rtnorm(mu, g[y], g[y + 1L])
    }
    ## adapt RW scale in burn-in toward ~30% acceptance (COWLES/LOGGAP)
    if (it <= burn && method != "AC" && it %% 100 == 0) {
      rate <- if (method == "COWLES") {
        acc / 100
      } else {
        rep(mean(acc) / 100, K - 2L)
      }
...
for (r in seq_len(nrow(scen))) {
  ...
  for (m in methods) {
    ...
    el <- proc.time()[3] - t0
    cat(sprintf(
      "%-6d %-4d %-7s %-8.0f %-8.0f %-8s  (%.1fs)\n",
      scen$n[r],
      scen$K[r],
      m,
      min(out$ess),
      mean(out$ess),
      if (all(is.na(out$accept))) {
        "-"
      } else {
        paste0(sprintf("%.2f", mean(out$accept)))
      },
      el
    ))
  }
