...
makeSampler <- function(y, x, seed, ntree = 1L) {
  ctrl <- dbarts::dbartsControl(n.chains = 1L, n.trees = ntree, n.samples = 1L, n.burn = 0L,
                                updateState = FALSE, verbose = FALSE, rngSeed = seed)
...
leavesA <- parseLeaves(A$getTrees()); leafA <- leafOfObservations(leavesA, orig)
for (j in seq_len(n)) {
  otherLeafObs <- which(leafA != leafA[j])
  if (length(otherLeafObs) == 0L) next               # A is a single leaf: nothing to move into
  xnew <- orig; xnew[j] <- orig[otherLeafObs[1L]]
  ...
  cA <- as.numeric(A$data@x[, 1L]); cB <- as.numeric(B$data@x[, 1L])
...
rm(A, B, x, n, orig, leavesA, leafA, j, otherLeafObs, xnew, instA, instB, instJ, cA, cB)
...
xnew <- orig; xnew[members] <- orig[targetObs]       # route every member of A's leaf out
...
rm(A, B, x, n, orig, leavesA, leafA, sizes, srcLeaf, members, m, targetObs, xnew, inst, cA, cB)
...
set.seed(505L)
n  <- 40L
x  <- rnorm(n); w <- rnorm(n); v <- rnorm(n)
ctrl <- dbarts::dbartsControl(n.chains = 1L, n.trees = 10L, n.samples = 1L, n.burn = 0L,
                              updateState = FALSE, verbose = FALSE, rngSeed = 3L)
A <- dbarts::dbarts(y ~ theta + w, data.frame(theta = x, w = w, y = x + rnorm(n)),    control = ctrl)
B <- dbarts::dbarts(y ~ v + theta, data.frame(v = v, theta = x, y = -x + rnorm(n)),   control = ctrl)
invisible(A$run(20L, 1L)); invisible(B$run(20L, 1L))
...
expect_true(noEmptyLeaf(A)); expect_true(noEmptyLeaf(B))

# a single sampler may be passed directly (not wrapped in a list)
inst2 <- updatePredictorPerObservationJointly(A, as.numeric(A$data@x[, "theta"]), "theta")
expect_true(all(inst2))

# argument checking
expect_error(updatePredictorPerObservationJointly(list(A, B), xnew))                     # column missing
expect_error(updatePredictorPerObservationJointly(list(A, B), xnew, c("theta", "w")))    # not a single column
...
C <- dbarts::dbarts(y ~ q, data.frame(q = rnorm(n), y = rnorm(n)), control = ctrl)
invisible(C$run(5L, 1L))
expect_error(updatePredictorPerObservationJointly(list(A, C), xnew, "theta"))           # column absent in a sampler
D <- dbarts::dbarts(y ~ theta, data.frame(theta = rnorm(n + 5L), y = rnorm(n + 5L)), control = ctrl)
