...
makeSamplerPriorMidchainGaussian <- function(seed) {
  set.seed(seed)
...
sampler <- makeSamplerPriorMidchainGaussian(31L)
sampler$run(50L, 0L)
...
sampler <- makeSamplerPriorMidchainGaussian(17L)
sampler$run(50L, 0L)
...
rm(sampler, samples, makeSamplerPriorMidchainGaussian, x, y, testData)
...
makeSamplerPriorMidchainProbit <- function(seed) {
  set.seed(seed)
  dbarts(x, z, test = x, control = control)
}

sampler <- makeSamplerPriorMidchainProbit(5L)
sampler$run(50L, 0L)
samples <- sampler$run(0L, 1L)
expect_equal(as.numeric(samples$train), as.numeric(samples$test))
...
rm(sampler, samples, makeSamplerPriorMidchainProbit, control, x, z, testData)
