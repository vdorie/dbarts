...
sampler.engine <- dbarts(x.engine, y, test = x.test, control = control.engine)
...
expect_true(
  mean((rowMeans(r.engine$train) - f)^2) < 0.25 * mean((mean(y) - f)^2)
)
...
expect_equal(length(print.live), nrow(sampler.engine$getTrees(treeNums = 1L)))
expect_true(all(grepl(
  "^ *n: [0-9]+ TBN: [01]{3} Avail: [01]+ (var: [0-9]+ |ave: )",
  print.live
)))
...
set.seed(11)
sampler.to1 <- dbarts(x.engine + 0, y, test = x.test, control = control.engine)
set.seed(11)
sampler.to2 <- dbarts(x.engine + 0, y, test = x.test, control = control.engine)
...
control.engine2 <- dbartsControl(n.chains = 2L, n.threads = 2L, n.trees = 50L)
...
installed.engine.joint <- updatePredictorPerObservationJointly(
  list(sampler.jointC, sampler.jointD),
  rep(10, n),
  "x1"
)
...
control.setdata <- dbartsControl(n.chains = 1L, n.threads = 1L, n.trees = 50L)
...
expect_error(
  sampler.setdata$setData(dbartsData(x2[, 1:2], y2)),
...
expect_error(
  dbarts(x, y.binary, weights = runif(n, 0.5, 1.5), control = control),
...
residual.means <- tapply(
  rowMeans(result.cat$train) - 2 * x.cat[, 2L],
  x.cat[, 1L],
  mean
)
...
expect_equal(
  sampler.livepred$predict(x.test, offset.test = 2),
  predictions.live + 2
)

# multiple chains add their dimension to predictions and a chain column to
# the trees
control.keep2 <- dbartsControl(
  n.chains = 2L,
  n.threads = 1L,
  n.trees = 10L,
  n.samples = 5L,
...
expect_equal(
  sampler.ssr$getSumsOfSquaredResiduals(),
...
expect_identical(
  capture.output(invisible(sampler.vb$run(0L, 2L))),
  character(0)
)

# worker threads relay chain-prefixed progress through the main thread
control.threaded <- dbartsControl(
  n.chains = 2L,
  n.threads = 2L,
  n.trees = 10L,
  n.samples = 5L,
