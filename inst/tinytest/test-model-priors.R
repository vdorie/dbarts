...
sampler.nv <- dbarts(
  y ~ a + b,
  df,
  tree.prior = cgm(2, 0.95, 1 / num.vars),
  control = control
)
...
sampler.split <- dbarts(
  y ~ a + b,
  df,
  tree.prior = prior.split,
  control = control
)
expect_equal(
  sampler.split$model@tree.prior@splitProbabilities,
  c(a = 0.75, b = 0.25)
)
...
expect_error(
  methods::new("dbartsModel", tree.prior = prior.split),
