...
sampler.cat <- dbarts(y ~ x1 + x2 + g, df, tree.prior = dart(),
                      factors = "categorical", missing = "incorporate",
                      control = control)
...
sampler.ind <- dbarts(y ~ x1 + x2 + g, df, tree.prior = dart(),
                      factors = "indicators", missing = "incorporate",
                      control = control)
...
rm(sampler.cat, sampler.ind, samples.cat, samples.ind, control,
   df, x1, x2, g, mu, y, n)
