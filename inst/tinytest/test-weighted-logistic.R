...
fit.w <- bart2(y ~ x, weights = w, family = "logistic",
               n.samples = 200L, n.burn = 100L, n.trees = 50L,
               n.chains = 1L, n.threads = 1L, verbose = FALSE, keepTrees = TRUE)
...
yr <- y[idx]
xr <- x[idx, , drop = FALSE]
fit.rep <- bart2(yr ~ xr, family = "logistic",
                 n.samples = 200L, n.burn = 100L, n.trees = 50L,
                 n.chains = 1L, n.threads = 1L, verbose = FALSE, keepTrees = TRUE)
...
expect_error(
  bart2(y ~ x, weights = runif(n, 0.5, 2), family = "logistic",
        n.samples = 5L, n.burn = 5L, n.trees = 10L,
        n.chains = 1L, n.threads = 1L, verbose = FALSE),
  pattern = "must be positive integers")

# a zero count is a dropped row, also refused
expect_error(
  suppressWarnings(bart2(y ~ x, weights = c(0, w[-1L]), family = "logistic",
        n.samples = 5L, n.burn = 5L, n.trees = 10L,
        n.chains = 1L, n.threads = 1L, verbose = FALSE)),
  pattern = "must be positive integers")
...
rm(fit.w, fit.rep, phat.w, phat.rep, sampler, samples, x, f, p, y, w, idx,
   yr, xr, n, control)
