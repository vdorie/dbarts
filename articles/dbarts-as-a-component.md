...
Metropolis-Hastings scheme rather than the whole of it. Six recipes
follow. Each is a complete, runnable outer sampler; each answers the
same three questions, which are the ones a composition gets wrong:
...
reads; this vignette is the worked set. Every recipe here is also a cell
in the package’s test suite, because a composition that runs to
completion can still be sampling the wrong posterior.
