...
For users who are comfortable with writing their own posterior samplers,
`dbarts` makes it easy to incorporate a linear BART component in a Gibbs
sampler. In the example below, we fit a functional mixture model, where
observations may have come from one of two underlying functions and
class membership is unobserved.
...
and that $`Z`$ is unobserved. To make the model fully Bayesian, we
...
where $`\phi`$ is the density of a normal distribution. This yields an
overall strategy of:
...
To illustrate, we create some toy data. The simulated data is visualized
later, together with the fitted model.
...
predictor variables/covariates. This can be done with a `dbartsSampler`
reference class object by calling the `sampler$setPredictor` function.
As, given any $`Z = z`$, the sampler also needs to evaluate
$`f(x, 1 - z)`$, we utilize the test slot of the sampler and keep it
up-to-date with the “counterfactual” predictor using the
`sampler$setTestPredictor` function. Models that instead modify the
response variable can use the `sampler$setResponse` or
`sampler$setOffset` functions.
...
Finally, we visualize the results. In the graph below, the estimated
label of observations encircles the true labels, both of which are
represented by point color. We see that, beyond the range of the
observed data, the estimated functions regress towards each other and
points start to be mislabeled.
...
![](gibbs_sampler_mixture_model_files/figure-html/result_visualization-1.png)

## Several Forests with Multipliers

The sampler above holds one function. A model may want several, each
entering the response through a multiplier the outer loop knows:

``` math
Y_i \sim \mathrm{N}\left(\sum_{f=1}^{K} m_{f,i}\, g_f(x_i),\ \sigma^2\right),
```

where $`g_1, \ldots, g_K`$ are separate sums of trees and $`m_{f,i}`$ is
a known per-observation scalar - an exposure, a dose, a treatment
indicator, a group loading. Bayesian causal forests is the $`K = 2`$
case with $`m_1 \equiv 1`$ and $`m_2 = z`$.

One `dbartsSampler` per forest composes this in R, and the composition
is ordinary backfitting once the multiplier is folded in. Given the
residual $`r_i = y_i - \sum_{g \neq f} m_{g,i} g_g(x_i)`$, forest
$`f`$’s leaf conditionals want response $`r_i / m_{f,i}`$ and precision
weight $`w_i m_{f,i}^2`$; those are the two channels the loop writes,
with `setSigma` for the third and `run(0L, 1L)` to advance one sweep.
`updateScale = FALSE` (the default) is what makes the swap coherent:
every sweep’s residual is read on the scale creation fixed, so the leaf
prior keeps meaning what it meant.

A multiplier at or near zero says the row carries no information about
that forest. Divide anyway and the response is amplified without bound,
so the recipe snaps such a row to an exactly zero response paired with
its exactly zero weight - which is what the engine does internally, at
...
Measured on one machine against a single batched engine sampler at
matched total trees - the denominator matters, and a price quoted
without one says nothing - K samplers driven a sweep at a time cost
about 1.9-2.1 times that sampler at $`K = 2`$ and 4.6-5.2 times at
$`K = 8`$, growing roughly linearly in $`K`$: each sampler pays its own
per-sweep response install, weight install and result allocation over
the full data. Driving one sampler a sweep at a time rather than in a
batch is a separate and much smaller charge, about 1.1x.

### Five things the recipe does not get

- **No joint move on the multipliers.** If the outer loop also draws the
  $`m_f`$, it draws them in a block alternating with the forests. A
  sampler that holds both can interweave them instead, and the two mix
  differently whenever a forest and its multiplier are jointly weakly
  identified.
- **The division is yours.** The caller forms $`r / m_f`$ and so owns
  the conditioning, including the snap above.
- **$`K`$ quantized predictor stores and $`K`$ sampler states**, where
  one sampler holding $`K`$ forests keeps one of each.
- **$`K`$ leaf calibrations to state**, one per sampler, rather than a
  single calibration stated across the forests at creation.
- **A serial outer loop.** The forests are drawn one after another in R,
  so the composition cannot spread them across threads.
...
1.  centres $`y`$ at its midrange, or absorbs the response transform’s
...
3.  accepts that with a multiplier far from 1 the internal response a
    forest sees leaves the band its leaf prior was calibrated for.

Without (1) and (2) the two routes are different priors that agree
closely. With them the remaining difference is the leaf calibration
alone. Measured on a pinned two-forest sampler against the recipe above,
at $`n = 400`$ over eight chains: with the remedies the posterior mean
conditional effect agreed to 3.4% pointwise, inside Monte Carlo error in
root-mean-square; without them the difference was 11.9% pointwise and
several times Monte Carlo error. Both recovered the truth about equally
well, which is the point - the difference is a prior, not a defect.

State equally plainly what the recipe does not reach and never will: a
response family, link or latent structure the sampler does not provide,
and a leaf or hyperprior law it does not provide. Those are selected by
argument, not composed in R.
[`forest()`](https://vdorie.github.io/dbarts/reference/forest.md)
carries the same scale knobs the composition sets by hand, and
`$setForestBasis`, `$setForestWeights` and `$getForestAmplitudes` are
the post-creation routes into a multi-forest sampler, so a prototype
ports without changing the model it targets.

## Response Families

Everything above fits a continuous response, `dbarts`’s default
(`family = "auto"`, which chooses `"gaussian"` for a continuous $`y`$
and `"probit"` for one coded 0/1). `family` can also be set to
`"gaussian"`, `"probit"`, or `"logistic"` explicitly. The two binary
families model $`P(Y = 1 \mid x) = F(f(x))`$ for a link $`F`$ and the
sum-of-trees $`f`$, and report fits and predictions on that latent scale
rather than as probabilities (`pnorm`/`plogis` recovers the probability
...
When only a handful of predictors out of many are relevant, the CGM tree
prior’s uniform choice of split variable wastes proposals on the rest.
`tree.prior = dart(...)` replaces it with a Dirichlet prior over
split-variable probabilities (Linero 2018): the sampler learns which
variables matter and increasingly proposes splits on them, which doubles
as a variable-importance diagnostic.
...
probabilities more aggressively.
[`?dbartsPriors`](https://vdorie.github.io/dbarts/reference/dbartsPriors.md)
documents the rest of `dart`’s arguments.
...
## A Random-Intercept Model Through `setOffset`

The response-side conduit carries a whole outer block just as
`setPredictor` carries the predictor side. A random-intercept model
``` math
y_i = f(x_i) + b_{g(i)} + \epsilon_i, \quad
\epsilon_i \sim N(0, \sigma^2), \quad
b_j \sim N(0, \tau^2)
```
is the smallest worked example: $`b`$ and $`\tau`$ are two Gibbs blocks
the outer loop owns, and BART sees them only as an offset. Each sweep
draws $`b`$ from its conjugate normal against the current residual,
installs $`f`$’s new offset with `setOffset`, runs one sweep, subtracts
the offset back off the reported fit, and then draws $`\tau`$.

`setOffset(offset, updateScale)` takes `updateScale = FALSE` after
warmup: the response transform BART calibrated at creation must stay
put, or $`b`$ and $`\tau`$ - which the outer loop holds in response
units - would come to mean something else from one sweep to the next.
...
n_ri     <- 200L
n_groups <- 8L
g        <- rep_len(seq_len(n_groups), n_ri)
x_ri     <- matrix(runif(n_ri * 3L), n_ri, 3L)
b_true   <- rnorm(n_groups, 0, 0.8)
y_ri     <- sin(pi * x_ri[, 1L]) + 2 * x_ri[, 2L] + b_true[g] + rnorm(n_ri, 0, 0.5)

n_warmup_ri  <- 200L
n_samples_ri <- 500L

control_ri <- dbartsControl(updateState = FALSE, verbose = FALSE,
                            n.burn = 0L, n.samples = 1L, n.chains = 1L,
                            n.threads = 1L, n.trees = 50L)
sampler_ri <- dbarts(x_ri, y_ri, control = control_ri)

# rows of each group, and its size; the conjugate draw needs both
rows_of  <- split(seq_len(n_ri), g)
size_of  <- lengths(rows_of)

# Half-Cauchy(0, 2.5 sd(y)) on tau, sampled by a random-walk Metropolis step
tau_scale <- 2.5 * sd(y_ri)
log_tau_posterior <- function(tau, b) {
  if (tau <= 0) return(-Inf)
  -n_groups * log(tau) - 0.5 * sum(b^2) / tau^2 +
    dcauchy(tau, 0, tau_scale, log = TRUE)
}

b     <- rnorm(n_groups, 0, tau_scale / 5)
tau   <- tau_scale / 5
sigma <- sd(y_ri)
f_hat <- rep_len(0, n_ri)

samples_b   <- matrix(NA_real_, n_samples_ri, n_groups)
samples_tau <- rep_len(NA_real_, n_samples_ri)

for (i in seq_len(n_warmup_ri + n_samples_ri)) {
  # b | f, sigma, tau: independent normals, one per group
  resid     <- y_ri - f_hat
  post_var  <- 1 / (size_of / sigma^2 + 1 / tau^2)
  post_mean <- post_var * (vapply(rows_of, function(rows) sum(resid[rows]), 0) /
                             sigma^2)
  b <- rnorm(n_groups, post_mean, sqrt(post_var))

  # f, sigma | b: one BART sweep against the intercepts as an offset. The
  # scale re-anchors during warmup only, and is pinned for every kept draw.
  sampler_ri$setOffset(b[g], updateScale = i <= n_warmup_ri)
  draw  <- sampler_ri$run()
  f_hat <- as.vector(draw$train) - b[g]
  sigma <- draw$sigma[1L]

  # tau | b: a random-walk Metropolis step on the log scale
  proposal <- tau * exp(rnorm(1, 0, 0.2))
  logRatio <- log_tau_posterior(proposal, b) - log_tau_posterior(tau, b) +
    log(proposal) - log(tau)
  if (log(runif(1)) < logRatio) tau <- proposal

  if (i > n_warmup_ri) {
    samples_b[i - n_warmup_ri, ] <- b
    samples_tau[i - n_warmup_ri] <- tau
  }
}

# the group intercepts and their scale are only identified up to a shared
# level, which the forest absorbs, so compare them centered
...
effect, a frailty. `setResponse` is the companion for a block that
changes $`y`$ itself rather than shifting it.
...
across chains, so an outer loop that owns per-chain state runs one
