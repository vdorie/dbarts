...
For binary outcomes the leaf-scale parameter k takes a chi hyperprior with 1.5 degrees of freedom and scale 2, and the chi constructor's own scale default moves from infinite to 2. No alternative was weighed beyond the values themselves; a study in July rejected the infinite scale clearly and preferred scale 2 over 5 narrowly, saying nothing about the degrees of freedom. Every probit fit's posterior moves against 0.9-x, and an explicit call giving only the degrees of freedom moves too. The relabel of the constructor's first argument is the maintainer's; as of 2026-09-13 the default is not accepted, because the poor coverage that motivated the hyperprior may have been poor mixing rather than the prior, so the study is extended to separate the two on converged chains and with the scale grid extended below 1, a defensible default is chosen from that for 1.0-0, and the default is revisited after the mixing research. See also: [dec-B106].
Record: docs/plans/archive/chi-hyperprior-df.md for the relabel and docs/plans/archive/chi-default-research.md for the scale default, the second with no maintainer quote. Marked: blank. [dec-A07]
...
The variable-intercept fit stays out of dbarts, with a tombstone naming stan4bart and one line saying that the group-spread prior differs there, so results move. The alternative, an R-only grouped intercept that drives the plain sampler by setting the offset each sweep with no engine work, is rejected. On 2026-09-13 the maintainer ruled on the mixing bar the removal had been conditioned on: at stan4bart's defaults the bar is met in no case, and the remedy is a closed-form slice move along the scale ridge in stan4bart, built by rebuilding its frozen sampler rather than editing its vendored sampler headers, after those are refreshed to their upstream head and the bar re-run as a control. That bar gates the CRAN submission and not the merge to main, and until the move lands the documented interim is a thinning setting that clears the bar at about twice the wall time. A user who wants a multilevel BART fit installs stan4bart, and bartCause builds its grouped route through that package before the release.
...
The binary node hyperprior evaluation completes before a release candidate is declared, rather than before 1.0-0 if time allows. Only the timing moves: the current default and the study design stand. The release candidate therefore waits on a study that has not started, and on the breadth that study is meant to have, real datasets with both the degrees of freedom and the scale varied. The maintainer, on 2026-09-11: "I'd like it to be considered pre-release, and even pre-release candidate". See also: [dec-B106].
