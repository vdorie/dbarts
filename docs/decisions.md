...
The residual law rides the family object: student(df) is a family in the exported list of families, so the resid.dist argument and both residual-law vocabulary lists are gone from the user-facing signature. A caller still passing resid.dist gets a warning and the value is used until 1.1-0. The alternative, which shipped first, defined a list of residual-law constructors exactly like the exported list of priors but did not export it, so a user who found the priors looked for the residual laws and did not find them. The maintainer was given three options and took the recommendation. See also: [dec-B101].
