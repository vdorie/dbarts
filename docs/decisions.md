...
A fit object leaves out any draw channel or family-specific element that has no value, so which names it carries depends on the family, the number of forests and the options given. No alternative was weighed. The maintainer has since ruled the descriptor half: the components that say what the model is, such as the family, the number of forests and the residual scale model, are fixed by the family and always hold a value, and no code infers the model from which draw channels are present. The draw channels themselves still come and go with the run options, so code reading one must allow for its absence. Not yet ruled on for the draw channels. See also: [dec-B126].
Record: docs/plans/archive/bart2-argument-consolidation.md tabulates the packaging; the descriptor rule is recorded under dec-B126. Marked: not mine. [dec-A17]
...
The package exports blocks() alongside the existing idiom of naming groups in the interactions constructor. No alternative was weighed and the only maintainer approval on record is tacit. A user now chooses between two constraint constructors, each with a manual page and tests: blocks fixes each group's tree capacity, while naming groups in interactions only forbids splits across groups and lets the allocation float, so the two express different priors over allocation rather than one approximating the other. Not yet ruled on.
...
The package exports interactions, blocks, forest and varianceForest as bare top-level names, while the prior and family constructors are bundled into lists to keep generic names out of a user's search path. No alternative was weighed, and only blocks carries even a tacit maintainer approval. A user attaching the package takes four ordinary English nouns into the search path, under a rule the same design document states in the opposite direction about a hundred lines away. Not yet ruled on.
