...
For binary outcomes the leaf-scale parameter k takes a chi hyperprior with 1.5 degrees of freedom and scale 2, and the chi constructor's own scale default moves from infinite to 2. No alternative was weighed beyond the values themselves; a study in July rejected the infinite scale clearly and preferred scale 2 over 5 narrowly, saying nothing about the degrees of freedom. Every probit fit's posterior moves against 0.9-x, and an explicit call giving only the degrees of freedom moves too. The relabel of the constructor's first argument is the maintainer's; as of 2026-09-13 the default was not yet accepted, because the poor coverage that motivated the hyperprior may have been poor mixing rather than the prior, so the study was extended to separate the two on converged chains and with the scale grid extended below 1. That extended study has since run and reported (docs/plans/binary-hyperprior.md: 28 priors, three chain lengths, 162 simulated cells and 22 real datasets including sixteen from UCI) and recommends keeping chi(1.5, 2); the maintainer's confirmation of that default on the new evidence is the one thing still owed (TODO release item 1(a)). See also: [dec-B106].
...
The binary node hyperprior evaluation completes before a release candidate is declared, rather than before 1.0-0 if time allows. Only the timing moves: the current default and the study design stand. That study has since run and reported, on 2026-09-14 (docs/plans/binary-hyperprior.md: 28 priors, three chain lengths, 162 simulated cells and 22 real datasets including sixteen from UCI), recommending that chi(1.5, 2) stay; the release candidate now waits only on the maintainer's confirmation of that default (TODO release item 1(a)). The maintainer, on 2026-09-11: "I'd like it to be considered pre-release, and even pre-release candidate". See also: [dec-B106].
