...
A fit object's components split into two kinds. Descriptors say what the model is: the family, the family as specified, the number of forests, and on a family with a residual law the error law's shape and its scale model. Which descriptors a fit carries is fixed by its family, never by run options, and each always holds a value. Draw channels hold posterior draws and may be absent because a run chose not to keep them; no code infers the model from them. The package's own readers now follow this: the probability transform, the posterior-predictive draw, the plot and the survival curves read the family rather than checking whether the residual scale or the hazard grid happens to be present. The heteroscedastic marker, the only true-or-false component on any fit and written only when true, becomes a token beside the error law, constant or forest. The forest count is written on every fit, 1 for a single forest. The error-law token leaves probit and logistic fits, where it meant nothing and for logistic was wrong. Every fit class answers R's family() with the family as specified, carrying its settings, while the fit's own family element keeps the engine's token, which the link follows, so a Student-t fit's element says gaussian and a hazard fit's says probit or logistic, as the survival design settled. The alternatives were keeping the existing markers, or making only family() and the internal readers follow the rule and leaving the markers as they were. Code that tested whether the forest count or the heteroscedastic marker was absent changes meaning, though both were documented as internal. The maintainer, 2026-09-24, after the markers were laid out: "Go ahead. Do all four reshaping tasks now," with the scale model spelled as a token beside the error law. See also: [dec-B81].
