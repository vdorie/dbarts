...
After the merge to main the bridge gains an internal plain-C specification that the parsing of R objects fills and the engine is built from, with the family as one of its fields, and the engine's five remaining R touchpoints, three density functions, printing and error reporting, go through hooks the host installs, so that draws stay bit for bit the same. A creation entry taking that specification and a host-neutral error contract join the shipped header. All of it lands after bartcore merges to main and before the 1.0-0 submission, so the creation entry is part of the 1.0 contract. The alternatives were doing it before the merge, in a 1.x release, or not at all. Nothing an R user sees changes; the cost is engine edits and a header change between the merge and the release, with the sister packages re-verified after it. The maintainer adopted the staged shape alongside the header decision, then placed it on 2026-09-23: "post merging into main, but before 1.0 since it might change a number of packages and is an interface change." On 2026-09-24 the maintainer settled how the math is reached: the engine calls only the support library's math names, and which implementation stands behind them is chosen when the package is built, R's own math library under R and, for every other host, a copy of R's routines kept in the support library, rather than hooks a host installs at run time. Run-time hooks had been meant for implementations chosen by the instructions a machine supports, and none of these functions has one; the engine calls no optimized linear algebra library either, so no run-time reason remained. Draws under R stay bit for bit the same, and another host gets the same routines. The chi-squared quantile that calibrates the residual prior stays with the host, which hands the engine the resulting scale. Printing is settled the same way under [dec-A74]. See also: [dec-A50].
...
A fit object's components split into two kinds. Descriptors say what the model is: the family as specified, the same family with its settings, the number of forests, and on a family with a residual law its scale model. Which descriptors a fit carries is fixed by its family, never by run options, and each always holds a value. What follows from the family - its link, the engine family, the error law's shape - is looked up from it in one table that stops on a family it does not name, rather than stored beside it. Draw channels hold posterior draws and may be absent because a run chose not to keep them; no code infers the model from them. The package's own readers now follow this: the probability transform, the posterior-predictive draw, the plot and the survival curves read the family rather than checking whether the residual scale or the hazard grid happens to be present. The heteroscedastic marker, the only true-or-false component on any fit and written only when true, becomes a scale-model token, constant or forest. The forest count is written on every fit, 1 for a single forest. The error-law token leaves every fit, since the family fixes it. The fit's family element holds the family as specified, so a Student-t fit says student and a hazard fit says hazard.probit or hazard.logistic, and every fit class answers R's family() with that family and its settings. This reverses the survival design's choice to keep the engine's binary token on hazard fits so no reader could apply the wrong link: the lookup that stops on an unknown family guards that more strongly, where the old default sent any token but logistic through the probit link. The alternatives were keeping the existing markers; making only family() and the internal readers follow the rule; and a first pass that kept the engine's token in the family element beside a separate specified family, which left two family fields that disagreed. Code that tested whether the forest count or the heteroscedastic marker was absent, or read the engine's token or the error-law token off a fit, changes meaning; all of these are new in 1.0-0 and no released package reads them. The maintainer, 2026-09-24, after the markers were laid out: "Go ahead. Do all four reshaping tasks now," with the scale model spelled as a token; then, asked what the design should be without holding to past decisions, adopted the specified family in the family element: "Go ahead." See also: [dec-B81].
