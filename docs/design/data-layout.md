...
Per forest ([`Forest`](../../src/bartcore/combiner.hpp)):

- `indexBuffer` : `n * numTrees` index_t (uint32). Tree t owns the contiguous slice
  `indexBuffer + t*n`; `Tree::indices` points at it ([`Tree::indices`](../../src/bartcore/tree.hpp), [`Tree::initialize`](../../src/bartcore/tree.hpp)).
  It is a PERSISTENT per-tree permutation P_t: initialized to identity
  ([`Tree::initialize`](../../src/bartcore/tree.hpp)), partitioned in place by moves ([`partitionChildren`](../../src/bartcore/tree.hpp)
  partitionChildren), and carried across sweeps. Each node owns a
  contiguous RANGE `[node.begin, node.end)` of P_t ([`Node::begin`](../../src/bartcore/tree.hpp), [`Node::end`](../../src/bartcore/tree.hpp)); the
...
- `treeFits` : `n * numTrees` doubles. Tree t's fitted contribution, in
  ...
  `muByTree[t][leafOf[t * n + i]]` ([`Forest::muByTree`](../../src/bartcore/combiner.hpp), [`Forest::leafOf`](../../src/bartcore/combiner.hpp)). The
  dense slab survives for the vector and function leaves only.
- `treeY` : ONE `n` buffer, the running residual, OBSERVATION order,
  reused across all trees, rolled incrementally ([`rollTreeResidual`](../../src/bartcore/chain.hpp)).
  Its element type is the forest's `ResidT` ([`Forest::treeY`](../../src/bartcore/combiner.hpp)):
...
1. Residual roll ([`rollTreeResidual`](../../src/bartcore/chain.hpp)): `treeY[i] += oldFits[i] -
   prevFits[i]` etc. Fully CONTIGUOUS (obs order); 3 streams; already
   near-optimal, bandwidth-bound (~15% inside Chain::run's 28.7% self).
2. setNodeAverages -> computeLeafStats ([`computeLeafStats`](../../src/bartcore/tree.hpp)): per leaf,
...
3. metropolisJumpForTree scoring (moves.hpp): a proposed birth partitions
   ...
   rollback ([`snapshotSubtree`](../../src/bartcore/tree.hpp)).
4. sampleParametersAndSetFits ([`sampleParametersAndSetFits`](../../src/bartcore/chain.hpp)): per leaf, draw a
...
must snapshot the double segments too ([`snapshotSubtree`](../../src/bartcore/tree.hpp) currently
...
The single place the two axes meet is partitionChildren ([`partitionChildren`](../../src/bartcore/tree.hpp)),
