...
| memory-footprint.md | OPEN (validated by [memory-footprint-audit.md](../plans/memory-footprint-audit.md) step 2) | The closed-form memory model: every engine and R-layer allocation by component, symbol, scope and bytes per unit, with two worked reference cases. |
