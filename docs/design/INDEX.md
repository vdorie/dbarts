...
| per-draw-callbacks.md | DESIGN PROPOSAL, 2026-09-10 (revised the same day against an independent verification) | Proposes a per-draw C callback fired from each chain's worker thread over every channel `storeSample` settles, an R `callback` argument taking a function and context external pointer, a `keep.fits` storage opt-out, and a worked Rcpp example, so a large fit reduces draws in place instead of materializing the n x draws x chains array the memory audit found to be five sixths of its peak; reverses dec-B62's worker-thread refusal for an observer hook, and leaves nine forks open. |
