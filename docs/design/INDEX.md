...
| nog-gibbs.md | PROPOSED, 2026-09-07 | Designs `rule_gibbs`, an exact collapsed Gibbs draw of the split rule at a nog node (an interior node whose two children are both leaves), replacing the Metropolis change proposal there: the closed ancestor-determined neighbourhood of every (available ordinal variable, admissible cut) weighted by the cut scan's collapsed marginal times the prior factors that do not cancel, acceptance identically one and no reverse count, with a rank-aware scan so the prior-only balance arm can run, a twenty-three-file surface enumeration beside the landed perturb move, and a benefit study pre-registered on minimum effective sample size at the He and Hahn cell; nothing is built. |
