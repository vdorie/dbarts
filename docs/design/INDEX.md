...
| aft-status-setter.md | LANDED slice 1, 2026-09-07; slices 2-4 PROPOSED | Lets a live `family = "aft"` sampler take a new censoring status through `$setResponse(y, status =)`, so the structure stops being fixed at creation; enables the aft and heteroscedastic SBC arms. |
