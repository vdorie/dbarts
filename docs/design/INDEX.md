...
Manifest of every `docs/design/*.md` (42 docs besides this index), grouped by
theme. STATUS is
...
| feature-matrix.md | LIVING REFERENCE, current at e218393d (2026-08-12) | Status/tracking matrix of the 13 shipped response models x the capabilities that bear on scheduling: construction surfaces, every mutation channel, zero-weight subsetting, the two designed-but-unbuilt arcs (latent-subset-mask, nameable-calibration), getLatents, pointwise loglik, variance forest, grouped, DART, warm starts, grow-from-root, flat C API reach, equivalence baselines, SBC verdicts. Every SHIPPED/REFUSED cell anchored to live code; closes with a per-family gap list. Updated in place at every landing that changes a cell. |
...
| r-c-division.md | ACCEPTED (VD 2026-08-11) | The R-vs-C/C++ boundary as a guide: "R addresses the conditionals; C++ addresses the integrand" (four clauses, accepted as amended). Adjudicates VD's conjecture - mutation-built models are R-doable; residualization is Gaussian-bound only on the response-swap route (the offset route is family-generic and K-block splitting with host-drawn latents targets the same posterior, both measured); the real wall is nameable calibration. Prices an adoption slate, ALL items pre-release by VD's frame (utility decides, price sizes a budget only), led by nameable calibration and a latent-family subset mask; amends two extension-surface fork justifications (all four forks RESOLVED); and records six adversarially confirmed defects incl a setResponse segfault - defects 1-4 and 6 FIXED at 33f6fdc, defect 5 a remaining docs slice. Artifacts gitignored under .claude/r-c-division-design/. |
...
| tree-mixing-proposals.md | COMPLETE (survey), 2026-08-09; ADDENDUM sec 12, 2026-08-10 | Adjudicated survey of how the BART posterior is sticky (five modes, separating the two measured in-house from three that are code-derived hypotheses) and of every proposal mechanism beyond the shipped four-move set. Refutes the annealed-burn recommendation an earlier pass ranked first (no published result evaluates it; the one supporting experiment's released code tempers the whole posterior across its kept draws). Ranks a same-variable cut move ("perturb") first among tree-space proposals on cost plus a tunable-step-size argument; the first-overall rank of parametric absorption by composition was WITHDRAWN 2026-08-10 by the composition mixing probe's fired harm clause (sec 4.1 erratum - the mechanism's absorption leg holds, the mixing payoff does not reliably follow, and linear leaves harm accuracy when nothing is absorbable; outer composition survives as a conditional tool). Carries a pre-registerable first falsifier whose pilot is the per-move acceptance census nobody has measured. Corrects grow-from-root-default.md sec 4.8 (the root-lock datum cannot separate proposal frequency from acceptance) and reopens the Metropolis-coupling door. **Section 12** adds the second research cycle (forest specialization + XBART-scan whole-tree regrow): refutes the claim that BART's leaf prior forbids specialization (the CGM tree prior favours it by 208-2382 nats at m=75) and the claim that the grow-from-root proposal density equals the prior at depth 1 (correct object `pi/q = Z_root prod_{w != root} [(1-g_w) + g_w B_w]`, verified by exact enumeration); establishes that a whole-tree regrow is exact under a reverse-replay refusal predicate and that its acceptance is high rather than negligible (erratum to sec 5.4); adjudicates eight orchestrator candidates (3 admitted, 5 set aside, including the marginal-sigma top pick, which is inert because BART's leaf prior is sigma-free); amends sec 3.1 (apportionment stickiness is its fixed-structure form, not a sixth mode). |
