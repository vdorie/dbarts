...
| per-draw-callbacks.md | DESIGN PROPOSAL, 2026-09-10 | Proposes a per-draw C callback fired from each chain's worker thread, an R `callback` argument taking a function and context external pointer, and the storage opt-out that composes with it, so a large fit reduces draws in place instead of materializing the n x draws x chains array the memory audit found to be five sixths of its peak. |
