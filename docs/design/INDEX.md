...
| aft-status-setter.md | PROPOSED, 2026-09-07 | Lets a live `family = "aft"` sampler take a new censoring status through `$setResponse(y, status =)`, so the structure stops being fixed at creation; enables the aft and heteroscedastic SBC arms. |
