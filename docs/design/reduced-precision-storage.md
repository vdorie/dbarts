...
`createSampler` ([`createSampler`](../../src/bartcore/facade.hpp)) reads the family / IsCategorical /
ServesRawValues / leaf-covariate axes and dispatches ONCE to build a concrete
`SamplerFacade<L>` ([`SamplerFacade`](../../src/bartcore/facade.hpp)), handing the R bridge a type-erased
`SamplerBase*`; every later call is "one virtual hop into fully typed code"
([`SamplerBase`](../../src/bartcore/facade.hpp)). The sweep is fully monomorphic - no per-element dispatch.
...
ALREADY uint16 cutpoint codes ([`xint_t`](../../src/bartcore/data.hpp), [`codes`](../../src/bartcore/data.hpp)) - splits run on
...
- **Index narrowing size_t -> uint32** (Forest::indexBuffer [`Forest::indexBuffer`](../../src/bartcore/combiner.hpp),
  its Tree::indices alias [`Tree::indices`](../../src/bartcore/tree.hpp), and VarianceForest::indexBuffer
  [`VarianceForest::indexBuffer`](../../src/bartcore/chain.hpp)). THE TOP LEVER: footprint n*trees*8B (the single biggest hot
  ...
  Tree::indices ([`Tree::indices`](../../src/bartcore/tree.hpp)), the 8 scalar C++ partition kernels, and
  SubtreeSnapshot.indexSegment ([`SubtreeSnapshot::indexSegment`](../../src/bartcore/tree.hpp), std::vector<size_t> + a sizeof(size_t) memcpy).
  ...
  (mirroring the existing ["static_assert(std::is_same_v<misc_xint_t, std::uint16_t>);"](../../src/bartcore/tree.hpp)
  assert); that guard shipped with the lever and is live as
  ["static_assert(sizeof(index_t) == sizeof(misc_index_t));"](../../src/bartcore/tree.hpp), six lines below the misc_xint_t assert it mirrors.
...
- **leafOf uint32 -> uint16** ([`Forest::leafOf`](../../src/bartcore/combiner.hpp), n*trees). PRESERVING (node
...
- fp32 running residual (treeY). DESIGN = FORK A (a master+shadow Fork B was
  ...
  WHY FORK B IS DEAD ([`Chain::run`](../../src/bartcore/chain.hpp)): treeY is a running residual updated
  PER-TREE inside the backfit loop - `rollTreeResidual(forest, t, ...)`
  ([`rollTreeResidual`](../../src/bartcore/chain.hpp)) rewrites it, then the gather
  ([`Tree::setNodeAverages`](../../src/bartcore/chain.hpp)) reads tree t's freshly-rolled
  ...
  THE ROLL IS INCREMENTAL ([`rollTreeResidual`](../../src/bartcore/chain.hpp): t=0 recomputes
  ...
  ([`finalizeTotalFits`](../../src/bartcore/chain.hpp)) computes total = y - resid + mu[leaf] - i.e. FROM the drifted resid -
...
- fp32 SCRATCH/FITS BUNDLE (same opt-in flag, same SBC gate as treeY,
  gaussian/continuous path only): Forest::totalFits ([`Forest::totalFits`](../../src/bartcore/combiner.hpp), n),
  ...
  [`combinedVariance`](../../src/bartcore/chain.hpp), [`meanResidual`](../../src/bartcore/chain.hpp), [`divisor`](../../src/bartcore/chain.hpp), [`treeResidual`](../../src/bartcore/chain.hpp)), and the gaussian working response yRescaled_ (model.hpp, n).
...
- Predictor codes uint16 -> uint8 ([`codes`](../../src/bartcore/data.hpp), [`maxNumCutsRepresentable`](../../src/bartcore/data.hpp)): the largest n*p array but
  ...
  transparent tier.
- fp32 cutpoints ([`cutPoints`](../../src/bartcore/data.hpp), tiny footprint) and fp32 ownedTestValues
  ([`ownedTestValues`](../../src/bartcore/data.hpp), cold re-quantize source): fp32 flips near-tie quantization ->
