...
RIB:2377, called from both creation routes at RIB:2430 and RIB:3366) and at
...
only (RIB:4822-4823) and is refused for grouped (RIB:4824) and aft (RIB:4817);
...
attribute on the model SEXP (RIB:2653-2661, gaussian-only gate) and refused for
...
same public dispatch every family uses, `bartcore_create` (RIB:3678), whose
multinomial arm is `createMultinomialDataHolder` (RIB:3644); the dedicated
...
(`enforceBinaryWeightPolicy` RIB:2870) holds on every mutation conduit too, and
...
and `setSigma`; grouped probit is refused on both (RIB:2904-2912, RIB:3041-3049); grouped
...
`$setActiveRows` (dbarts.R:1413) and the bridge entry (RIB:4192). S2 landed at
...
per-observation channel, `bartcore_setForestWeights` (RIB:4142-4145) - see
...
returns `R_NilValue` (RIB:6306). A K-forest sampler is no longer one of them.
...
(RIB:6304) gates only on that pointer being null, so since M4.4 a probit
...
channel, `bartcore_setForestWeights` (RIB:4142-4145), naming the model reason
...
BCF-composition gate (`refuseUnsupportedAmplitudeComposition`, RIB:2427), and
the multinomial forest builder (`buildMultinomialSampler`, RIB:3470). S2
...
(RIB:4386) - is what the unexported `dbarts:::bartcoreSetForestPriorScale`
...
than unbuilt: `setForestWeights` (RIB:4143) already expresses it - though note
...
RIB:4319, `bartcore_setCalibration` RIB:4374), not by a hetero-specific gate.
...
replaces row by row"), with a `createHolder` backstop (RIB:3165) closing the
...
[f32] No surface at all: `applyGroupAttribute` (RIB:2017) is called from exactly
one site, RIB:3188, on the single-forest holder path, so `bartcore.groups` is
...
(RIB:3418-3485) copies only power/base/proposal-probability fields, so a DART
...
RIB:2377, `createAmplitudeSampler` FAC:900-921 - the last sitting directly
...
the bridge's `refuseMultiForestResponseMutation` (RIB:2758) keys on `numForests
...
replay (`bartcore_predictPerForest` RIB:6071,
...
refused through `refuseUndefinedTestFits` (RIB:3011) because the blend
