...
([Heteroscedastic AFT: lifting the variance-forest refusal](aft-variance-forest.md#heteroscedastic-aft-lifting-the-variance-forest-refusal)).
...
extractSurvivalResponse ([`extractSurvivalResponse`](../../R/dbarts.R)), or a thin sibling of it that skips
the log() transform (shipped as [`extractSurvivalTimes`](../../R/dbarts.R)) and returns the raw times and status for
...
at [`dbarts`](../../R/dbarts.R)) - so the discrete-time model is always requested
...
bartcore.survival -> $status precedent, [`packageBartResults`](../../R/bart.R)), and the
...
- Alternative B: an orthogonal `discrete.time = TRUE` flag with the link
  ...
  scrutiny - ALL three forms need the identical guard edit at [`dbarts`](../../R/dbarts.R), so
...
([`dbarts`](../../R/dbarts.R), [`bart2`](../../R/bart.R)) and remapped as above; xbart
omits them (its match.arg vector, [`xbart`](../../R/xbart.R), IS the refusal, the
...
exactly where extractSurvivalResponse is ([`extractSurvivalResponse`](../../R/dbarts.R)), BEFORE
...
[`resolveFamily`](../../src/R_interface_bartcore.cpp),
[`applySurvivalAttribute`](../../src/R_interface_bartcore.cpp)), and
...
[`resolveSamplerSpec`](../../R/spec.R)) - the censoring is already baked into y'.
...
policy unchanged - probit refuses non-unit weights ([`enforceWeightPolicy`](../../R/spec.R)),
logistic requires positive integer counts ([`enforceWeightPolicy`](../../R/spec.R)). No new offset or
...
([`packageBartResults`](../../R/bart.R)), which the remap set. The hazard provenance rides a
separate marker: $periods, the ordered period grid, present only on
hazard fits (the aft $status precedent, [`packageBartResults`](../../R/bart.R), and the
...
pnorm ([`probabilityFromLatents`](../../R/generics.R)) - had $family recorded a hazard token instead,
...
and errors on families it does not know ([`pointwiseLogLikelihood`](../../R/generics.R); its
...
sample = "train", [`survivalProbabilities.bart`](../../R/bart.R)) because every subject has exactly
...
keepTrees UNCONDITIONALLY (the predict.bart guard, [`refuseWithoutTrees`](../../R/generics.R)),
