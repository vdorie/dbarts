...
- dbarts has all four moves:
  ...
  (src/bartcore/moves.hpp:771). Neither lower bound analyzes a sampler
...
(src/bartcore/sampler.hpp:1037-1046, `Chain::growForestFromRoot`) - a
...
`changeMove` (src/bartcore/moves.hpp:420-559) draws its target node
uniformly over ALL non-bottom nodes INCLUDING the root
(`tree.fillNotBottom(0, notBottom)` at moves.hpp:427, seeded from index 0;
uniform draw at moves.hpp:430-432) - the root is not structurally excluded
...
moves.hpp:540) and rescores the FULL subtree's likelihood under the new
routing (`logLikelihoodForBranch`, moves.hpp:535 and :545, walking every
bottom descendant via `fillBottom` and vetoing with `-HUGE_VAL` if any
leaf empties, moves.hpp:73). Acceptance is `alpha = exp((belowY-belowX) +
(yLogL-xLogL) + correction)`, capped at 1 (moves.hpp:547-549). At the root
