...
([Defects found by this arc (live at da37242; all adversarially](r-c-division.md#defects-found-by-this-arc-live-at-da37242-all-adversarially), "needs a guard or prominent
...
present at tip ([`refuseAmplitudeMutation`](../../R/bartcore.R)). Neither commit touches a probed path
...
is gone (section 1.5).
`result$fit <- sampler` at [`bart2Multinomial`](../../R/bart.R),
[`bart2MultinomialCounts`](../../R/bart.R), [`bart2Ordinal`](../../R/bart.R),
[`bart2Negbin`](../../R/bart.R), gated on `control@keepTrees || keepSampler`.
...
([`createChainRngs`](../../src/R_interface_bartcore.cpp), draws one
...
codes `0..K-1`; [`responseIsBinary`](../../R/spec.R) resolves auto on a numeric response by
...
- **`family = "auto"` never reaches this path.** [`detectAutoMultinomial`](../../R/bart.R)
  requires `n.levels >= 3L`, and a 2-level factor
...
S2+S3, which built one). [`dbarts`](../../R/dbarts.R)'s `family` formal
now lists thirteen tokens with `"multinomial"`
among them; [`dbartsSpec`](../../R/spec.R) reaches it the same way; and creation runs
through the single public dispatch every family uses, [`bartcore_create`](../../src/R_interface_bartcore.cpp),
whose multinomial arm is
[`createMultinomialDataHolder`](../../src/R_interface_bartcore.cpp) - the dedicated
...
`src/R_interface_bartcore.cpp` and [The surface](multinomial.md#the-surface)).
...
| **test offset** | [`bartcore_setCategoryTestOffset`](../../src/R_interface_bartcore.cpp) | [`bartcoreSamplerSetCategoryTestOffset`](../../R/bartcore.R) | S, unexported |
| predictors: whole / column / per-obs / joint | [`bartcore_setPredictor`](../../src/R_interface_bartcore.cpp), [`bartcore_updatePredictor`](../../src/R_interface_bartcore.cpp), [`bartcore_updatePredictorPerObservation`](../../src/R_interface_bartcore.cpp), [`bartcore_updatePredictorPerObservationJointly`](../../src/R_interface_bartcore.cpp) | | S - no multi-forest guard, by decision ([2. Which mutations are legal between sweeps](bart-as-a-component.md#2-which-mutations-are-legal-between-sweeps)) |
...
`multinomial-counts-mutation` entry (retired: [`multinomial-counts-mutation`](../../TODO) - the entry no longer exists as its own heading, archived at [multinomial-counts-mutation](../plans/archive/multinomial-counts-mutation.md#multinomial-counts-mutation) and referenced only from TODO's survival-followups entry) then said "The multinomial
...
[`dbartsSampler`](../../R/dbarts.R): 43 methods, 7 fields (`pointer`,
`control`, `model`, `data`, `state`, `hostFor` (retired: the field was
deleted with the mechanism at S4/F1), [`forestWeights`](../../R/dbarts.R)).
...
original refuses. [`copy`](../../R/dbarts.R) constructs through
`dbartsSampler$new`, which sets `hostFor <- character(0L)` (retired: [`hostFor`](../../R/dbarts.R) - both
...
class, no re-creation path (contrast [`getPointer`](../../R/dbarts.R)).
...
`data`, fork G; `length(data@y)` appears at
[`dbartsSampler$setWeights`](../../R/dbarts.R), [`dbartsSampler$setActiveRows`](../../R/dbarts.R), [`dbartsSampler$setForestWeights`](../../R/dbarts.R), [`dbartsSampler$setForestBasis`](../../R/dbarts.R),
and [`bartcoreSamplerSetOffset`](../../R/bartcore.R); `$predict`'s offset coercion
[`dbartsSampler$predict`](../../R/dbarts.R) needs a K-matrix arm - S2+S3 built it, the
`!is.null(counts)` branch inside it).
...
[`checkFamilyUnsupportedArgs`](../../R/bart.R) has **four** callers, all inside
`bart2` - multinomial, ordinal, nbinom, and
...
matrix IS drawn every sweep ([`MultinomialForestCombiner::drawForestGlue`](../../src/bartcore/combiner.hpp)). (i) build a
...
Facts, corrected and re-anchored: `DBARTS_C_API_HASH`
([`DBARTS_C_API_HASH`](../../inst/include/dbarts/dbarts.h)) was
`0x66d33f1613892406ULL` when this section was written and has since
re-baked again (current value `0xca7b56a64c812b8dULL`, per Fork C's own
cost note below);
...
move before it"
([`DBARTS_C_API_MAJOR`](../../inst/include/dbarts/dbarts.h), [`DBARTS_C_API_MINOR`](../../inst/include/dbarts/dbarts.h),
which is the citation to use, not
...
`DBARTS_C_API_HASH` ([`DBARTS_C_API_HASH`](../../inst/include/dbarts/dbarts.h)) AND `dbarts_apiSignatureToken`
([`dbarts_apiSignatureToken`](../../src/C_interface.cpp), `static_assert(... == 0xcb83367ee0c4175bULL)`),
because appending to `DBARTS_C_API_DECLS` moves the signature half too.
Plus `inst/tinytest/test-capi.R`: the live-hash `expect_identical`
(["expect_identical(hashes$text"](../../inst/tinytest/test-capi.R)) pins the literal and must
change, and one `expect_false` for the superseded literal should be added
alongside the ones already there. The quoted pin has since moved more than
once: the header re-baked when `dbarts_sampler_predict` took a thread count
(superseding ["0x6c9776ae1197e8f5"](../../inst/tinytest/test-capi.R)), and has
re-baked again since - `0x66d33f1613892406`, this section's own then-live
value, is now ALSO superseded
(["0x66d33f1613892406"](../../inst/tinytest/test-capi.R)), beside
["0x1a911c00bb26dcd7"](../../inst/tinytest/test-capi.R) and
["0xcd88efcd67de55d7"](../../inst/tinytest/test-capi.R) and
["0x85bd1ef04beb3848"](../../inst/tinytest/test-capi.R).
...
["0x1a911c00bb26dcd7"](../../inst/tinytest/test-capi.R) names it and
["0xcd88efcd67de55d7"](../../inst/tinytest/test-capi.R)
...
(`holderFinalizer`, [`holderFinalizer`](../../src/R_interface_bartcore.cpp)) - no double free, since each externalptr
...
([`dbartsSampler$initialize`](../../R/dbarts.R)), so it resolves against the adopted engine; and
...
([`fitSummaries`](../../benchmarks/R/equivalence.R)), not of the change. The prize is
...
(["REPRODUCTION GATE"](../../inst/tinytest/test-multinomial-surface.R)), whose comparator `internalMultinomialFit`
([`internalMultinomialFit`](../../inst/tinytest/test-multinomial-surface.R)) mirrors the host-then-bc sequence "with no reseed in between".
...
`thresholds.raw` ([`bart2`](../../R/bart.R)) and `dispersion.raw` ([`bart2`](../../R/bart.R)) inside
...
under D0 those cells INVERT - the sampler becomes mutable - so they must
be rewritten as capability assertions, not deleted.
...
precedent is exact ([`dbartsData`](../../R/A_class.R), whose own comment states the
rule). Re-creation becomes free; save/load works; `$setCounts` mirrors as
`$setWeights` does.

Three costs the first draft missed, all confirmed:

- **The `dbartsData` validity method is in scope.** [`dbartsData`](../../R/A_class.R)
...
- **Two family gates are required, and one cannot key on family.**
  ...
  ([`dbartsData`](../../R/data.R)) fires `response values are indistinguishable, or
  ...
  one condition (`fixedUnitScale`, [`fixedUnitScale`](../../R/spec.R), already a
  family-keyed flag). The warning gate is NOT: `dbartsData()` has no
  `family` formal ([`dbartsData`](../../R/data.R)), so it must key on the data object
...
- **Option (ii) is a hard error, not "five sites".** MEASURED:
  ...
  0` ([`parseData`](../../src/R_interface_bartcore.cpp)) if it got there. Option (i)
...
repeats a shape [The multiplier snap and the per-forest weight (2026-08-10)](bcf.md#the-multiplier-snap-and-the-per-forest-weight-2026-08-10) already books as a defect - "the two
...
`refuseMultiForestResponseMutation` ([`refuseMultiForestResponseMutation`](../../src/R_interface_bartcore.cpp)) is shared with the
flat C API and its own comment forbids stating two rules ([`refuseMultiForestResponseMutation`](../../src/R_interface_bartcore.cpp)).
...
S2+S3: that is the message the response arm now carries ([`refuseMultiForestResponseMutation`](../../src/R_interface_bartcore.cpp)).

Same edit, cheap: with `supportsCountsMutation` true the WEIGHTS conduit
then fell through to the RESPONSE message (tested only
...
arm is inside the same function
([`refuseMultiForestResponseMutation`](../../src/R_interface_bartcore.cpp)),
and it states the model reason - an integer weight
...
| **Door 2: flat ABI (Fork C2)** | `dbarts_sampler_createMultinomial` + counts/offset setters + a K-aware predict. | ~260 / ~200, **two literal re-bakes** ([`DBARTS_C_API_HASH`](../../inst/include/dbarts/dbarts.h), [`dbarts_apiSignatureToken`](../../src/C_interface.cpp)), ["expect_identical(hashes$text"](../../inst/tinytest/test-capi.R) + one new `expect_false`, one in-house consumer rebuild | multinomial stays R-only; the named consumer (stan4bart) is on its own branch and its own release, so nothing is blocked |
| **Door 3: nbinom loop collapse** | Replace the per-sweep loop ([`bart2`](../../R/bart.R)) with one `run`. | ~40 / ~30; **`bench-sampler` IS owed** - it removes `n.samples` R-level round trips from the sampling path, which is sweep-proportional | a per-sweep R round trip per kept draw |
...
`setSavedSlotBase(currentSampleNum_)`, [`Sampler::run`](../../src/bartcore/sampler.hpp), advanced at
[`Sampler::run`](../../src/bartcore/sampler.hpp)), so `keepTrees` is not a counterexample. The collapse is safe
...
- **S0**: all 42 existing scenarios bitwise; one appended
  ...
  established isolation pattern ([`runScenarios`](../../benchmarks/R/multinomial-equivalence.R)), so
...
`sbcMakeMultinomial` ([`sbcMakeMultinomial`](../../benchmarks/R/sbc.R)) goes through
...
([The surface](multinomial.md#the-surface)), G1 puts them where re-creation already reads
...
- `man/dbartsSampler-class.Rd`: the host-shell sentences (retired: deleted
  ...
  [S1. What getLatents returns, and the fit without the offset](../plans/adoption-slate.md#s1-what-getlatents-returns-and-the-fit-without-the-offset) added.
- [`storeState`](../../man/bart.Rd) (save/load) extended to the three families - and it
...
- `docs/design/feature-matrix.md`: the `multinom` row of section 2, the
  ...
  [Gaps](feature-matrix.md#gaps).
- [Defects found by this arc (live at da37242; all adversarially](r-c-division.md#defects-found-by-this-arc-live-at-da37242-all-adversarially) defect 6 closes;
  [The surface](multinomial.md#the-surface);
  `docs/design/bart-as-a-component.md` section 2.
- `TODO`: `host-shell-read-guards` (retired: [`host-shell-read-guards`](../../TODO) - the entry is gone) closes as OBVIATED;
  `multinomial-counts-mutation` (retired: [`multinomial-counts-mutation`](../../TODO) - the entry no longer exists as its own heading, archived at [multinomial-counts-mutation](../plans/archive/multinomial-counts-mutation.md#multinomial-counts-mutation) and referenced only from TODO's survival-followups entry) gains the surface note.
- `inst/NEWS.Rd`: the `$fit` change, the `$bc` deletion (recorded at
  [inst/NEWS.Rd:2196-2199](https://github.com/vdorie/dbarts/blob/e5bb3251fab43561f59434fccb3a29e0c7c5e470/inst/NEWS.Rd#L2196-L2199) until the 1.0-0 notes were trimmed to
...
model").** [`AmplitudeForestCombiner`](../../src/bartcore/combiner.hpp) and [`Chain`](../../src/bartcore/chain.hpp) no longer say BCF;
TODO (retired: [`residue strings`](../../TODO) - no TODO entry naming this
residue survives; `grep -rn "static_assert\|residue string\|amplitude
coupling" TODO` finds nothing, so the sweep was evidently discharged or
dropped without a trace rather than left pending) said sweep
opportunistically with the
...
[inst/NEWS.Rd:2196-2199](https://github.com/vdorie/dbarts/blob/e5bb3251fab43561f59434fccb3a29e0c7c5e470/inst/NEWS.Rd#L2196-L2199) recorded the deletion).** `$bc` was a bare
...
six code readers (retired: ["$bc"](../../R/generics.R)),
three test assertions (retired: ["$bc"](../../inst/tinytest/test-ordinal.R),
retired: ["$bc"](../../inst/tinytest/test-nbinom.R),
retired: ["$bc"](../../inst/tinytest/test-multinomial-surface.R)), three Rd
paragraphs, one shipped NEWS sentence (retired: ["$bc"](../../inst/NEWS.Rd)), and the
...
1. [docs/design/r-c-division.md:339-341](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/design/r-c-division.md#L339-L341) - "whether to extend the `hostFor` guard to
   [ordinal and nbinom] is a VD question BY NAME, unscheduled."
   **STALE.** Extended at adoption-slate S7 (`cedf4c34`); [R/bart.R:1754](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L1754)
   and [R/bart.R:1999](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L1999) carry the assignments. (Defect 6 spans [docs/design/r-c-division.md:334-341](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/design/r-c-division.md#L334-L341); my
   revision-1 citation [docs/design/r-c-division.md:337-341](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/design/r-c-division.md#L337-L341) clipped it.)
2. [TODO:150-168](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/TODO#L150-L168) - "four shipped readers ... answer from the shell."
   **TRUE BUT UNDERSTATED.** Thirteen substantive unguarded methods;
   `$predict` among them, MEASURED to return a constant; `$copy()` a
   further hole.
3. [docs/plans/adoption-slate.md:212-215](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/plans/adoption-slate.md#L212-L215) (**`docs/plans/`, not `docs/design/`**) -
...
4. [docs/plans/multiforest-extension-surface.md:906-908](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/plans/multiforest-extension-surface.md#L906-L908) (**`docs/plans/`**) - "the
   R5 fields are exactly six ... ([R/dbarts.R:711-722](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/dbarts.R#L711-L722))". **STALE, both
   halves.** Seven fields at [R/dbarts.R:848-866](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/dbarts.R#L848-L866).
5. `feature-matrix.md` `[f11]` - "Every R5 mutator is refused by
   `refuseHostMutation` ([R/dbarts.R:914-925](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/dbarts.R#L914-L925))". **VERIFIED.** 22 sites.
6. `feature-matrix.md` `[f11]` anchors [R/bart.R:1451](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L1451), [R/bart.R:1534](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L1534). **STALE.**
   Live [R/bart.R:1462](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L1462), [R/bart.R:1539](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L1539); `hostFor` at [R/bart.R:1443](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L1443), [R/bart.R:1526](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L1526).
7. `feature-matrix.md` `[f4]` anchors [R/bartcore.R:846](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bartcore.R#L846), [R/bartcore.R:909](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bartcore.R#L909). **STALE.**
   Live [R/bartcore.R:852](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bartcore.R#L852), [R/bartcore.R:915](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bartcore.R#L915).
8. [docs/plans/adoption-slate.md:248-249](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/plans/adoption-slate.md#L248-L249) (**`docs/plans/`**) anchors four
   `R/bartcore.R` wrappers. **VERIFIED** at [R/bartcore.R:1093](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bartcore.R#L1093), [R/bartcore.R:1329](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bartcore.R#L1329), [R/bartcore.R:1339](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bartcore.R#L1339),
   [R/bartcore.R:831](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bartcore.R#L831).
9. `bart-as-a-component.md` anchors. **FIVE VERIFIED EXACT** -
   `refuseMultiForestMutation` [src/R_interface_bartcore.cpp:2629](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/R_interface_bartcore.cpp#L2629), `refuseMultiForestResponse
   Mutation` [src/R_interface_bartcore.cpp:2652](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/R_interface_bartcore.cpp#L2652), `refuseUndefinedTestFits` [src/R_interface_bartcore.cpp:2849](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/R_interface_bartcore.cpp#L2849),
   `Chain::supportsResponseMutation` [src/bartcore/chain.hpp:1068](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/bartcore/chain.hpp#L1068),
   `AmplitudeForestCombiner` [src/bartcore/combiner.hpp:726](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/bartcore/combiner.hpp#L726). The sixth
   (`refuseAmplitudeMutation`, [R/bartcore.R:36](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bartcore.R#L36)) is exact as an anchor
   but is cited at **doc line 74**, outside the [R/bartcore.R:42-71](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bartcore.R#L42-L71) window I quoted;
   "ALL SIX VERIFIED EXACT" overstated the window, not the anchors.
10. [TODO:309-322](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/TODO#L309-L322) - "a full conditional inside a larger Gibbs/MH
    sampler." **TRUE OF THE ENGINE, FALSE OF THE PUBLIC SURFACE.**
11. [docs/design/r-c-division.md:28-31](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/design/r-c-division.md#L28-L31) - "the response side is sealed."
    **STALE BY LANDING** at the bridge; still true at the surface.
12. [docs/design/feature-matrix.md:999-1005](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/design/feature-matrix.md#L999-L1005) - the multinomial gap list.
    **VERIFIED, every clause.** The most accurate single statement of the
    problem in the repo.
13. [docs/plans/adoption-slate.md:307](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/plans/adoption-slate.md#L307) (**`docs/plans/`**) - "the shell cell."
    **VERIFIED**: [inst/tinytest/test-fits-without-offset.R:239-242](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/inst/tinytest/test-fits-without-offset.R#L239-L242) and
    [inst/tinytest/test-dispersion-channel.R:271-272](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/inst/tinytest/test-dispersion-channel.R#L271-L272). Both lose their subject under F1.
14. **RETRACTED AND REPLACED.** Revision 1 claimed
    ...
    at [src/C_interface.cpp:460](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/C_interface.cpp#L460), and [inst/tinytest/test-capi.R:78](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/inst/tinytest/test-capi.R#L78) asserts the full
    hash is NOT it. The conclusion survives on other evidence -
    [inst/tinytest/test-capi.R:72-73](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/inst/tinytest/test-capi.R#L72-L73) names `0x1a911c00bb26dcd7` and
    ...
    **[inst/include/dbarts/dbarts.h:100-104](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/inst/include/dbarts/dbarts.h#L100-L104)**, the header's own words, not to a plan; the
    carve-out text sits at [docs/plans/adoption-slate.md:1757-1759](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/plans/adoption-slate.md#L1757-L1759) and decision 8
    is owned by `docs/plans/archive/dbarts-h-reshape.md`.
15. [man/bart2.Rd:291](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/man/bart2.Rd#L291)/[man/bart2.Rd:295](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/man/bart2.Rd#L295)/[man/bart2.Rd:299](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/man/bart2.Rd#L299) - "`bc` and `fit` present only under
    `keepTrees`". **WRONG for `$fit`**, and contradicted by
    [inst/tinytest/test-multinomial-surface.R:340-354](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/inst/tinytest/test-multinomial-surface.R#L340-L354). Moved to S0.
16. [docs/design/INDEX.md:3](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/design/INDEX.md#L3) - "47 docs besides this index".
    **VERIFIED** (48 files).
...
18. `benchmarks/R/sbc.R` - **no `bart2(` call anywhere**;
    `sbcMakeMultinomial` ([benchmarks/R/sbc.R:1360-1362](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/benchmarks/R/sbc.R#L1360-L1362)) uses
    `getFromNamespace("bartcoreMultinomialSampler")`. No SBC re-run is
    owed for S1a or S4.
19. [R/bart.R:596-624](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L596-L624) `checkFamilyUnsupportedArgs` has **four callers**:
    [R/bart.R:891](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L891) multinomial, [R/bart.R:1041](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L1041) ordinal, [R/bart.R:1072](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L1072) nbinom, [R/bart.R:1114](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L1114)
    **hurdle.lognormal**. The `samplerOnly` block is [R/bart.R:604-606](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bart.R#L604-L606).
    Revision 1's "two lines" was wrong and named three families where
    there are four.
20. [R/A_class.R:548-610](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/A_class.R#L548-L610) - the `dbartsData` **validity method**, which
...
21. [R/data.R:1259-1270](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/data.R#L1259-L1270) degeneracy check and [R/spec.R:189-190](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/spec.R#L189-L190) sigma
    ...
    **MEASURED**: the warning text, and a starting sigma at
    ...
    `dbartsData()` has no `family` formal ([R/data.R:735-746](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/data.R#L735-L746)), so the
    warning gate must key on `@counts`, not on family.
22. [src/bartcore/combiner.hpp:728](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/bartcore/combiner.hpp#L728) / [src/bartcore/chain.hpp:760](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/bartcore/chain.hpp#L760) carry the two residue
...
| 3 | ACCEPTED; draw movement is now stated as conditional on `seed = NA`, with the seeded-identical probe and the [benchmarks/R/equivalence.R:1396](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/benchmarks/R/equivalence.R#L1396) fixture attribution. |
...
| 11 | ACCEPTED; ledger 14 RETRACTED and replaced, MAJOR/MINOR re-cited to [inst/include/dbarts/dbarts.h:100-104](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/inst/include/dbarts/dbarts.h#L100-L104), C2's cost corrected to two re-bakes, **and Fork C's recommendation changed from C2-now to C3-now**. |
...
| 17 | ACCEPTED; all anchor corrections applied (plans-vs-design paths, [docs/design/r-c-division.md:334-341](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/design/r-c-division.md#L334-L341), [TODO:309-322](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/TODO#L309-L322), [docs/plans/adoption-slate.md:1757-1759](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/docs/plans/adoption-slate.md#L1757-L1759), [src/bartcore/combiner.hpp:730-731](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/bartcore/combiner.hpp#L730-L731), and the [R/bartcore.R:42-71](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/R/bartcore.R#L42-L71) window overstatement). |
...
1. **Finding 11(b)**, "the 'one text-hash cell edit' is likely zero edits
   plus one new `expect_false`." Refined: [inst/tinytest/test-capi.R:80](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/inst/tinytest/test-capi.R#L80) pins the
...
2. **Finding 4**, option (ii) "is a hard bridge error ...
   [src/R_interface_bartcore.cpp:939-955](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/R_interface_bartcore.cpp#L939-L955)." Refined: it dies one layer
...
4. **Finding 11**, `dbarts_apiSignatureToken` "static_asserted at
   [src/C_interface.cpp:462](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/C_interface.cpp#L462)." Refined: the `static_assert` statement
   begins at **[src/C_interface.cpp:460](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/C_interface.cpp#L460)**; [src/C_interface.cpp:462](https://github.com/vdorie/dbarts/blob/0f5a2285f4899710406aab255f5cae3b66be878f/src/C_interface.cpp#L462) is inside its message string.
