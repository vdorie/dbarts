...
`metropolisJumpForTree` (`moves.hpp:777-798`) draws one uniform per tree
per sweep and dispatches to exactly one of three kernels:

```
u < birthOrDeathProbability          -> birthOrDeathMove   (moves.hpp:154)
u < birthOrDeath + swapProbability   -> swapMove           (moves.hpp:672)
else                                 -> changeMove         (moves.hpp:420)
```

Shipped mixture `birth_death = 0.5, swap = 0.1, change = 0.4, birth = 0.5`
(`R/model.R:5-10`, `R/dbarts.R:366`, engine defaults `chain.hpp:52-55`).
...
- **Swap** exchanges the rules of a parent and one child. It preserves the
  ...
  (`moves.hpp:758`). It cannot lift a rule more than one level.
- **Change** picks uniformly among all non-leaf nodes *including the root*,
  ...
  held fixed** (`moves.hpp:537-540`) and every observation is rerouted
...
3. **dbarts has no same-variable-only cut move.** `changeMove` always
   ...
   `split.probs = 1/num.vars`, `R/bart.R:517`). The machinery for such a
   move already exists and is already gated (section 5.1).

Adjacent machinery already landed, relevant to cost: `scan.hpp:75`
`scanOrdinalCuts` (leaf-templated full-cut scan); `grow.hpp:63`
`growTreeFromRoot`; `chain.hpp:1330` `growForestFromRoot` (opt-in
`n.grow.sweeps`, init only, with a reset/regrow/rebuild/redraw loop at
`chain.hpp:1370-1387`); `tree.hpp:951-973` `SubtreeSnapshot` (restores node
*contents* for a fixed set of node ids - it cannot undo a shape change);
`tree.hpp:1017`/`:1203` `collapseEmptyNodes` / `collapseSubtreeToLeaf`.
...
split-count feedback loop (`chain.hpp:1109-1113` recomputes split counts
...
`centeredSumOfSquares / residualVariance` - a fact documented in the engine
itself (`moves.hpp:69-71`). As the residual standard deviation sigma falls,
...
hands that forest weight `w_i * a^2` (`combiner.hpp:463-472`):
...
- **Inner composition**: the parametric part lives in the *leaves* - dbarts'
  ...
  linear leaf's `logIntegratedLikelihoodForNode` (`model.hpp:1023-1040`)
...
| outer composition, arbitrary block, user-driven | `dbartsSampler$setOffset` (`R/dbarts.R:1230`), and `dbarts_sampler_setOffset` in the shipped C API (`dbarts.h:366`) | shipped, supported |
| inner composition | `node.prior = linear(columns)` / `gp(columns)` (`R/model.R:886`, `:897`) | shipped |
...
- **The Metropolis-Hastings correction is provably 1** for the uniform
  form, verified by construction at `moves.hpp:461-526`: with the new
...
- The subtree-prior term below the node must still be computed in general
  ...
  `model.hpp:2062-2080`), and `changeMove` already computes it.
...
- *Interaction constraints.* A rotation lifts one variable above another -
  ...
  `tree.interactionSubtreeIsValid` (`moves.hpp:742`). That guard is
...
thread-parallel chain layout (`R/bart.R:522-523`), and destroys the
...
3. **The temperature cannot be chosen from the literature.** The BCF
   combiner hands a forest weight `w_i * a^2` (`combiner.hpp:463-472`), so
...
per-node candidate weights (`grow.hpp:130-150`), so the forward density is
...
The reset/regrow/rebuild/redraw loop already exists (`chain.hpp:1370-1387`)
...
- **More trees.** Tan et al.: "Increasing the number of trees consistently
  ...
  ambiguous." dbarts defaults to `n.trees = 75L` (`R/bart.R:519`), below
...
- **The change move's root sensitivity is not a bug.** It is the
  fixed-skeleton design working as specified (`moves.hpp:537-540`), and the
...
- **A same-variable ordinal cut redraw carries acceptance correction
  exactly 1** under shipped machinery (`moves.hpp:461-526`). This is the
...
| `model.hpp:1023-1040` (linear leaf integrated likelihood) | Read. Confirms the inner variant marginalizes leaf coefficients out of the structural score. |
| `R/dbarts.R:1230`, `dbarts.h:366`, `R/model.R:886,897`, `R/rbart.R:639` | Read. Confirms the composition surface is public on both the R and C sides, and that rbart_vi's loop is 1:1 alternation. |
...
- **`getTrees` is necessary but not sufficient.** It returns "a data.frame
  containing the internal state of the trees" (`R/dbarts.R:1792`) - flat
