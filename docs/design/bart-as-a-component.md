...
`refuseMultiForestMutation` (`src/R_interface_bartcore.cpp:2644`) fires on a
...
`refuseMultiForestResponseMutation` (`:2673`) is the one multi-forest family
...
`refuseUndefinedTestFits` (`src/R_interface_bartcore.cpp:2926`) closes the
...
`bartcore_runWithCallback` (`src/R_interface_bartcore.cpp:4569`) is the
