...
`refuseMultiForestMutation` (`src/R_interface_bartcore.cpp:2671`) fires on a
...
`refuseMultiForestResponseMutation` (`:2700`) is the one multi-forest family
...
`refuseUndefinedTestFits` (`src/R_interface_bartcore.cpp:2953`) closes the
...
`bartcore_runWithCallback` (`src/R_interface_bartcore.cpp:4609`) is the
