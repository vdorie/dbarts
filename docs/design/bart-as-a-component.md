...
`refuseMultiForestMutation` (`src/R_interface_bartcore.cpp:2651`) fires on a
...
`refuseMultiForestResponseMutation` (`:2680`) is the one multi-forest family
...
`refuseUndefinedTestFits` (`src/R_interface_bartcore.cpp:2933`) closes the
...
`bartcore_runWithCallback` (`src/R_interface_bartcore.cpp:4582`) is the
