...
`refuseMultiForestMutation` (`src/R_interface_bartcore.cpp:2810`) fires on a
...
`refuseMultiForestResponseMutation` (`:2839`) is the one multi-forest family
...
`refuseUndefinedTestFits` (`src/R_interface_bartcore.cpp:3092`) closes the
...
`bartcore_runWithCallback` (`src/R_interface_bartcore.cpp:4748`) is the
