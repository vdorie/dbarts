...
Our chain runs these as `metropolisJumpForTree` ([`metropolisJumpForTree`](../../src/bartcore/moves.hpp)) then
`sampleParametersAndSetFits` ([`Chain::sampleParametersAndSetFits`](../../src/bartcore/chain.hpp)). The constraint modifies each: the
...
column is `Tree::splitInterval` ([`Tree::splitInterval`](../../src/bartcore/tree.hpp)), the ancestor-constrained cut
interval used for availability (`hasAnyAvailableVariable`,
[`Tree::hasAnyAvailableVariable`](../../src/bartcore/tree.hpp),
[`birthableNodeExists`](../../src/bartcore/moves.hpp)). Neighbor determination for a constrained axis walks the tree's
...
(`leafCovariateColumns` [`SamplerOptions::leafCovariateColumns`](../../src/bartcore/chain.hpp); `columnTypes` is per-column too but
rides `PredictorSource` at [`PredictorSource`](../../src/bartcore/data.hpp), reaching the options through
`SamplerOptions::predictors` [`SamplerOptions::predictors`](../../src/bartcore/chain.hpp)) and consumed once at construction.
As shipped the field is `monotoneDirections` ([`SamplerOptions::monotoneDirections`](../../src/bartcore/chain.hpp)). A nonzero
entry on any column selects the constrained
instantiation at the factory (`createSampler`, [`createSampler`](../../src/bartcore/facade.hpp)); an all-zero
...
monotone scan, [`monotoneConstraintIsActive`](../../src/bartcore/facade.hpp)). ORDERED factors are their own `ColumnKind`
...
constant-leaf posterior, [`ConstantGaussianLeaf::drawFromPosterior`](../../src/bartcore/model.hpp), from the node's (sum w, sum wz)
...
[`Chain::sampleParametersAndSetFits`](../../src/bartcore/chain.hpp), then an independent `drawFromPosteriorForNode` per bottom node,
[`Chain::sampleParametersAndSetFits`](../../src/bartcore/chain.hpp)), the leaf-model concept exposes ONLY that per-node scalar
draw (`ScalarLeafModel`, [`ScalarLeafModel`](../../src/bartcore/model.hpp)), and the moves are leaf-templated free
functions that read the node sufficient statistics and ZERO leaf parameters
(`logLikelihoodForBranch`, [`logLikelihoodForBranch`](../../src/bartcore/moves.hpp)). Three real changes follow, none a
mere re-pointing:
...
1. A new concept method - a tree-granularity draw
   ...
   per-node loop at [`Chain::sampleParametersAndSetFits`](../../src/bartcore/chain.hpp), when the leaf model declares it. Independent
...
2. Leaf values must stay VALID through the move phase. The move score needs
   ...
   ([`metropolisJumpForTree`](../../src/bartcore/moves.hpp)). Those values already survive there: `sampleParametersAndSetFits`
   runs AFTER the moves and only then zeroes and refills `muByTree[t]`
   (`mu.assign(tree.nodes.size(), 0.0)`, [`Chain::sampleParametersAndSetFits`](../../src/bartcore/chain.hpp) - `mu` is a reference
...
The alternative seat - reusing `FunctionLeafModel::beginTreeDraw` ([`FunctionLeafModel`](../../src/bartcore/model.hpp))
and carrying mu as a degenerate function leaf - is rejected: it drags the
function-leaf test-cache and fits-are-parameters machinery ([`Chain::sampleParametersAndSetFits`](../../src/bartcore/chain.hpp))
...
lower, upper)` already exists ([`ext_rng_simulateTruncatedNormalScale1`](../../src/include/external/random.h), landed by the ordinal arc:
...
Gaussian marginals (the current `logLikelihoodForBranch`, [`logLikelihoodForBranch`](../../src/bartcore/moves.hpp)). The
...
[`logLikelihoodForBranch`](../../src/bartcore/moves.hpp) with the constrained joint marginal that reads the frozen
...
nodeScale/sqrt(numTrees), nodeScale = 0.5 ([`ConstantGaussianLeaf::scale`](../../src/bartcore/model.hpp), [`SamplerOptions::nodeScale`](../../src/bartcore/chain.hpp)). The
...
[`Chain::sampleParametersAndSetFits`](../../src/bartcore/chain.hpp)). Under truncation and a per-leaf-variable prior scale, the
...
- **Factory ([`createSampler`](../../src/bartcore/facade.hpp)):** any nonzero direction selects the monotone
...
- **Leaf store (chain.hpp):** muByTree must PERSIST across the move phase - stop
  the zero-rebuild at [`Chain::sampleParametersAndSetFits`](../../src/bartcore/chain.hpp) so the moves can read frozen neighbors - and
  the tree-granularity draw replaces the per-node loop at [`Chain::sampleParametersAndSetFits`](../../src/bartcore/chain.hpp)
...
- **Leaf model (model.hpp):** a constant leaf with a tree-granularity Gibbs draw
  ...
  ([`Tree::splitInterval`](../../src/bartcore/tree.hpp)).
- **Move (moves.hpp):** retired: `ConstrainedConjugateMove` never shipped as a
  ...
  ([`MonotoneConstantGaussianLeaf::redrawAfterBirth`](../../src/bartcore/model.hpp), [`MonotoneConstantGaussianLeaf::redrawAfterDeath`](../../src/bartcore/model.hpp))
...
growth is live, [`drawBirthableNode`](../../src/bartcore/moves.hpp) - and a b-leaf structure's reference marginal
