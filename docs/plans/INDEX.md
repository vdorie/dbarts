...
| file | STATUS | purpose |
|---|---|---|
| front-door-formals.md | DECIDED 2026-09-10 (dec-B116), S1 open | Sheds the seven `bart` scalars that mirror prior-object slots, moves `proposal.probs` onto `dbartsControl`, and gives `bart` and `xbart` a `control =` formal under a flat-wins precedence rule with fit-state controls refused. |
