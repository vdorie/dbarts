...
| classic-compare.md | RUN AND REPORTED, 2026-09-12 | Widens the cross-release evidence past equivalence.R's nine-scenario classic record: 25 scenarios written in the 0.9-x vocabulary, recorded under an installed dbarts 0.9-34 and under this release with every moved default pinned on both sides, and compared by the same Welch-z and disjoint-range verdict. Twenty-two scenarios agree at the null's own rate; the three that do not are the zero-weight fit (the positive-weight sigma degrees of freedom and veto rank, with a drop-the-rows reference showing 1.0-0 is the one that reproduces it) and the two crossvalidation rows (no chain carried across folds, with a hand-rolled fold-by-fold CV as the control). |
