...
| file | STATUS | purpose |
|---|---|---|
| column-kind-consolidation.md | PARTLY LANDED (S0, S1, S2, S3; S4 open) | Closes four defects sharing one root in the predictor store's semantic-type axis (no ordered-factor marker below the model matrix, an overloaded `numCuts`, an unchecked double-to-code cast, `FlatKind`'s duplicated cross-check) via a three-valued `ColumnKind` plus a derived `splitsBySubset` predicate, an ordered-factor midpoint cut grid (posterior-changing), engine-side ingestion validation, and a full native-integer-ingestion/typed-storage rework so a factor column's raw is its codes rather than a codes-plus-double pair; seven slices S0-S4c landing in the ruled order S0, S1, S2, S3, S4a, S4c, S4b. S0, S1, S2 and S3 have landed: the three-valued `ColumnKind` with a real `categoryCounts` field, the ordered-factor midpoint grid (draw-changing, baseline `equivalence-02d41365.rds`), `DBARTS_C_API_HASH` re-baked to `0xe14b499a84f501d2`, and engine-side ingestion validation with a matching bridge-side level-count ceiling. |
