...
| forest-cache-drift.md | IN PROGRESS, 2026-09-25 (removal implemented, 5eb0df0a; D4 and D5 ruled; speed compare done; gaussian equivalence re-record owed, that baseline stale on bartcore since 093dd035) | Removes the amplitude rescaling move, which scaled a forest's cached fits and its leaf tables separately, so the rounding gap between them compounded and biased every amplitude-coupled family; a measured comparison with the defect fixed showed the move did not earn its cost. A rule, two component pins and a debug-build check guard against any cache gap being multiplied across a sweep boundary. |
...
| archive/bcf-b-ridge.md | designed, not built, not scheduled | Treatment-scale (b) ridge interweaving move; cleared as the cause of the sigma residual (both controls clear it) and derived and prototype-checked as a separate mixing win, but never implemented (dec-A72). |
