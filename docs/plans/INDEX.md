...
| forest-cache-drift.md | LANDED 53583db0, 2026-09-25 (removal 12bb1607; D4 and D5 ruled; gaussian and BCF equivalence baselines recorded under d49e2103) | Removes the amplitude rescaling move, which scaled a forest's cached fits and its leaf tables separately, so the rounding gap between them compounded and biased every amplitude-coupled family; a measured comparison with the defect fixed showed the move did not earn its cost. A rule, component pins and a debug-build check guard against any cache gap being multiplied across a sweep boundary. |
