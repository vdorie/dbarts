...
| file | STATUS | purpose |
|---|---|---|
| engine-performance.md | PARTLY LANDED (S1 167e2c62, S3 70305575, weighted fused pass 046c35d7 landed; S2 measured and closed without shipping, dec-B113; S4, S5 open) | One gate matrix for the four engine items: a scalar reference build behind a configure flag with the vectorized draw path shipped, the multi-worker wait loop, within-chain threading measured and revived as an explicit opt-in, and the audit of the fixed engine constants with measurements and control settings. |
