...
were made. No entry the register has gained since is marked `not mine`, so the
table gains no members. One earlier entry has since turned `not mine` without
gaining a row: dec-B50, which the maintainer disowned as a misattributed
standing fact rather than a decision, so it has nothing to evaluate. Three rows,
dec-A03, dec-A40 and dec-A65, are now marked blank rather than `not mine`; their
rows stay as the record. Where a later entry or ruling changed what a row
evaluates, its `reason` says so.
...
| dec-A17 | fit object drops NULL-valued elements | KEEP | WEAKENED | KEEP | matches base-R practice of varying the return component set; the proposed NEWS addition does not apply, since indexing a missing list element by name returns NULL rather than erroring. Since narrowed: dec-B126 fixes the model descriptors by family and always writes them, so this verdict now covers only the draw channels |
...
| dec-A31 | errors are Rf_error longjmps only, no return codes | KEEP, name the leak sites | WEAKENED | KEEP | longjmp-only fits both known consumers' .Call frames; the leak claim was not reproduced in the C interface file but reproduces at the predict fan-out, which raises after building several owning containers. No longer applies as framed: dec-B119 catches exceptions in engine and callback code and raises the R error only at the entry point, fixing the leak sites; errors still reach a consumer as R errors, the part this verdict kept |
| dec-A32 | a non-void return is one of three documented kinds | KEEP | not reviewed | KEEP | no entry returns 0 with two meanings; the cost is reading which kind an entry is, not resolving an ambiguity. The header now documents two kinds, value and capability status, the third having left with the entries that returned it |
...
| dec-A44 | BCF treatment-forest ridge ships off | DISCUSS - unrun gate or baseline hygiene | OVERTURNED to KEEP | CLOSED | settled as dec-B127: the amplitude rescaling move is removed, and the treatment-forest switch that never ran goes with it |
...
| dec-A61 | equivalence only against baselines recorded from the new engine | CHANGE, adopt anchor-main.md sec 7 | OVERTURNED to KEEP | KEEP | benchmarks/R/change-balance.R and bd-balance.R are already permanent baseline-independent gates for the change move; the claim that no gate descended from the new engine can notice it reversed does not hold. Since then dec-B120 records a hand-run 26-scenario comparison against 0.9-34; no automatic gate compares against it |
...
| dec-A65 | a standing discretion grant settled user-facing forks | DISCUSS - ratify the adopted defaults, narrow the grant | WEAKENED | CLOSED | the register is itself the ratification mechanism the discussion asks for; entries dec-B74 through dec-B77 are VD rulings it has already produced, and VD ratified the multinomial defaults on 2026-09-12 (dec-A65) |
...
| dec-A68 | documented-but-inert arguments ship | CHANGE, drop tau and align the multinomial vars formals | WEAKENED | CHANGE | man/summary.bart.Rd already documents tau as a token silently dropped when absent, so it is not undocumented dead vocabulary; the multinomial vars asymmetry is real but the cheaper fix gives summary.bartMultinomial the vars formal its siblings carry. The run method's thread count, the third argument, was removed on VD's ruling of 2026-09-24; this verdict covers the other two |
| dec-A69 | reference-class forest index 1-based, C API 0-based | KEEP | WEAKENED | KEEP | 1-based R over 0-based C is the conversion at both named call sites; a third origin exists beyond those - sparseFactor takes 1-based positions and stores them 0-based in the exported S4 i slot |
...
| dec-B18 | BCF prognostic forest takes a half-Cauchy amplitude | KEEP | not reviewed | KEEP | the half-Cauchy fixture is bcf's own prior; the disagreement with the R-side family-aware default is unreachable, since no flat-C entry creates the amplitude sampler |
...
| dec-B62 | a per-sweep callback ships inline-multi-chain only | KEEP | not reviewed | KEEP | the conditioning setters a callback exists to call are sampler-wide, so per-sweep conditioning under threaded multi-chain runs has no meaning to preserve. The only entry that reaches it now is internal and single-chain, with no C header entry and no caller in dbarts |
...
- dec-A68: drop "tau" from the vars defaults that carry it and give summary.bartMultinomial a vars formal matching its draws siblings. Not applied at the tip: R/diagnostics.R still carries "tau" in eight defaults, the summary and draws methods of four fit classes, and summary.bartMultinomial takes no vars. The run method's thread count, outside this change, was removed under VD's 2026-09-24 ruling.
...
- dec-B41: settled 2026-09-08 as dec-B110, a warning keyed on the realized constant-leaf fallback share; landed, and the warning fires through the cross-validation, multinomial, ordinal and negative-binomial doors too.
