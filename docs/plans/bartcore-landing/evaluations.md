...
| dec-A70 | NA response errors rather than dropping rows | DISCUSS a missing = "omit" token | STANDS, redirected | DISCUSS | missing governs how trees route an NA predictor at a split, not row deletion; na.action is hard-set to na.pass with no user formal, so a remedy belongs there, not on missing |
...
| dec-B31 | saved states carry a format version, refuse on mismatch | DISCUSS a 0.9-x recognition message | WEAKENED | DISCUSS | NEWS.Rd already states that a state saved by an earlier version cannot be restored and must be refit; setState's class-mismatch path is reached only through an explicit call, so how often a 0.9-x user actually meets it is unclear |
...
| dec-B41 | document 10-25 trees for GP leaves rather than change the default | DISCUSS a message above n.trees ~50 | STANDS, strengthened | DISCUSS | at n.trees = 75 and max.leaf.size = 256, most leaves fall back to the constant leaf rather than running slow; a message keyed on the realized fallback rate says more than one keyed on n.trees |
...
| dec-B58 | the exact-ABI hash check is opt-in, lockstep is not the default | DISCUSS - keep the flag past 1.0? | WEAKENED | DISCUSS | both known consumers already opt in and the flag's removal is already scheduled in dbarts-h-freeze.md sec 6; the residual question is a TODO release-procedure line, not a reopened row |
...
| dec-B61 | item 1 covers seven entries; the basis setter is renamed, not transposed | KEEP item 1; CHANGE item 4 | STANDS | CHANGE | both extant callers already transpose the basis matrix before calling setForestBasis, so basisRowMajor exports an internal detail for a copy nobody else avoids; item 1's seven entries stand |
...
| dec-B73 | bit-identity of the draw path is a toggle (never built) | CHANGE to superseded | WEAKENED | CLOSED | the shipped rule and its opposite are already cross-referenced in dec-A42's and dec-B73's own cost columns; retiring dec-B73, a VD quote, waits on dec-A42's own revisit mark |
