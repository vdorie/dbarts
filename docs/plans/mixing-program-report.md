...
the cut keeps about half that gain on the gated mean function and all of it
on the other, for a thirtieth of the arithmetic. The proposal is to
...
cells do not exist, and the cost question is open.
...
chain-configuration arms on the same twenty seeds read
...
chains disagreeing enough that pooling supplies the width. The posterior is
right and the sampler is slow. The maintainer's ruling followed: the
coverage deficit is a mixing symptom and mixing is the lever, so the
statistic a change has to move here is per-chain effective sample size, not
coverage
...
gates, and eight pathologies, each isolating a mode the others do not
...
- **P5**, a checkerboard interaction on correlated columns, with an exact
  inclusion truth and strong near-decoys.
...
control, not merely statistically equal; and P1 checks the harness, since
unless its 90 percent coverage sits near 0.71 in the control arm no verdict
is valid. It reads 0.725 held-out at the shipped default
...
once as a check on that bar, not its source, and reads
-2.3 +/- 9.7 at a paired standard error of 2.17, putting four times at 8.7,
so +8 is marginally optimistic rather than wrong
...
is section 5's candidate kernel in full, at a share of 0.32 - twice that of
the arm whose coverage flag it dissolves - at four chains of 1000 burn-in
and 2500 kept. The eight-chain arm separating chain count from chain length
is the same kernel again.
...
and
the kernel producing it is an exact Gibbs step on the posterior the shipped
...
against an independently assembled neighbourhood, and a prior-only
detailed-balance gate carrying two poisons that both fail. What it is not
is independent: no kernel but the candidate mixes well enough here to serve
as a reference, which is the finding itself. Residual disagreement widens a
...
under the trees, this package's distinguishing use - change with swap at
zero matches the default on every contrast, and only the arm that drops
both loses
...
the sampler's subject-level hazard against a brute-force enumeration of all
62 reachable trees, where the maximum gap is 0.0120 against a 0.004
tolerance without swap and 0.0008 with it. Swap alone rotates a child's
rule up; at fifty and two hundred trees the ensemble averages the trap
away, so no default moved
([9. Reversal: the move returns at default zero](../design/swap-removal.md#9-reversal-the-move-returns-at-default-zero)).
...
ungated mean function, recorded as residue rather than survival, it gains
+9.5 summed on the fresh block, 19 of 20 seeds at t 5.35, with its own
1.027 held-out error regression
([5. Benefit, pre-registered](../design/perturb-move.md#5-benefit-pre-registered)).
...
Nog nodes are 48.5 to 98.3 percent of interior nodes by cell and take 62.7
to 99.1 percent of change's proposals, and the incumbent rule sits far from
the node's full conditional exactly where it matters:
...
The run also put C1's pooled scored acceptance at 24.9 percent against the
default Friedman cell's 8.0, so its minimum effective sample size of 2 is
no acceptance deficit - the reading that elevated section 6's
frozen-structure test.

### Cross-chain exchange

An exchange of one tree between two chains at one temperature is ordinary
Metropolis on the product target, the two tree priors cancelling, and it
was priced in closed form without drawing anything.
...
are also the couplings that would collapse the between-chain spread pooling
...
two cases and they are not the same law: where the incumbent sits in the
candidates' own empty-leaf-veto rank stratum - the only case with no weight
mask and no missing value at the node - the step is an exact Gibbs draw at
acceptance one; where the stratum entered is strictly better it is
...
share to 0.32 buys more effective sample size, +37.0 at one seed block, and
nothing else. Cost is 2.21 sweeps where a sweep costs one. On the ungated
function the arm gains +14.4 and +23.9 summed minimum ESS and pays 1.028
and 1.023 in held-out error, past the 1.02 margin at both blocks
...
change, and it sits behind a compile-time switch, off in every shipped
build; the balance script passes on it with both poisons failing.
...
coverage movement that flagged the full draw. The restricted kernel clears
the +8 bar at both seed blocks on the gated function, at t 3.1 against 7.5
and 8.1, and its coverage straddles the -0.010 margin rather than clearing
it
([2.4 Cost, and the two restricted variants](../design/nog-gibbs.md#24-cost-and-the-two-restricted-variants)).

What it has not been tested against: its run carried the bitwise null and
...
draw's run carried. Both cut-scan figures come from a
census cell at its own data and sampler seed on one chain of 200 + 500
sweeps rather than a replay of the arms' twenty seeds at four chains, and
that cell takes its 0.16 share from change and birth/death both. Neither
departure is scaled for.
...
draw is closed form, at about a five-thousandth of a sweep and no change of
rule
...
kernel alone in every column. KILLED as a general default.
The explanation is the pilot's scope: it measures one channel with the
structure fixed, where the sweep's structural half moves the level fibre
...
response swaps would never otherwise reach it. A non-frozen forest draws
nothing, so the shipped engine is bitwise unchanged
...
fresh seeds and averaged away by pooling. It stays opt-in, and its design
law - per-cell checks, frozen thresholds, mandatory fresh-seed re-runs, a
null control voiding the family - is what the battery inherits
...
gain of 1.06 to 1.17, a rider on an aim improvement rather than anything
alone.
...
it is not - the minimum rises only from 1.6 to between 4 and 21, at a
coordinate carrying two to three times the median posterior spread. The
leaf draw is slow there, which is why the level-fibre step helps a frozen
chain and why a structural kernel cannot be the whole answer.
...
draw, and why the core's coverage margin now names it.
...
not this kernel: on P5, dropping change costs 0.027 of the true columns'
inclusion share and 3.9 percent of held-out error, confirmed on fresh
seeds, and on P2 at one tree it stops the root variable moving.
...
`rule_gibbs` has recorded is on C1, a core cell, and it has won no
pathology. P2 ran as a must-not-degrade control, and by the design's own
account the move cannot reach the recorded parked chains: at one tree the
root is a nog node only while the tree has a single split, and every parked
chain sits at three or four interior nodes with a child split on the
confounded column. P1's rung was re-run as the absolute gate at the shipped
mixtures and carries no `rule_gibbs` arm; nor do P5 and P6. And the core
gate is checkable on one cell of four.
...
criteria, of which two matter here: the statistic is minimum effective
...
([6. Benefit, pre-registered](../design/nog-gibbs.md#6-benefit-pre-registered)).
That is section 2's ruling in arithmetic, accepted when the program was
framed around mixing rather than tree count. Defaulting this kernel accepts
that departure: it treats the core cell's own mixing statistic as the
target rather than as a gate, and leaves the pathologies untouched.

**The candidate for a default is the cut-only rule draw, and the records do
not yet propose it as one.** It clears the improvement bar on the gated
mean function at both seed blocks, gains the whole of the full draw's
benefit on the ungated one, moves coverage to the edge of the margin at one
block and past it at the other, pays 1.033 in held-out error there, and
costs 1.04 sweep-equivalents against 2.21. The record that measured it says
it is not proposed as a default and that its run settles only the price of
the variable axis; the default share is recorded as
post-release, blocked on a plateau-error gate that does not exist
([8. Slices](../design/nog-gibbs.md#8-slices)).

[DOSE RESPONSE: pending, filled from nog-gibbs.md section 6 when the run lands]

**On cost, three things that go together.** 1.04 sweep-equivalents is four
percent more arithmetic, not one; that figure is a cut-scan count, wall
time not being readable on these hosts; and the equal-cost arm, which
answers the per-second margin, is owed.
...
ledger's baseline footnote and four seeded-drift regression files re-record
in one bundle, and the consumer packages on their lockstep branches with
them: mechanical work against a bitwise oracle, not a risk. The restricted
draw costs more, its restriction being a compile-time switch rather than a
runtime option, so defaulting it means either deleting the variable axis
from `rule_gibbs` or adding a sixth name to `proposal.probs` with its own
fill rule, formals, slot, validity term, Rd pages and tests - twenty-four
files for the last name added.
...
seed blocks, with the pathologies untouched and the equal-cost arm and
plateau-error gate owed.

**A standing rule.** No exploration kernel is removed before this report is
read; a kill parks it at weight zero.

## 7. What was not measured

- **The equal-cost arm**, the control given the sweeps the cost ratio buys
  it, on a quiet machine
  ([6. Benefit, pre-registered](../design/nog-gibbs.md#6-benefit-pre-registered)).
  Host load ran 4 to 269, so no cost claim here is more than a scan
  count.
- **The plateau-error gate.** The house rule wants a per-cell harm check on
  plateau prediction error in a noise-heavy or large-n stratum before any
...
- **Whether composing BART with a parametric block helps tree-space
  ...
  factor of six and the benefit unmeasured
...
- **Non-gaussian families.** Every built cell is gaussian or causal
  gaussian, and coverage of a true mean calibrates rather than proving the
  posterior right
...
- `docs/design/benchmark-surfaces.md` - the battery: cells, rule, margins,
  every run of the five built cells.
