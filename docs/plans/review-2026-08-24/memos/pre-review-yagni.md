...
and task bodies in `src/misc/moments.c` (74 functions spanning :96-2770 of 2775).
...
`src/bartcore/chain.hpp:4066-4083`, plus the single-threaded moments.
...
(`src/bartcore/sampler.hpp:410, 636, 1271`) and rows with `misc_mt_`; a future
...
- `tools/build-aux/install-sh`: a 0-byte placeholder; `AC_PROG_INSTALL` is not in
  `configure.ac`.
- `inst/common/bartcoreHandle.R:425` `bartcorePredictPerForest`: zero callers in
...
`src/bartcore/model.hpp:1313-2109` (`GPGaussianLeaf`, 773 lines, the sole
`FunctionLeafModel` instantiation), `R/model.R:1452-1490` (`gp()`) plus
column/lengthscale resolution at `:198-290`, 6 refs in `R/xbart.R`,
...
`inst/include/dbarts/dbarts.h` X-macro entries :573 `numForests`, :575
`setForestBasis`, :579 `getForestFits`, :582 `numForestAmplitudes`, :584
`getForestAmplitudes`, :588 `setForestWeights`, :591 `getForestCalibration`,
...
(`docs/plans/capi-shape.md:204`).

Best value claim, and it is real: `docs/plans/dbarts-h-reshape.md:1264-1267`
...
- `dbarts_getForestCalibration` (:591) - 39 lines of implementation, its own
...
- `dbarts_sampler_setForestPriorScale` (:595) - its own documentation concedes a
...
`src/bartcore/combiner.hpp:1141` (impl), `:985` (branch), `:1478`
(`shippedShape()`), with `generalAmplitudeDraw` plumbing at `:348`, `:759`,
`:1518`.
...
The comment at `:960-972` names its own deletion trigger verbatim ("at which
...
`R/bartcore.R:687-709` (`bartcoreSamplerFromHandle(..., columns = NULL)`) and its
bridge at `src/R_interface_bartcore.cpp:3622, 3694-3720` - the 8th argument of
...
(`R/xbart.R:702`), which passes seven positional arguments and never reaches
`columns`; the six test call sites (`test-data-handle.R:32, 47, 114`,
`test-linear-leaves.R:190, 207`, `test-gp-leaves.R:237, 254`) never name it
...
`src/bartcore/combiner.hpp:316-353` (fields), `:362-378` (`expandForestSpecs`),
`:1424` (`synthesizeIndicatorBasis`), `:781` (guard).

Its own comment at `:326` says it: "a FIXTURE default that no consumer reaches -
`createAmplitudeSampler` has no flat-C entry point, and the bridge fills
`spec.forests` unconditionally." Confirmed at `src/R_interface_bartcore.cpp:2157`.
...
`R/sliceSample.R:1-44` (`rejectionSample`, two parallel implementations of the
same loop) and the `useLog` branches through `sliceSample` (313 lines total). The
single production call site is `R/rbart.R:676`, at the default `log = TRUE`.
...
Does not hold - `R/rbart.R:19` documents the contract inline ("on log scale"),
...
`R/multipleAssignment.R:118-139` (`unpack` and its method) has **zero** uses
outside `inst/tinytest/test-multipleAssignment.R`. Its sibling `massign` has
exactly four uses, all `R/partialDependence.R:44, 75, 215, 329`, all of the form
`massign[sampler, fit] <- ...` - two-element positional destructuring. The
named-argument branch of `[<-.lval` (:55-97, with duplicate-name warnings,
...
`R/partialDependence.R:214, 328` already carry `sampler <- fit <- NULL ## for R
...
- `R/bart.R:2972-2975` **`makeind(x, all = TRUE)`**: the body is
  `ignored <- all ## for R check` and then `makeModelMatrixFromDataFrame(x,
  TRUE)`. `man/makeind.Rd:25` documents `all` as "not currently implemented;
  retained for signature compatibility with `BayesTree::makeind`", and
  `inst/tinytest/test-makeModelMatrix.R:299` asserts
  ...
  three.
- `R/dbarts.R:1675-1689` **`getSigmas(result)`** and `:1695-1708`
...
- `R/utility.R:899-905` **`quoteInNamespace(name, character.only = FALSE)`**: 9
...
`inst/include/dbarts/dbarts.h:614`. `docs/plans/dbarts-h-freeze.md:42` states it
...
freeze doc records at `:198-199` that the accessor fails at the diagnostic it was
...
`inst/include/dbarts/dbarts.h:506` and `:600`.
`docs/plans/capi-callbacks.md:9-13` names the outer-Gibbs host (stan4bart), then
`:84-85` puts migrating stan4bart out of scope; `:23-24` records that callbacks
...
`src/bartcore/chain.hpp:127-132` (decl), `:694-701` (consume), `:764` and `:878`
(null-outs). R's `vars =` restriction routes exclusively through
`ForestStructureSpec::columns` (`R/spec.R:726` ->
`src/R_interface_bartcore.cpp:2197`). Writers of the `SamplerOptions` twin: 0 in
...
- `R/model.R:1633` **`dart(rho = NULL, b = 1)`**: never supplied anywhere in
  `R/`, `inst/tinytest`, vignettes, `man`, or the four consumers (`dart(a = )`
  *is* tested). `rho <= 0` falls back to `numPredictors`, so the non-default
  branch at `src/bartcore/model.hpp:2446, 2454` never executes in any test.
  Value: they are Linero (2018)'s own sparsity hyperparameters. Add coverage.
- `R/validateComposition.R:52, 142` **`seed`, `alpha`**: never supplied. `alpha`
  drives the nSim floor (:81), the band quantile (:93), and the Bonferroni
  correction (:225), and its own error message (:172) is untested; `seed` is the
...
- `R/dbarts.R:1986` **`printTrees(chainNums)`** has no default and is never
  supplied; resolved through `match.call()` at :1988.
- `R/bartcore.R:740` **`mu.blocks`**: reached only from
  `bartCause/tests/testthat/test-14-bcf.R:353`; absent from `inst/tinytest`
  entirely.

## 17. The per-generic "foreign argument" reason tables - KEEP (value named)

`R/generics.R:2050` (`foreignArgsFor`) plus five tables at `:2109-2164`
...
verdict for the 8 one-line refusal methods at `R/generics.R:3069-3078,
3096-3105`.

## 18. `PredictorUpdateResult::unsupportedSource` - produced, never distinguished - CUT (~4 lines)

`src/bartcore/sampler.hpp:86`, produced at `:1383` and `:1397`. Readers: zero.
`src/C_interface.cpp:238` collapses it into the `accepted ? 1 : 0` mapping;
`inst/tinytest/test-capi.R:1212` documents it as counterfactual ("Handing the
...
`src/bartcore/facade.hpp:86`, filled at `:443`. Sole reader:
`tests/cpp/test_facade.cpp:521-522`, which asserts the shape field equals the
`impl_` accessor it was copied from one line earlier. No R reader, no `dbarts.h`
reader, no behavioral test. (+ `chain.hpp:918` / `sampler.hpp:818` accessors if
nothing else reads them.)

## 20. `bart()`'s lowercase formals and the plotting knobs - KEEP (value named)

`R/bart.R:2736` `sigquant`, `printevery`, `numcut`, `proposalprobs`; also
`R/dbarts.R:2115` `treePlotPars`, `R/plot.R:476, 505` `cols`/`justmedian`,
`R/partialDependence.R:200, 314` `plquants`, `R/diagnostics.R:177+` `vars` at the
...
Best value claim: `man/bart.Rd:17` states that `bart` **is** "the
...
`R/A_class.R:236, 256, 332-338`; `R/dbarts.R:218, 231, 238`; consumed at
`src/R_interface_bartcore.cpp:170` (an opt-in fp32 running residual); refused at
`R/spec.R:450, 678`.
...
`R/dbarts.R:388, 408-410`; `R/bart.R:705, 721-725, 2698-2704`; three Rd files;
...
- **`R/sparseFactor.R:1-7`**: the header comment says the class "is recognized by
  ...
  ahead of engine support." `man/sparseFactor.Rd` and `R/utility.R:437-450` say
...
- **`DESCRIPTION`**: `person("Armon", "Dadgar", role = "ctb", comment = "adaptive
  ...
  restore the code).
- **`R/model.R:1556-1567`**: `normal(k = "chi(1.5)")` string-spec parsing,
  introduced as "compatibility with string specifications." Reached from
  `R/rbart.R:110` and two tests. Pre-release there is no compatibility to
...
- **Not YAGNI, but found on the way and worth its own look**: `R/dbarts.R:959`
  `run(..., n.threads = control@n.threads)` is never read in the body -
  `bartcoreSamplerRun(.self, numBurnIn, numSamples)` (`R/bartcore.R:145`) takes
  no thread count - while `man/dbartsSampler-class.Rd:56, :154` describe it on
  the same footing as `predict`'s, which *does* honor it (`R/dbarts.R:1145`).
...
- **No zero-registrant engine hooks.** `SweepCallback` has 2 registrants
  (`rbart_vi` at `src/R_interface_bartcore.cpp:4594`, flat C at
  `src/C_interface.cpp:605`); `pollInterrupt` 1 with 3 call sites;
...
- **No `#if 0`, no experimental `#ifdef`s, no dead `constexpr bool` flags** other
  ...
  `AmplitudeChain`'s family switch (`src/bartcore/chain.hpp:775-789`) are
...
- **`dbartsSpec(parentEnv = )`** looks unused in-repo but **is** supplied by
  stan4bart (`~/Repositories/stan4bart/R/stan4bart_fit.R:556`). Keep.
