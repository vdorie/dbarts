...
- **Burn-in sweeps are NOT recorded.** `chain.hpp:1365-1367` gates on
  `record = (iteration+1) % numThin == 0 && iteration/numThin >= numBurnIn`, and
  every store site is `if (record && ...)` (`chain.hpp:1494-1498`, `:1537`,
  `combiner.hpp:1375-1382`); `sampler.hpp:427-429` advances the cursor by
  `numSamples` only; `tests/cpp/test_state.cpp:210` already pins it.
...
the shape of `runWithBurnIn` (`R/bart.R:550-590`) minus its keepTrees toggle.
...
(`chain.hpp:2582-2584`). It survives serialization (E): `state.currentSampleNum`
round-trips (`sampler.hpp:666`, `:731`), readers ignore it. Partial fill (cap 6,
...
(`R/bart.R:2068-2070`); correct today, `predict(fit,x,"bart")` vs
`extract(fit,"bart")` best k=0 resid 1.9e-15. Ordinal (`:1820`) runs once.

## 2. Mechanism

**Store and write.** Per-forest circular buffer, slot-major
`capacity x numTrees` (`chain.hpp:426-432`, `combiner.hpp:221-232`), capacity
fixed at creation from `control@n.samples` (`R_interface_bartcore.cpp:1866`) or
by `setTreeStorage` (`:4881`), allocated whole up front (`chain.hpp:2546-2580`).
`Sampler::run` sets each chain's base to the sampler cursor
(`sampler.hpp:272`); the chain writes slot `(savedSlotBase + sampleNum) %
capacity` (`chain.hpp:1494-1498`; variance twin `:4244-4248`; the amplitude
rescale patches the same slot in place, `combiner.hpp:1375-1382`, so the glue is
folded INTO the saved leaves and cannot itself mis-pair), and the cursor then
advances by `numSamples` (`sampler.hpp:427-429`). Reset only by `setTreeStorage`
when keepTrees/capacity change (`:958-970`) and by a warm-start install (`:913`).

**Read (the defect).** All four readers walk slots 0..capacity-1 and never
consult the base: `sampler.hpp:548` (predictColumns), `:601`
(predictPerForestColumns), `:650` (predictVarianceColumns),
`R_interface_bartcore.cpp:7275-7281` (getTrees; its 0-based `sampleIndices` are
range-checked against capacity at `:7475`, `C_interface.cpp:842`). The bridge
sizes predict's draw axis as `capacity`, not the last run's `numSamples`
(`R_interface_bartcore.cpp:5615-5617`, `:5783`).

**bart()/bart2()/rbart_vi dodge.** `R/bart.R:1166-1171` forces keepTrees FALSE
whenever `n.burn > 0`; `runWithBurnIn` (`:550-590`) runs burn as
`sampler$run(0L, n.burn)` with storage OFF (capacity 0, so `sampler.hpp:428`
skips the advance), then re-installs keepTrees via `setControl` - flipping
capacity 0 -> n.samples resets the cursor (`sampler.hpp:969`) - and runs from
base 0. `rbart_vi` toggles identically (`R/rbart.R:939-941`, `:969-972`). Probe
G confirms the toggle is what saves them.

**Docs.** `docs/design/core-generalization.md:680-683` specifies the store
("capacity = n.samples at creation ... currentSampleNum advancing per run") but
is silent on read order; `docs/design/multinomial-mutation-arc.md:586-589`
records the carry as deliberate and measured;
`docs/plans/correctness-audit.md:433` asserts keepTrees indexing "is uniform
across store, read, and predict including circular wrap" - the false claim.

## 3. Options

**(A) Don't record burn-in.** Already true, pinned by `test_state.cpp:210`;
...
the `capacity` most recent recorded draws. Files: `sampler.hpp:548`, `:601`,
`:650` (one expression each plus a `savedSlotForDraw(i)` helper beside
`currentSampleNum()` at `:476`), and `R_interface_bartcore.cpp:7275-7281`.
`SamplerBase::currentSampleNum()` already exists as a virtual (`facade.hpp:202`,
`:480`): **no new virtual, no facade churn, no dbarts.h signature change**, no
`stateFormatVersion` bump (stays 2, `:6250`). Preserves the nbinom loop and the
Door 3 batching equivalence (both end with cursor 0, so their read order is
unchanged). Cost: `tests/cpp/test_state.cpp:225-240` asserts a partial run's new
draws land at output 0..1; under B they land at the tail (~6 lines).
`dbarts.h:837-849` and `?dbartsSampler` each gain one prose sentence.

**(C) Reset the cursor at each sampling phase.** One line at `sampler.hpp:272`.
**Disqualified**: breaks `bart2(family="nbinom")` outright (`R/bart.R:2068-2070`
records draw s in its own `run` call; with a reset every draw lands in slot 0),
deletes the accumulation idiom the ring exists for, contradicts `:231` above.
...
(numSamples 0 - no advance, `sampler.hpp:428`) then one recorded run
(`:1034-1087`), summarizing only run channels, never predict; the two that DO
read the store, `fitViaHazard` (`:1186`) and `fitViaHurdle` (`:1224`), go
...
5. C++: after `run(0,2); run(0,n)` on a capacity-n store, `predict` equals the
   second run's recorded `testFits` exactly; update `test_state.cpp:225-240`.
...
- **Semantics call, not just a fix.** B redefines predict's draw axis as "the
  ...
  changed return extent. `docs/plans/correctness-audit.md:433` is false and
