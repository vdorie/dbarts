...
`R/data.R:1164-1166`, `if (is.null(y)) { y <- rep(0, NROW(modelFrame)) }` - right for `dbartsData()` inside a composed sampler,
...
on 60 continuous values silently builds a **60-category** ordinal fit (levels `"-3.73309396896429"`, ...). `R/data.R:648-650`
only refuses `K < 2`; every sibling family guards its response (`R/spec.R:188` nbinom, `R/spec.R:206` probit).
...
**B2. `Depends: R (>= 4.0.0)` cannot be honored by a C++20-concepts package - pre-RC, 1 line.** `src/bartcore/model.hpp:40-145`
...
- **bartCause** (`dbarts-1.0`, `7ae6e83`, clean/pushed): the `alignForestBasisToSubset` failures at
  `tests/testthat/test-04-bartc.R:301`, `test-14-bcf.R:187`, `test-15-subset-pscore.R:38` are **consumer's to fix**. dbarts
  deliberately inverted the basis/subset contract *after* bartCause's tip - `4aa52288` (08-20), `47cdb96a` (08-24), both in
  NEWS.Rd - and `R/model.R:955-978` now refuses a basis whose row count matches the subset rather than the full data;
  `bartCause R/bcf.R:144` still pre-subsets on the formula path. **Two edits the recorded count misses**:
  `test-14-bcf.R:220-224` asserts the *old* direction, and the oracle at `test-14-bcf.R:16-17` pre-subsets its basis.
  **`TODO:306-309` still calls bartCause "ready and PUSHED ... Status OK"** - stale, contradicted by
  `docs/plans/bartcore-review-tour.md:463-467`. Correcting it is pre-RC.
...
- All three still owe the `TODO:302-305` re-verify against the final re-baked header.
...
calls this "essential": use `R CMD config CXX20`/`CXX20STD`/`CXX20FLAGS`; `configure.ac:29-34` uses plain `CXX`/`CXXFLAGS`.
...
- **README never shows how to fit a model**, and documents priors that are not reachable - pre-RC (surface). `README.md:12-27`
  ...
  `substitute`, `R/model.R:1608-1633`): `p <- dart()` gives `could not find function "dart"`, and `dbartsPriors` carries
...
- **A bad family token gives a bare `match.arg`**: `'arg' should be one of "auto", "gaussian", ...` - never names the argument,
  ...
  family, with no hint that `bart2` fits it. `R/bart.R:2621-2640` (`bartRedirectedFamilies`) already does this right, but only
  for `bart()`. By contrast the response/family *mismatch* messages are best-in-class and actionable (`R/data.R:571-614`).
  Separately, the formula-refusal message (`R/bart.R:2266`; also `R/dbarts.R:495,553,587`) says "use the matrix interface -
...
- **Minor**: `xbart` supports 4 of the 13 family tokens, so the new families cannot be cross-validated (capability gap, not a
  message problem). `Biarch: yes` is obsolete (32-bit Windows went in R 4.2). `gibbs_sampler_mixture_model.Rmd:398` links
...
- **The C++20 declaration is exactly right.** WRE (R 4.6.1) says a package "should" put the standard in `SystemRequirements`
  *and* "If it has a Makevars file ... this should include the line `CXX_STD = CXX20`". dbarts has both, in
  `src/Makevars.in:33` **and** `src/Makevars.win:23`; they agree, and check reports `specified C++20` with no mismatch. The
...
- **Portability + Windows + registration**: zero `#pragma` in `src/`; 5 `__builtin_*`, all behind configure/compiler guards
  (`misc/partition.c:28`, `include/misc/intrinsic.h:36,37,60`, `include/misc/alloca.h:19`); no `-O3`, `-march`, `-mtune`,
  ...
  `R_registerRoutines` + `R_useDynamicSymbols(FALSE)` (`src/R_interface.cpp:285-287`), the flat C API registered by expanding
...
- **Metadata and hygiene**: all 11 distinct URLs in DESCRIPTION/Rd/vignettes/CITATION/README return **200** (`urlchecker` not
  installed; checked with `curl -L`). Non-ASCII is one char, `inst/NEWS.Rd:2080`, under a declared `\encoding{UTF-8}` (line 3);
