...
[R/data.R:1164-1166](https://github.com/vdorie/dbarts/blob/7a8c7286b6b86ce0c6d12c492dde77b8b0ad25b4/R/data.R#L1164-L1166), `if (is.null(y)) { y <- rep(0, NROW(modelFrame)) }` - right for `dbartsData()` inside a composed sampler,
...
on 60 continuous values silently builds a **60-category** ordinal fit (levels `"-3.73309396896429"`, ...). [R/data.R:648-650](https://github.com/vdorie/dbarts/blob/7a8c7286b6b86ce0c6d12c492dde77b8b0ad25b4/R/data.R#L648-L650)
only refuses `K < 2`; every sibling family guards its response ([R/spec.R:188](https://github.com/vdorie/dbarts/blob/7a8c7286b6b86ce0c6d12c492dde77b8b0ad25b4/R/spec.R#L188) nbinom, [R/spec.R:206](https://github.com/vdorie/dbarts/blob/7a8c7286b6b86ce0c6d12c492dde77b8b0ad25b4/R/spec.R#L206) probit).
...
**B2. `Depends: R (>= 4.0.0)` cannot be honored by a C++20-concepts package - pre-RC, 1 line.** [src/bartcore/model.hpp:40-145](https://github.com/vdorie/dbarts/blob/7a8c7286b6b86ce0c6d12c492dde77b8b0ad25b4/src/bartcore/model.hpp#L40-L145)
...
- **bartCause** (`dbarts-1.0`, `7ae6e83`, clean/pushed): the `alignForestBasisToSubset` failures at
  ...
  NEWS.Rd - and [R/model.R:955-978](https://github.com/vdorie/dbarts/blob/47cdb96a4c5bbfe0889f92006f73e89ee649b8be/R/model.R#L955-L978) now refuses a basis whose row count matches the subset rather than the full data;
  ...
  **[TODO:306-309](https://github.com/vdorie/dbarts/blob/47cdb96a4c5bbfe0889f92006f73e89ee649b8be/TODO#L306-L309) still calls bartCause "ready and PUSHED ... Status OK"** - stale, contradicted by
  [docs/plans/bartcore-review-tour.md:463-467](https://github.com/vdorie/dbarts/blob/7a8c7286b6b86ce0c6d12c492dde77b8b0ad25b4/docs/plans/bartcore-review-tour.md#L463-L467). Correcting it is pre-RC.
...
- All three still owe the [TODO:302-305](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/TODO#L302-L305) re-verify against the final re-baked header.
...
calls this "essential": use `R CMD config CXX20`/`CXX20STD`/`CXX20FLAGS`; [configure.ac:29-34](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/configure.ac#L29-L34) uses plain `CXX`/`CXXFLAGS`.
...
- **README never shows how to fit a model**, and documents priors that are not reachable - pre-RC (surface). [./README.md:12-27](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/./README.md#L12-L27)
  ...
  `substitute`, [R/model.R:1608-1633](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/R/model.R#L1608-L1633)): `p <- dart()` gives `could not find function "dart"`, and `dbartsPriors` carries
...
- **A bad family token gives a bare `match.arg`**: `'arg' should be one of "auto", "gaussian", ...` - never names the argument,
  ...
  family, with no hint that `bart2` fits it. [R/bart.R:2621-2640](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/R/bart.R#L2621-L2640) (`bartRedirectedFamilies`) already does this right, but only
  for `bart()`. By contrast the response/family *mismatch* messages are best-in-class and actionable ([R/data.R:571-614](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/R/data.R#L571-L614)).
  Separately, the formula-refusal message ([R/bart.R:2266](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/R/bart.R#L2266); also [R/dbarts.R:495](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/R/dbarts.R#L495), [R/dbarts.R:553](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/R/dbarts.R#L553), [R/dbarts.R:587](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/R/dbarts.R#L587)) says "use the matrix interface -
...
- **Minor**: `xbart` supports 4 of the 13 family tokens, so the new families cannot be cross-validated (capability gap, not a
  message problem). `Biarch: yes` is obsolete (32-bit Windows went in R 4.2). [vignettes/gibbs_sampler_mixture_model.Rmd:398](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/vignettes/gibbs_sampler_mixture_model.Rmd#L398) links
...
- **The C++20 declaration is exactly right.** WRE (R 4.6.1) says a package "should" put the standard in `SystemRequirements`
  *and* "If it has a Makevars file ... this should include the line `CXX_STD = CXX20`". dbarts has both, in
  [src/Makevars.in:33](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/src/Makevars.in#L33) **and** [src/Makevars.win:23](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/src/Makevars.win#L23); they agree, and check reports `specified C++20` with no mismatch. The
...
- **Portability + Windows + registration**: zero `#pragma` in `src/`; 5 `__builtin_*`, all behind configure/compiler guards
  ([src/misc/partition.c:28](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/src/misc/partition.c#L28), [src/include/misc/intrinsic.h:36](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/src/include/misc/intrinsic.h#L36), [src/include/misc/intrinsic.h:37](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/src/include/misc/intrinsic.h#L37), [src/include/misc/intrinsic.h:60](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/src/include/misc/intrinsic.h#L60), [src/include/misc/alloca.h:19](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/src/include/misc/alloca.h#L19)); no `-O3`, `-march`, `-mtune`,
  ...
  `R_registerRoutines` + `R_useDynamicSymbols(FALSE)` ([src/R_interface.cpp:285-287](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/src/R_interface.cpp#L285-L287)), the flat C API registered by expanding
...
- **Metadata and hygiene**: all 11 distinct URLs in DESCRIPTION/Rd/vignettes/CITATION/README return **200** (`urlchecker` not
  installed; checked with `curl -L`). Non-ASCII is one char, [inst/NEWS.Rd:2080](https://github.com/vdorie/dbarts/blob/c44fcbc50c929be2734fa8476e9ffe641b385768/inst/NEWS.Rd#L2080), under a declared `\encoding{UTF-8}` (line 3);
