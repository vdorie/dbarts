...
## Citations of the tour by line
    ...
    docs/plans/review-2026-08-24/memos/prerc-lens2-backlog.md:73   -> tour:290
    docs/plans/review-2026-08-24/memos/prerc-lens3-external.md:49  -> tour:463-467
    docs/plans/INDEX.md:254                                        -> no line
    docs/plans/release-candidate-review.md:594, :1094              -> no line

Only **:290** and **:463-467** are line-cited. :290 still holds its cited
claim (the per-person-period LOO trap, "stated nowhere"). :463-467 still holds
...
stamp at :37 overstates what was done. Three anchor families do not hold - the
...
| 68 | "a 'Gaps' section (:924)" | STALE-FALSE | `(:929)` |
| 100-103 | "Two doc-vs-code divergences still open" (ColumnStore 16-bit, not arena-allocated) | unverified | see section 4 |
| 105-113 | "**One document that contradicts itself where a reviewer will land.** `docs/plans/variance-forest-mutation-routing.md:415-426` opens ... with 'CLOSED: built at c95a5e83' and then ... continues with the original present-tense description of the hole" | STALE-FALSE | That door was rewritten: :416-426 now closes cleanly ("Nothing remains open under this bullet ... the description of the hole that preceded it is retired with the door"). Cut the whole paragraph. The `feature-matrix.md:1016-1022` cite in it is also stale (now :1017-1027). |
| 125-141 | Stop 1 read-order anchors | STALE-FALSE | `facade.hpp (908)` - `SamplerBase` :138-413 (the `sed -n '138,413p' ... grep -c '= 0;'` count is still 59), `SamplerFacade` :415, `createConstantLeafSampler` :732, `createSampler` :789, `createSamplerOverStore` :842, `createAmplitudeSampler` :870, `createMultinomialSampler` :896. sampler.hpp and chain.hpp anchors all hold as written. |
| 169 | "`stateFormatVersion` is 3 (`src/R_interface_bartcore.cpp:6380`)" | STALE-FALSE | `(src/R_interface_bartcore.cpp:6446)` |
| 228 | "`setWeights`/`setSigma` refused by name (`src/R_interface_bartcore.cpp:2773`, :2898)" | STALE-FALSE | `:2791`, `:2928` |
| 272 | "DART is refused by name (`R/bart.R:901`)" | STALE-FALSE | `R/bart.R:902` |
| 298 | "`hurdleLogLik` (`R/generics.R:2045`)" | STALE-FALSE | `R/generics.R:2344` |
| 327-331 | "`heteroscedasticScale` (`R/generics.R:32`) ... in `pointwiseLogLikelihood` (:95) and, via `ppdNoiseScale` (:2754), in `sampleFromPPD` (:2785)" | STALE-FALSE | `:32` and `:95` hold; `ppdNoiseScale` is `:3115`, `sampleFromPPD` `:3146` |
| 338 | "(`src/R_interface_bartcore.cpp:7118`, :7433)" | STALE-FALSE | `:7184`, `:7499` (feature-matrix.md:1027 already carries these) |
| 379 | Stop 3 Known open: "`monotone-leaf-branch-fill` is a live TODO entry." | STALE-FALSE | "`monotone-leaf-branch-fill` is CLOSED, not adopted: benched 2026-08-26 with no measurable effect (`fillBottom` 0.18% of a monotone run, within noise), bitwise; the arm is archived at `archive/monotone-branch-fill` (98067fb3) and the memo is `docs/plans/review-2026-08-24/memos/monotone-branch-fill-bench.md`. Door left: `coneProbability`'s quadrature (TODO:126-129)." |
...
| 478-490 | Stop 6 entry map ("`R_interface_bartcore.cpp` is 7,759 lines", `_create` :3489 ... guards :2265 ...) | STALE-FALSE | 7,825 lines. `bartcore_create` :3541, `_createDataHandle` :3565, `_createFromHandle` :3619, `_createBCF` :3791; `_run` :4307, `_runWithCallback` :4551, `_growFromRoot` :4632; `_setOffset` :4649, `_setResponse` :4669, `_setSigma` :4693, `_setData` :4700, `_setWeights` :4935; `_setPredictor` :5119, `_updatePredictor` :5167, `_setActiveRows` :4074; `_setForestBasis` :3981, `_getForestAmplitudes` :4118, `_getForestVariableCounts` :4280; `_getCalibration` :4205, `_setCalibration` :4260; `_storeState` :5684, `_setState` :5689, `_installForests` :5702; `_predict` :5837, `_predictPerForest` :5938, `_getTrees` :6022, `_getLatents` :6171. Guards: `refusedAmplitudeFamilyReason` :2266, `refuseMultiForestMutation` :2618, `refuseMultiForestResponseMutation` :2647, `refuseBinaryWeightChange` :2791, `refuseUndefinedTestFits` :2890, `refusePinnedSigmaChange` :2928, and NEW `refuseNonBinaryMask` :2946 (shared with the flat entry since 9df0cb50). |
| 498 | "`refusePinnedSigmaChange`'s own comment (:2885-2897)" | STALE-FALSE | `(:2915-2927)` |
| 511-514 | "`dbarts.h` is 1,158 lines ...; `C_interface.cpp` 1,175. Constants: `DBARTS_C_API_MAJOR 1` (:103), `DBARTS_C_API_MINOR 0` (:104), `DBARTS_C_API_HASH 0x66d33f1613892406ULL` (:142)." | STALE-FALSE | "`dbarts.h` is 1,307 lines (was 477); `C_interface.cpp` 1,315. Constants: `DBARTS_C_API_MAJOR 1` (:143), `DBARTS_C_API_MINOR 0` (:146), `DBARTS_C_API_HASH 0x5a32aa4cd3872d55ULL` (:189)." Two re-bakes since the tour was written (freeze -> 0x0939c022...; capi-shape -> 0x5a32aa4c...). |
| 516-534 | Stop 7 body: threaded predict as the whole story | STALE-INCOMPLETE | Threaded predict is now the *older* of two ABI events. Must add the capi-shape slice (9df0cb50): (i) **a three-class return doctrine** stated at CAPI:53-69 - VALUE (12 entries), TRANSACTION (`setPredictor`/`updatePredictor`), CAPABILITY STATUS (16); (ii) **seven setters void -> int** (`setResponse`, `setOffset`, `setWeights`, `setSigma`, `setTestPredictors`, `setTestOffset`, `predict`), 0 meaning "this sampler cannot, nothing was touched", a raise meaning "this call is wrong"; (iii) **`forest` moved to argument 2** on `getTrees` (:1023) and `printTrees` (:1034), the last two that took it after `useLiveTrees` - a pointer-to-integer change at position 2 that fails to compile in C++ and may only warn in C, with the hash re-bake as the real backstop; (iv) `setForestBasis`'s `basis` renamed **`basisRowMajor`** (:1137), naming the one documented exception to the column-major rule (:106-110); (v) `setActiveRows` now **raises** on a non-binary mask instead of answering 0 (src/C_interface.cpp:1174-1189, shared `refuseNonBinaryMask`), so its 0 is purely a capability answer; (vi) dbarts.h no longer defines `USE_FC_LEN_T` (nor includes `<Rversion.h>`) - a consumer that relied on that pull-in now includes it itself. Entry count stays 48; MAJOR/MINOR do not move. Landing note: `docs/plans/capi-shape.md`. |
| 521-523 | "`dbarts_sampler_predict` gained `size_t numThreads` as its new last parameter (:861)" | STALE-FALSE | It is the fourth of five parameters (`out` is last), at :990, and the entry now RETURNS `int`: a capability status, 1 with `out` written or 0 leaving `out` untouched where the sampler's blend is undefined. |
...
| 576-584 | "(`bartRedirectedFamilies` `R/bart.R:2621`, `refuseBartRedirectedFamily` :2634)" and "`checkFamilyUnsupportedArgs` (`R/bart.R:620`)" | STALE-FALSE | `:2699`, `:2712`; `:618` |
| 586 | "`$n.chains` is set unconditionally on every fit that keeps a sampler." | STALE-FALSE | "`$n.chains` is on every fit, kept sampler or not: `packageRbartResults` sets it unconditionally (R/rbart.R:1295) since 52c10e02, closing the gap where an `rbart_vi()` fit that kept its samplers carried none and `fitNChains` had to infer it." |
| 590-595 | "`tools/check-rc-codoc.R` (259 lines, wired into `lint.yaml`)" | OK | 259 lines, confirmed |
| 597-601 | Stop 8 Known open: "**`predict(bases =)`, `predict(sample =)` and `fitted(combineChains =)` on the own-class fits are still silent no-ops** - explicitly outside the judgement table ... do not assume the generics work closed every swallow." | STALE-FALSE | All three are refused by name since d48aef8a. Replace with: "**Foreign arguments are now refused by name across the surface.** `refuseUnusedGenericArgs`/`foreignArgsFor` (R/generics.R:2028, :2050) compose four derived reason tables - `predictForeignReasons` :2109, `extractForeignReasons` :2118, `fittedForeignReasons` :2128, `residualsForeignReasons` :2141, plus `survivalProbabilitiesForeignReasons` :2154 - so `predict(bases =)`, `predict(sample =)` and `fitted(combineChains =)` on the own-class fits now name the channel that serves the caller instead of vanishing. One inert argument survives on purpose: `as_draws_array`/`as_draws_df.bartMultinomial`'s `vars` (R/diagnostics.R:226, :229), documented rather than refused because the `posterior` generics fix the signature." |
| 601-604 | "`setForestBasis(k, ~var)` still evaluates the formula in `environment(basis)` ..." | OK | Still true: `$setForestBasis` calls `evaluateForestBasis(basis)` with `data = NULL` (R/dbarts.R:1476 -> R/model.R:808-820). |
| 604-606 | "`plotTree`'s dead padding branch stays out"; "`dbartsData(counts =)` with a formula still reaches its own later refusal" | OK | The counts/formula refusal is live at R/data.R:1287. plotTree padding not re-checked in detail. |
| 606-608 | "**`par(mfrow)` restoration is now partial**: ... but `plot.bart` and `plot.rbart` set `mfrow` through the shared `plotSigmaTrace` (`R/plot.R:9-12`) and still leak it." | STALE-FALSE | Fixed at fcbbc478. `plot.bart` (R/plot.R:67-68) and `plot.rbart` (:133-134) both `par(no.readonly = TRUE)` + `on.exit(par(oldpar))`; `plotSigmaTrace` gained `setLayout =` so a caller with its own layout is not reset. Replace with: "**`par(mfrow)` is restored by every plot method.** All eight save and restore (`R/plot.R` :67, :133, :232, :314, :374, :423, :521 and `plotTree`); `plotSigmaTrace`'s `setLayout = FALSE` (:9) is the one caller-owned-layout escape." |
| 597-608 | Stop 8 Known open, as a list | STALE-INCOMPLETE | Missing every surface-refusals fact a reviewer needs: `fitted()`'s positional slot 3 is now `ci.level` on all six methods, and `fitted.bartHurdle` lost `sample` (R/generics.R:910, 1185, 1515, 1804, 2363, 2889); `plot.bartMultinomial` reordered to `(x, plquants, cols, ...)` (R/plot.R:226) matching its five siblings, with `plot.pdbart` deliberately a fourth shape; `type = "class"` with `ci.level`, `ci.level` on `type = "forest"`, forest selection outside the forest arm and `residuals(ci.level =)` all refused; `$getSigmas`/`$getSumsOfSquaredResiduals` refuse a vestigial `result` (R/dbarts.R:1675, :1695); `coerceOrError` refuses a fractional integer (R/utility.R:173-180). Landing note: `docs/plans/surface-refusals.md`. |
| 624 | "the ladder is `resolveTreePrior` (`R/bart.R:484`)" | STALE-FALSE | The symbol does not exist. It is `resolveDartShorthand` (`R/bart.R:467`), with the collision check at `R/bart.R:894-905`. |
| 629 | "`refuseUnsupportedAmplitudeComposition`, `src/R_interface_bartcore.cpp:2323`" | STALE-FALSE | `:2316` |
| 636-644 | setSigma: "R5 `R/dbarts.R:1504`, bridge :4627 ... `src/R_interface_bartcore.cpp:2898`" | STALE-FALSE | R5 `:1504` holds; bridge is `:4693`; the guard is `:2928`. Add that the flat entry now returns a capability status rather than raising. |
| 649-657 | "`estimateSigmaFromLinearModel` (`R/utility.R:542`) ... floored at ... (:617) ... re-checked *after* the fallback (:618) ... the sparse branch returns at :562" | STALE-FALSE | `estimateSigmaFromLinearModel` is `R/utility.R:551`; the three interior cites shift with it - re-derive rather than offset. |
| 675-676 | "`R/dbarts.R:1398` -> bridge :4022 -> `Chain::setActiveRows` chain.hpp:1666 -> facade.hpp:368" | STALE-FALSE | bridge is `:4074` (`:4022` is `bartcore_setForestWeights`); facade is `:370`. chain.hpp:1666 holds. |
| 679-684 | active-rows "Open:" list | STALE-INCOMPLETE | Add: the flat entry's value refusal now RAISES (shared `refuseNonBinaryMask`, `src/R_interface_bartcore.cpp:2946`), so a flat-C 0 means only "this family carries no mask". |
| 688 | "R5 `installTrees(donor, samples)` (`R/dbarts.R:1888`)" | STALE-FALSE | `R/dbarts.R:1973` (`:1888` is inside `reapplyForestWeights`) |
| 702-705 | "`extract.bart` (`R/generics.R:425` ...), `.bartMultinomial` :966, `.bartOrdinal` :1297, `.bartNegbin` :1576, `.bartHurdle` :2001, `.rbart` :2379" | STALE-FALSE | `extract.bart` :471, `.bartMultinomial` :1062, `.bartOrdinal` :1417, `.bartNegbin` :1715, `.bartHurdle` :2292, `.rbart` :2700; shared `pointwiseLogLikelihood` :56 |
| 712 | "`$getCalibration`'s docstring (`R/dbarts.R:1731`)" | STALE-FALSE | `R/dbarts.R:1794` (method at :1793) |
...
| 947-950 | "feature-matrix.md - ... a Gaps section (:924)" | STALE-FALSE | `(:929)` |
...
| 79 | gaussian flat C `S CAPI:678` | STALE-FALSE | `S CAPI:748` (the `dbarts_sampler_create` family paragraph; :678 is inside the `DBARTS_USE_STUBS` macro block, which the dbarts.h freeze inserted ahead of the prototypes) |
| 81 | probit flat C `S CAPI:676` | STALE-FALSE | `S CAPI:747` |
| 86 | aft flat C `S CAPI:679` | STALE-FALSE | `S CAPI:749` |
| 89 | bcf flat C `S CAPI:699-721` | STALE-FALSE | `S CAPI:770-792` - which is exactly what this file's own [f3] (:255) already cites for the K-forest paragraph, so the file contradicts itself |
| 161 | ordinal pointwise loglik `S generics.R:1425` | STALE-FALSE | `S generics.R:1437` (the `if (type == "loglik")` branch of `extract.bartOrdinal`; `ordinalLogLik` is :1488, which the Gaps section at :956 already cites) |
| 162 | nbinom `S generics.R:1741` | STALE-FALSE | `S generics.R:1750` (`negbinLogLik` :1778, per Gaps :963) |
| 163 | multinom `S generics.R:1096` | STALE-FALSE | `S generics.R:1105` (`multinomialLogLik` :1131, per Gaps :974) |
| 166 | hurdle `S generics.R:2300 [f25]` | STALE-FALSE | `S generics.R:2317 [f25]` (`hurdleLogLik` :2344, which [f25] itself cites at :620) |
| 168 | grouped `S generics.R:2802` | STALE-FALSE | `S generics.R:2812` |
| 157-165, 169 | nameable calibration column, nine cells reading `dbarts.R:1793, 1753` | STALE-FALSE | `dbarts.R:1793, 1820`. `:1793` is `getCalibration`; `setCalibration` is `:1820`, not `:1753` (`ptr,`). [f16]'s own later text at :497 already says `dbarts.R:1820-1850`, and [f23] at :604 says `:1847`. |
| 175 | gaussian variance forest `S FAC:795` | STALE-FALSE | `S FAC:798` (the `if (options.numVarianceTrees > 0 && ...)` gate in `createSampler`) |
| 184 | hurdle DART `S bart.R:2346, 2355 [f35]` | STALE-FALSE | `S bart.R:2359, 2368 [f35]` |
| 185 | bcf variance forest `R FAC:874 [f48]` | STALE-FALSE | `R FAC:879 [f48]` |
| 115, 840 | "`createAmplitudeSampler` FAC:868-889" (twice) | STALE-FALSE | `FAC:870-891` |
| 211-215 | "inst/tinytest/test-capi.R drives only the `\"\"`/`\"probit\"` tokens plus grouped (:697) and heteroscedastic (:222) by control attribute, and BCF (:1356)" | STALE-FALSE | `grouped (:785) and heteroscedastic (:803) ... and BCF (:1475)`. The claim itself (logistic, aft, ordinal, nbinom reachable and untested through dbarts.h) still holds. |
| 342 | [f12] "`bart2Hurdle` (bart.R:2304) composes ... an occupancy probit (bart.R:2346) and a lognormal positive part (bart.R:2355)" | STALE-FALSE | `(bart.R:2317)`, `(bart.R:2359)`, `(bart.R:2368)` |
| 355 | [f13] "The flat C API guards through the same call (C_interface.cpp:614, 636)." | STALE-FALSE | `(C_interface.cpp:658, 682)` - the two `refuseGroupedScaleUpdate` calls in `dbarts_sampler_setResponse`/`setOffset`; :614 and :636 are now inside `run`/`setCallback` |
| 364-371 | [f15] "the facade's pure virtual FAC:368 and its shape probe FAC:108" | STALE-FALSE | `FAC:370` (FAC:108 holds) |
| 431-439 | [f15] "The flat-C entry, `dbarts_sampler_setActiveRows`, LANDED ... body `C_interface.cpp:1174-1187` ... (`inst/tinytest/test-capi.R:1401-1446` pins a genuine mask, an all-ones no-op, a fractional refusal, a NULL clear, and a probit mask moving draws)" | STALE-INCOMPLETE | Anchors hold (:1174-1189 now; the test block still starts at :1401). But the semantics changed at 9df0cb50 and the footnote should say so: the fractional case now RAISES through the shared `refuseNonBinaryMask` (R_interface_bartcore.cpp:2946) rather than answering 0, so the entry's `int` is a pure capability status under the header's three-class doctrine (CAPI:53-69). |
| 464-465 | [f16] "facade FAC:354, 361" and "R5 ... dbarts.R:1793, 1753" | STALE-FALSE | `FAC:356, 363`; `dbarts.R:1793, 1820` |
| 756, 765 | [f34]/[f35] "bart.R:2346, 2355" | STALE-FALSE | `bart.R:2359, 2368` |
| 250-255 | [f3] header/attribute claims (CAPI:744-754, NOT CAPI:510-529, K-forest CAPI:770-792, four attributes undocumented) | OK | All four verified at 3080a9c5; the four selecting attributes are still absent from the header (only `"bartcore.survival"` appears, :753) |
...
| 1017-1027 | Gaps, heteroscedastic: "(RIB:7184, RIB:7499)" and the `columnMaskStateFeasible` closure | OK | Anchors hold; this is the passage the tour's :105-113 says is correct, and it still is |
...
- **`docs/plans/surface-refusals.md`** -> Stop 8, replacing the "Known open"
  ...
  beside `R/generics.R:2109-2166`; its section 17 is the residue list that
...
1. **tour:100-103** - "`ColumnStore` (data.hpp:501) uses uniform 16-bit codes,
   no per-column width ... and it is still not arena-allocated -
   `docs/architecture.md` says both in its own voice." I confirmed
   `ColumnStore` is at data.hpp:501 and that `hot-layer-u8.md` is a measured
...
3. **matrix cell VALUES** (as distinct from anchors). The file's own preamble
   ...
   forest) and the bcf pointwise-loglik `M`/`?` question at :1003-1009 are
...
| Consumer migration cost | one sentence at :526-527, threaded-predict only | ABSENT in substance |
...
| The one structural idea (leaf model = compile-time `L`) | Stop 0, :81-85 | CORRECTLY front-loaded; 5 lines that pay for themselves |
| Three renames that make greps lie | Stop 0, :91-98 | CORRECTLY front-loaded; prevents a false conclusion |
...
- **The recent-arcs appendix (:839-923, 579 w).** Eleven arcs of commit
...
- **Process narration**: the citation convention (:15-23), the anchor
  re-derivation history (:855-860), the "two doc-vs-code divergences" and "one
  document that contradicts itself" paragraphs (:100-113). These are
...
- :81-85 the one structural idea -> new s0
- :91-98 three renames that make greps lie -> new s0
- :755-768 the CI dormancy finding, with the cross-host amendment -> new s3
- :771-776 exact-gates, plus the two cross-host compares -> new s3
- :796-803 the calibration lane's 11 families / 75-of-83 verdict -> new s3
