...
predictColumns/predictPerForestColumns/predictVarianceColumns (sampler.hpp:565, :621, :674),
Chain::predictFromSavedSample / ...Multi / predictPerForest... and their live twins (chain.hpp:2797-2985),
addFlatPredictions (:2758), and the tree.hpp kernels addFlatPredictionsBelow / addFlatLinearPredictionsBelow /
partitionFlatIndices (:1723-1900).
- Adapters: DenseColumns/DenseColumnReader (tree.hpp:1700-1719), PredictorSourceColumns / SparseRawColumn /
  PredictorSourceColumnReader (data.hpp:346-442). All built in the constructor, `column(j) const` returns by value,
  `SparseRawColumn::at` is a pure popcount lookup with no search cursor and no memo. One shared `columns` object is
  safe.
- Accessors the replay reaches - model.hpp:984-988, :1347-1351 (covariateColumns/Means/Sds/lengthscales/
  numParams/numCovariates) and fitScale/fitShift/sigmaScale (:2718-2719 + overrides) - are plain returns of stored
  members. No lazy init.
- `mutable` audit over src/bartcore: model.hpp:521, :1302-1303, :2093-2107, :2121-2127 and tree.hpp:1604-1626 are all
...
- SIMD dispatch (misc/simd.c:193-350): `misc_setVectorToConstant` / `misc_addVectorsInPlace` are file-scope function
...
(sampler.hpp:1011-1014) and `dbarts_sampler_setNumThreads` (C_interface.cpp:875-878) store the value with NO
validation; dbartsControl's check (R/A_class.R:349-350) guards only the R path. A C consumer that sets 0 and then
...
(sampler.hpp:583-591), but the load-bearing half is that the partition is over SLABS: one slab per chain plus a slab
...
forests_[0].numTrees. A multinomial replay traverses sum_f numTrees_f (chain.hpp:2871-2884), the per-forest replay
...
(C_interface.cpp:773-802) and `predictFromSource` (R_interface_bartcore.cpp:5654-5758). Every refusal and every
R_alloc is strictly BEFORE the engine call: refuseUndefinedTestFits (:5781 / C_interface.cpp:780),
refuseEmptyTreeStore, translateSource (C_interface.cpp:185-241 - the R_alloc the header's main-thread warning is
about), validateTestSource, the offset-shape refusals (:5666-5686), and the result allocation. No ext_rng/unif_rand
and no Rf_error/ext_printf/R_alloc under chain.hpp's replay block. dbarts.h:45-47's main-R-thread-only contract is
...
R_interface_bartcore.cpp:4224-4231, is R_ToplevelExec-wrapped and used only by run). So deferring interrupt polling
...
**12. CONFIRMED-OK - mirror run's fan-out; do not reuse testFitPool_.** sampler.hpp:349-420 and the grow-from-root
twin :1072-1099: std::thread, numWorkers = min(numThreads, numChains), pthread_sigmask(SIG_BLOCK, SIGINT) around the
spawn under `#ifndef _WIN32`, mask restored immediately, join at the end. Portability is configure-provided
(configure.ac:53 AX_PTHREAD; src/Makevars.in:18-34 threads the flags through), and Windows needs no mask because R's
console Ctrl-C already lands on the main thread. testFitPool_ (chain.hpp:5466-5467) is per-Chain, budget
...
docs/plans/release-candidate-review.md:2529-2531 records the single trip as "xbart's auto n.threads trips CRAN's core
limit" (xbart uses `parallel::makeCluster`, man/xbart.Rd:61) "fixed with an explicit n.threads = 1L and the
as-cran-for-thread-spawning-tests lesson recorded in the runbook", and :2533 records the P8 battery running tinytest
...
**16. MINOR - the hash re-bake is TWO literals.** src/C_interface.cpp:461 `static_assert(dbarts_apiSignatureToken ==
0x85bd1ef04beb3848ULL, ...)` fires on any SIGNATURE change; dbarts.h:142 DBARTS_C_API_HASH on signature-or-layout.
Adding a parameter moves both. Both fail loudly with instructions, so it is self-correcting, but the flat-API cost
line names only one. stan4bart pins the hash by hard equality (src/init.cpp:972), so the lockstep break is real and
intended.

**17. MINOR - no default arguments on the facade virtuals.** facade.hpp:258/267/272 with overrides at :545/:549/:554
...
default arg; explicit at every call site. Also give the non-virtual dense convenience spellings (facade.hpp:276-286,
re-exposed by `using SamplerBase::predict;` at :542-543) the parameter, or the dbarts.h-shaped overloads quietly stay
...
failure mode.** It enumerates every virtual (FacadeVirtual list :40-41), spies them (`SPY_VOID(predict, ...)`,
`SPY_VOID(predictPerForest, ...)`, `SPY_VOID(predictVariance, ...)` at :140-146) and conformance-checks all three
predict virtuals at :752-785. A signature change forces edits in all three places; the cost table has no line for it.
...
the symbol. Besides R/dbarts.R:1140 and R/bartcore.R:1472, four test files call the entry point DIRECTLY with three
arguments: test-predict-sparse.R:163 (inside `expect_identical` - a hard error), test-multinomial-test-offset.R:405
and :414, and test-multinomial-category-offset.R:430 (inside `expect_error` with message patterns - they fail on the
arity message instead of the target refusal). Add R/bartcore.R:1490 and R/dbarts.R:1153 for the per-forest arity.
Loud, but unbudgeted.

**20. MAJOR - bartCause is not "ZERO lines" without a placement rule.** bartCause dbarts-1.0 @7ae6e83
R/generics.R:141-230: predict.bartcFit forwards `...` into FOUR different predicts - dbarts predict.bart,
...
inserting `n.threads` before `group.by` in predict.rbart (R/generics.R:1647-1657) would pass a factor as a thread
count. The migration is still zero LINES, but only under that constraint, and the memo says only "add the formal to
each". Each generic also needs its own default expression: predict.bartHurdle (:1614) has no `object$fit` at all (it
is `object$occupancy$fit`). stan4bart is exactly as described: one C site, src/init.cpp:342 in predictBART (:293),
hash equality :972, `-DDBARTS_USE_STUBS` (src/Makevars.in:1).

**21. MAJOR - predict.bart's validation sits BELOW two early returns, and predictBlend is missing from the site
list.** R/generics.R:207-300: `type == "forest"` returns through `predictForest` at :660 and an amplitude-coupled fit
through `predictBlend` at :672, both BEFORE `n.threads <- as.integer(n.threads)[1L]` at :294. Putting the
house-pattern validation at :294 leaves the amplitude family's n.threads unvalidated. And predictBlend (:727-738) is
...
never touches: the flat-offset add over EVERY slab (R_interface_bartcore.cpp:5736-5739 - one pass over the entire
output, 76 MB at the memo's nTest = 10000); `Rf_duplicate(resultExpr)` on the heteroscedastic path (:5745, a full copy
...
partialDependence calls `sampler$predict(x.test)` per grid level with NO n.threads (R/partialDependence.R:242, :244,
:346, :363, :365), taking the R5 default silently once per level; the fit-time budget was sized for CHAIN parallelism
...
predict both execute serially" (man/dbartsSampler-class.Rd:152-157) is half false, so splitting it is forced, not
optional.
...
constexpr carrying the ns/traversal derivation, in the style of `testFitParallelCutoff` (chain.hpp:5467).
...
anti-vacuity pass; the xbart pin at release-candidate-review.md:2529). On a loaded runner elapsed inflates and the
...
**34. MINOR - the sanitizer recommendation rests on a false premise.** sanitizers.yaml:77 runs
...
around the 2800/5600-5800 band: predictFromSource is :5654 (memo :5641), refuseUndefinedTestFits defined :2871 and
called :5781 (memo :2858, :5768). Everything else spot-checked is exact - facade.hpp, sampler.hpp, chain.hpp,
...
6. Add test_facade.cpp, the four direct `.Call` tinytest sites, R/bartcore.R:1490 and src/C_interface.cpp:461's second
