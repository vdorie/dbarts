...
sigma is the fixed unit residual of parameterisation A (heteroscedastic.md:279-281, `sigmaIsFixed_`)
rescaled by the y range - no posterior content at all. TWO CHANNELS THE MEMO MISSES, both live:

- `predict(type = "ppd")` (generics.R:270-296) computes `s` into a local at :273-279, attaches it as
...
[f28] (:802-805), which the memo's sec 3 concedes.
...
`splitVariableLogProbability` (model.hpp:2135-2148) scores them against `collectAvailableVariables`,
which excludes the masked column, so s(x) depends on a column the declaration forbids. Cite drift
only: the false comment is chain.hpp:3345-3346, the variance pass :3384-3403.
...
Case B builds different data from the same arguments, no diagnostic. Root cause data.R:1067
(post-subset count) vs data.R:1165 (full length + subset), with model.R:962-964 taking the
`n == full` branch first when full == length(index).

### (d) Multinomial twin-create - LIVE, but the SURFACE claim is REFUTED

Live at HEAD: bart.R:1790 + 1802 (ordinal), same pair at :2056 (nbinom), `adoptPointer` both. The
memo's "SURFACE: draw-moving at a fixed seed for two shipped families" is false. `createChainRngs`
(R_interface_bartcore.cpp:1864-1891): with a seed the chain seeds come from a dedicated MT; without
...
multinomial-mutation-arc.md:389-398 already says exactly this and already ruled: "MOVES DRAWS - but
...
6. group-by-exposure. Value clause SURVIVES: "a multi-site binary or count outcome cannot get a
   ...
   implies - `GroupedResponse` is a decorator applied unconditionally at chain.hpp:625-629, after
...
8. sparse-extensions halves. Refusals verified live (rbart.R:302-307, model.R:215-220 and :255-265,
   engine reason data.hpp:287-292). Removing a refusal is ADDITIVE-LATER by construction - the memo
...
9. rbart_vi logistic token. "Grouped binary silently gets probit" is QUALIFIED - p10.R:
   ...
   family accepting weights (spec.R:52-67), so grouped WEIGHTED binary is unreachable. Widened
...
## 3. Doors
- D1 per-forest row subsetting: MISSTATED. TODO:221-223 says the classes may want different SHAPES,
  not grids. The source fork is mask / ZERO WEIGHT / compaction; zero weight already SHIPPED
  (model-space-survey.md:307-312) and the per-forest MASK was permanently REFUSED as redundant with
  `setForestWeights` (latent-subset-mask.md:323-326, :488-492, arc COMPLETE). The memo drops the
  shipped arm and conflates mask with zero weight. Honest residue: PHYSICAL COMPACTION only, already
  deferred by VD with a named trigger (TODO:223-224). As posed it reopens two settled arms.
- D2 approximate-Polya-Gamma: correct and genuinely open (negative-binomial.md:105-108 resolved fork
  A only; :646-654 holds fork B). Two fixes: "explicit opt-in" is the TODO's gloss, never
  adjudicated - the record says a "project-level identity decision ... its own arc" (:309-313,
  :648-651); and TODO:498's FIRST half (integer-weight probit by replicated latents) is exact and
  cost-gated, not part of this door (weighted-binary.md:18-32).
- D3 hurdle samplerOnly: correct and open (multinomial-mutation-arc.md:956-961 recommends defer;
  hurdle.md carries no ruling). Live-code fix: THREE families opt back in via `allow.samplerOnly` -
  multinomial (bart.R:919), ordinal (:1072), nbinom (:1105); the comment at bart.R:603 saying
...
- BCF setData smuggling: NO. D1 is fixed-n membership, Door 1 is n-free joint x/y/z, and the survey
  ...
  C_interface.cpp:665-673 and :694-698 are the setPredictor/updatePredictor guard-retirement
  comments and `grep -c setData src/C_interface.cpp` = 0 - there is no setData in the flat C API at
  all. Re-anchor to R_interface_bartcore.cpp:4631.

## 4. Stale-premise findings
- dimnames (TODO:265-267): substance CONFIRMED (e8fcf8be; 211 commits before HEAD;
  bartcore.R:293-307, dbartsSampler-class.Rd:373, test-sampler-predictors.R:75-105). CHRONOLOGY
  ...
  ticket and not the fix. Sparse/container residual CONFIRMED (bartcore.R:331 on; mixedMatrix.R
  never reads the argument's names).
- feature-matrix [f19] / :1039: CONFIRMED (4975c20b, c3af16a1 real and as described;
  generics.R:66-93 guards and scores the t marginal). Nit: [f19]'s "per-row lambda_i is not stored"
  clause is still true, so it is stale in its verdict only.
- INDEX.md nameable-calibration row: CONFIRMED (INDEX.md:83 vs feature-matrix.md:1109-1113; ab3aa2fa
  real). MISSED: INDEX.md:74 still calls it one of "the two designed-but-unbuilt arcs".
- bcf-bartcause-relocation anchors: MIXED. Path is docs/PLANS/, not docs/design/. Stale TODO:80-143
  and :497-500 cites CONFIRMED; vanished bart2-argument-consolidation entry CONFIRMED (a dangling
  ordering reference survives at TODO:405); S13 172523e6 door stale CONFIRMED. "Two of its doors are
  answered too" REFUTED - the per-forest replay door at :1271-1274 ALREADY records "LANDED
  2026-08-20 at dbarts 63df524e", landing note at :1613. One stale door, not two. generics.R:505 is
  the comment above `predictForest` (:516).
...
- MINOR: the memo's header says "1 PROCESS" while sec 1 marks two (:372 and the release block
  :510). TODO holds 41 top-level entries, all covered, so nothing is missed - only the count line.
...
4. ordinal-scan-missing-rows (TODO:305). Independently CONFIRMED: scan.hpp:100 skips `naCode` and
   totals surviving bins (:105-108) while grow.hpp:181-186 builds the no-split term from the FULL
   node stats. Sole caller of `scanOrdinalCuts` is grow.hpp:268, so reach is `n.grow.sweeps > 0`
   (default 0L, bart.R:677) - opt-in, but it hits EVERY family, this being an ordered COLUMN scan,
...
6. survival `entry=` (TODO:427). Additive-later, face only. ~150 R + Rd + tests, Sonnet.
7. group-by-exposure (TODO:137). Additive-later, largest missing face piece, engine decorator
   already family-agnostic. COST ~500 R + Rd + tests plus a tau `rel.scale` decision. TIER Sonnet +
   Opus design read.
8. sparse-extensions, rbart_vi half (TODO:419). Additive-later widening. TIER Sonnet.
...
at `seed = NA`, the win is 3ms, and multinomial-mutation-arc.md:389-398 already ruled "Defer".
