...
`[[R/bart.R:811@3080a9c5]]`, `[[R/bart.R:2849@3080a9c5]]`, `[[R/rbart.R:100@3080a9c5]]` each do
...
the bridge refuses 0 at `[[src/R_interface_bartcore.cpp:432@3080a9c5]]`
...
Same knot, second contradiction: `[[man/dbartsControl.Rd:56@3080a9c5]]` says printEvery
"Must be a positive integer"; `[[R/A_class.R:356-357@3080a9c5]]` accepts 0 ("must be a
...
`[[R/generics.R:2864-2872@3080a9c5]]`:
...
kind is refused") and `[[inst/NEWS.Rd:139@3080a9c5]]` ("gain their own catch-all refusals").
Cure: `if (length(dots) > 0L)`, and pick the first *named* element for the
quoted name.

Same root, second instance, on the shipped fit surface:
`refuseUnusedGenericArgs` (`[[R/generics.R:2028-2043@3080a9c5]]`) matches on
...
migration a user makes. This is the residue at `[[docs/plans/surface-refusals.md:768-770@3080a9c5]]`
...
`[[inst/NEWS.Rd:139-143@3080a9c5]]` - "`extract` on a `dbartsSampler` and `plotTree` on a
`dbartsSampler` gain their own catch-all refusals". `refusePlotTreeArgs`
(`[[R/generics.R:2986-2999@3080a9c5]]`) refuses exactly two names, `sample` and `chain`.
...
note (`[[docs/plans/surface-refusals.md:816-818@3080a9c5]]`) already narrowed this claim once
...
`[[src/C_interface.cpp:969-972@3080a9c5]]` forwards `printEvery` unvalidated;
`[[src/bartcore/chain.hpp:1393@3080a9c5]]` evaluates `(iteration / numThin + 1) % options_.printEvery == 0`
...
print" is the obvious consumer reading, and `[[dbarts.h:1082-1083@3080a9c5]]` is the one
entry on that page carrying **no** doc comment at all while its neighbours carry
paragraphs. Recorded at `[[docs/plans/capi-shape.md:613@3080a9c5]]` as "still does not guard
...
(`main:[[R/generics.R:117-120@3080a9c5]]`), so `fitted(fit, "ev", "test")` is live 0.9-x code
...
is the flagship UPGRADING entry (`[[inst/NEWS.Rd:152-158@3080a9c5]]`). Cure: in the ci.level
...
Recorded as residue at `[[docs/plans/predict-surface.md:186@3080a9c5]]`. What makes it a
pre-review item is `[[inst/NEWS.Rd:76-79@3080a9c5]]`, which opens "Every `predict` method's
...
`[[docs/design/multinomial-mutation-arc.md:754-755@3080a9c5]]` - "`[[R/generics.R:1688@3080a9c5]]` and
`[[R/generics.R:1869@3080a9c5]]` now say so directly" (about `$bc`'s removal). `[[R/generics.R:1686-1690@3080a9c5]]` is
`print.bartOrdinal`'s `cat("n.trees: ", ...)` block. The real comment is
`[[R/generics.R:1590-1592@3080a9c5]]`. `bfc4571d` re-targeted this today, 1501 -> 1688, and
missed; the paired `[[R/generics.R:1869@bfc4571d]]` cite *is* correct, which is what hides it. This is
...
`[[feature-matrix.md:358@bfc4571d]]` ([[RIB:2791@bfc4571d]], [[RIB:2928@bfc4571d]], still landing on the right
predicates) and `[[multinomial-mutation-arc.md:654@bfc4571d]]`
(`[[multinomial-equivalence.R:189-196@bfc4571d]]`, real anchor now ~472-475, also today's).
...
(and `[[multinomial-equivalence.R:603-612@de67cf07]]`)" now lands in scenario bodies - the z
is at `[[multinomial-equivalence.R:826-831@de67cf07]]` / `[[multinomial-equivalence.R:911-916@de67cf07]]`. Below the freshness checker's radar (it walks
`docs/design`, and it exits 0 on WARN regardless: `[[.github/workflows/lint.yaml:89@de67cf07]]`).

### 9. Today's new per-push gate can pass while comparing nothing. ~4 lines.

`[[benchmarks/R/bcf-equivalence.R:764-767@de67cf07]]` (and the same shape in
...
`[[inst/tinytest/test-plot-generics.R:372@de67cf07]]`:
...
- `[[R/generics.R:2883@de67cf07]]` `stop("type must be 'predictors'")` sits two lines below the
...
- `[[R/utility.R:179-180@de67cf07]]` (new today) guards `mc[[2L]]` with `is.symbol()`, while
  `[[R/utility.R:164@de67cf07]]` and `[[R/utility.R:184@de67cf07]]` in the *same 20-line function* interpolate it unguarded. The
  asymmetry is visible in one screen. (`[[docs/plans/surface-refusals.md:771-774@de67cf07]]`
  defers these to error-style slice L - defensible for the corpus, not for two
  lines bracketing today's insertion.)
- `[[R/utility.R:180@de67cf07]]` `" must be a whole number, not ", deparse(x)[1L]` uses R10's
  `, not <type>` shape to carry a *value*; R12's shape is `; got '<value>'`.
- `[[R/generics.R:2092-2095@de67cf07]]` `bartUnusedArgs$contribution` is a verbatim copy of the
  `forest` reason and never mentions contribution; the same name now carries three
  different explanations (`[[R/generics.R:518@de67cf07]]`, `[[R/generics.R:2092@de67cf07]]`, `[[R/generics.R:2114@de67cf07]]`).
- `[[R/generics.R:2112@de67cf07]]` leaves `bases` unquoted while its list-mates quote
  `'sample'`, `'group.by'`.
- `[[R/generics.R:952@de67cf07]]` uses `;` for an explanation where the new siblings at
  `[[R/generics.R:1234@de67cf07]]/[[R/generics.R:1562@de67cf07]]/[[R/generics.R:1843@de67cf07]]` use `:`, so one method now emits both separators for
  adjacent argument names.

### 12. `summary`'s empty-table message names the wrong parameter set. ~3 lines.

`[[R/diagnostics.R:415@de67cf07]]`: `"No scalar parameters (sigma, k, tau) to summarize."` -
...
- `setResponse`/`setOffset`/`setWeights`' flat-unreachable 0 arms
  (`[[capi-shape.md:587-591@de67cf07]]`): stated contract, pinned by the BCF legs. Correct.
- `setWeights` finiteness at the flat entrance (`[[capi-shape.md:609-610@de67cf07]]`): C API is
  fast-over-safe by house rule and R validates. Correct.
- `setActiveRows` scanning its mask twice (`[[capi-shape.md:614@de67cf07]]`): perf only.
- The R5 twins of `getTrees`/`printTrees` carrying no forest selector (`[[capi-shape.md:611-612@de67cf07]]`):
  real surface gap, but well over 30 lines and additive. Correct to defer.
- `extract(type = "trees")` raising R's own "unused argument"
  (`[[surface-refusals.md:775-778@de67cf07]]`): disagrees in voice, not outcome.
- `fitted(type = "ppd")` consuming RNG (`[[surface-refusals.md:779-781@de67cf07]]`): documented, not a defect.
- `as_draws_*.bartMultinomial`'s inert `vars` (`[[surface-refusals.md:782-784@de67cf07]]`): VD's call, and it *is*
  documented - `man/summary.bart.Rd:97-105` says its `vars` "only ever means
  `meanProb`". Verified inert live. Correct.
- `plot.pdbart`'s fourth shape (`[[surface-refusals.md:791-793@de67cf07]]`), `makeind(all=)`, `run(n.threads)`:
  correct.
- `plot`'s `cols` default splitting `NULL` (multinomial/ordinal) from
  `c("blue","black")` (the other four): **documented** at `[[man/bart2.Rd:353-355@de67cf07]]`.
  Not a finding.
- `plotTree.bart`'s `chainNum` having no default while `plotTree.rbart` defaults to
  `1L`: **documented** at `[[man/plotTree.Rd:53-57@de67cf07]]`. Not a finding.
...
- D8 residues (`[[composition-refusals.md:424-435@de67cf07]]`), xbart fold-view scope: the
...
- `benchmarks/R/composition-matrix.R` runs in no workflow (zero hits in
  `.github/`), while `[[docs/plans/INDEX.md:217@23b9cde7]]` records a 0-disagreement state CI
...
- `[[inst/tinytest/test-capi.R:73-89@23b9cde7]]`'s six retired-token `expect_false`s are
  dominated by the live-token `expect_identical` at `[[inst/tinytest/test-capi.R:92@23b9cde7]]` - diagnostic naming, not
  coverage. Not stale (all six literals were genuinely live), just not load-bearing.
- The R5 `getTrees` range messages (`[[R/dbarts.R:2059@23b9cde7]]/[[R/dbarts.R:2065@23b9cde7]]/[[R/dbarts.R:2068@23b9cde7]]`) name
...
in `[[R/hooks.R:18-24@23b9cde7]]` because posterior is Suggests-only. The NEWS 1.0-0 section
...
near-miss for the record: `[[inst/NEWS.Rd:2067-2070@23b9cde7]]` ("a `keepSampler` fit ... now
