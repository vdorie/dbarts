...
- `R/bart.R:2421-2424` (training branch) and `:2432-2435` (matrix-`newdata` branch)
  ...
  `x1,x2,period` and refuses. The data-frame branch at `:2427-2428` is correct
...
- Repro (`generics/probe15.R`):
  ...
  also works.
- Contradicts survivalProbabilities.Rd:11-13, :45-47, :87-96 and bart.Rd:328. Hazard
  fits are matrix-interface only (the formula route is refused by name) and bart.Rd:75
  tells users column names are honoured, so the named matrix is the normal spelling.
- Why no test caught it: `inst/tinytest/test-hazard.R:12` builds
...
- `R/generics.R:375-380` (`extract.bart`) rewrites `match.call()` onto
  ...
  `getTrees(treeNums, chainNums, sampleNums, current, newdata)` (R/dbarts.R:1917).
  `extract.rbart` (:1857-1866) repeats it for `sample` and `combineChains`.
...
- Contradicts bart.Rd:203-205 (`sample` documented with no type qualification) and
  :219 (only `\dots` is documented as reaching `getTrees`). Correction to
...
- `extract.bartMultinomial` (R/generics.R:862), `.bartOrdinal` (:1071) and
  `.bartNegbin` (:1237) have formals `(object, type, sample, ...)`, so it lands in
...
- `R/bart.R:390-394`: `if (keepSampler) result$fit <- fit else result$n.chains <- n.chains`.
  `keepTrees/keepSampler` in {TT, TF, FT} all give `is.null(fit$n.chains)`; only FF
  sets it. bart.Rd:315-317 documents it unconditionally, and its stated purpose
...
refusing.**
- The guard at R/generics.R:363-373 tests `is.null(object$fit)`, i.e. the SAMPLER, not
  ...
  `sample` column, no warning - where bart.Rd:257 scopes the feature to
  `keepTrees = TRUE` and :260-266 documents `chain, sample, tree, n, var, value`.
...
the channel loo/WAIC consume, and bart.Rd:201 documents `"loglik"` on the `extract`
...
- The hurdle fit has no `$fit` of its own (`names()` = call, family, y, occupancy,
  positive), so bart.Rd:248's `bartFit$fit$storeState()` does not apply, and :248
...
- Fix class: agent-fix (extend bart.Rd:248).

**G10. dbartsSampler-class.Rd:328 "Reading it forces the sampler's *current* state" is
...
- The behavior follows the documented opt-in convention at :116; the sentence at :328
  is the wrong one. Fix class: agent-fix (Rd).

### MINOR

- M1. `sample` validation inconsistent: bart/rbart raise `sample must be in 'train',
  'test'` (R/generics.R:385-391); all four own-class `extract` methods use bare
  `match.arg` -> `'arg' should be one of "train", "test"`.
- M2. `residuals(fit, sample = "train")` -> raw `formal argument "sample" matched by
  multiple actual arguments` (R/generics.R:814, :2078, :1564 all pin it).
- M3. `plotTree(fit, treeNum = 1L, sample = 2L)` silently partial-matches `sampleNum`
  via `do.call` (R/generics.R:2096-2103). Benign today; same mechanism as G2.
- M4. `setForestBasis` on an amplitude-free sampler says `forest index out of range`
  (R/dbarts.R:1456-1458) where `setForestWeights` says `requires a sampler that
  carries forest amplitudes`; dbartsSampler-class.Rd:189 promises the latter.
- M5. `extract(type = "trees")` omits the `chain` column on a single-chain fit;
  bart.Rd:262 lists `chain, sample, tree` unconditionally.
- M6. bart.Rd:248 says only `predict` stops on a fit saved without `storeState()`;
  `extract(type = "trees")` and `plotTree` stop identically.
- M7. dbartsSampler-class.Rd:308 offers "the return of `storeState`" to `setState`,
  but it returns NULL invisibly (:423), so `s$setState(s$storeState())` refuses.
- M8. `setForestBasis(k, ~var)` evaluates the formula in `environment(basis)`, never
  against the sampler's data (R/model.R:810-821), so `~x1` naming a predictor column
...
- `plotTree` and `survivalProbabilities` on all four own-class fits -> `no applicable
  ...
  something to draw. plotTree.Rd:26-29 names only bart/rbart/dbartsSampler. SILENT.
...
- `dbartsSampler`: `predict`/`survivalProbabilities` -> `no applicable method`;
  ...
  `print` -> `$show()`, correct. dbartsSampler-class.Rd:355 states the division, so
...
- `predict.bart`'s `\dots` is documented "Not used" (bart.Rd:219) and indeed ignores
  `sample=`: AGREES, but a live trap because extract/fitted take `sample`.
  `predict.rbart` reads `dotsList[["value"]]` by exact name only (:1619-1624), so a
...
- bart.Rd:201 type vocabulary, `"response"`/`"link"` synonyms on all four generics,
...
- bart.Rd:203-205 `sample` - CONTRADICTS under `type = "trees"` (G2).
- bart.Rd:215-216 `ci.level` -> `est, ci.lower, ci.upper` - AGREES on
  bart/rbart/bartHurdle; ignored on the other three (G4).
- bart.Rd:219 `\dots` reaches `getTrees` (`chainNums`, `sampleNums`, `treeNums`,
  `newdata`, all four verified); "Not used in predict" - AGREE.
- bart.Rd:247-248 Saving - AGREES for bart/rbart/multinomial/ordinal/nbinom and the
  amplitude fit; SILENT on hurdle (G9), understates the affected generics (M6).
- bart.Rd:257-258 - CONTRADICTS (G6). :260-266 columns - CONTRADICTS on `chain` (M5)
  and on the keepSampler-only shape (G6). :263 `newdata` changes the `n` column -
  AGREES (960 train vs 192 for 8 test rows). :315-317 `n.chains` - CONTRADICTS (G5).
- bart.Rd:277 chain-major collapse with all collapsed fields sharing the row order -
  AGREES (combined `sigma` == the two chains concatenated, chain 1 first).
  :288-289, :294-295 (varcount `12x3`, variable-named columns), :300-301
  (`forestFits` `6x40x2` named `forest1,forest2`, `glue`, `bases`, `n.forests`),
  :303, :306-307, :312-313, :318-319, :324-326, :327-328, :331, :335 - AGREE.
- summary.bart.Rd:11-17 (mean.s on a heteroscedastic fit), :19-25, :42-51
  (`field[column]` naming), :64-77, :79-82; plotTree.Rd:18-22, :26-29, :39-42, :51
  (including the current-trees fallback); extract.dbartsSampler.Rd:8, :15, :27
  (`type must be 'predictors'`; `40x3` with the predictor column names) - all AGREE.
  The first two are SILENT on the own-class fits.
- survivalProbabilities.Rd:40-41, :49-52, :98-105 (`12x2x60` / `2x6x2x60`) and the
  rbart branch - AGREE. :45-47 and :87-96 - CONTRADICTS on a named-predictor hazard
  fit (G1).
- dbartsSampler-class.Rd:351 - AGREES on all ten documented multinomial refusals
  ...
  is now tested and clean. :189 - CONTRADICTS for `setForestBasis` (M4).
  :308 - CONTRADICTS (M7). :328 - CONTRADICTS (G10). :358-359 - AGREES.
- bart2.Rd:310, :312, :314 (`cutpoint[1]`, `cutpoint[2]`), :318 (`dispersion`), :322
  (hurdle `prob`/`link`/`log`/`ppd`, `ci.level`, two-component summary) - AGREE. :38-49
...
- E. "Does `dbartsSampler` ever grow the bart-family S3 surface?" - dbartsSampler-
  class.Rd:355 already answers no; the open part is only whether the six defaults
...
- F. "Should `extract(type = 'trees')` read the CURRENT trees on a keepSampler-only
  fit, or refuse?" - plotTree.Rd:39-42 already documents that fallback for plotTree,
...
- Numeric correctness: cells record class, dims, dimnames and attributes only, so the
  ...
  `contribution = TRUE` arithmetic and the `bases =` identity at bart.Rd:301.
- `plot` CONTENT: an accepted `plot` cell means only that it drew without error.
- `pdbart`'s own argument surface, and `run()`'s shapes at dbartsSampler-class.Rd:387.
