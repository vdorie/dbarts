...
sites warned (-Wswitch): chain.hpp:582 (single-forest response factory),
R_interface:2268 (refusedAmplitudeFamilyReason), :6181 (drawAugmentationLaws).
...
By call graph, :756 and :5026 take spec.family from the K-forest route, which
R_interface:2268 refuses for aft/ordinal/nbinom BEFORE either runs, so those
arms are gaussian-only today.  :2812 is the worst door - its own comment
...
family already exists: chain.hpp:862 sets `family_ = logistic` for multinomial
commenting "family() is not read on this path", yet SamplerShape::family
(facade.hpp:432) reports logistic and the bridge branches on it at :2729,
:4611, :4662, :4702, :4877, :4949 - each unreachable only because a SEPARATE
...
data 2, tree 2, facade 1) are compile-time - no gap.  The six runtime asserts:
  chain.hpp:4425 recoveredFactors || bottomNodesAreOccupied - counterpart YES,
    recoverVarianceLeafValuesBelow's numObservations() > 0 test returns 1.0.
  chain.hpp:4494/:4522 leaf[j] < muByTree[t].size() - NO.  Its own comment
  ...
  chain.hpp:4628 leaf[j] < numNodes && isBottom - PARTIAL: the fused pass
    declines a map FLAGGED stale (:4617), not one that is fresh and WRONG.
  grow.hpp:249, model.hpp:2224 numPresent in [2, numReachable] - NO, but the
...
(test_moves.cpp:1456).  M10, the only other SIGABRT, also reddens under -DNDEBUG
...
1 BLOCKER chain.hpp:1518 (also :1578,:1616,:1665,:1678) [C1].  Deleting the
  ...
  facade.hpp:816 applies no family gate, R/spec.R refuses a linear/GP node
  prior only on the multinomial (:445) and K-forest (:643) routes, and a
...
2 BLOCKER facade.hpp forwarding [F1,F3,F4,F6,F7].  5 of 7 facade mutations
  survive: numSavedDraws reporting capacity (:429), setResponse forwarding
  !updateScale (:454), savedSlotForDraw returning the identity (:514),
  setForestWeights installing on forest 0 regardless (:619), savedTree reading
  forest 0 regardless (:519).  Only the two shape() fields were caught, by
...
3 MAJOR chain.hpp:4207 [C3].  Binding y + meanFits for y - meanFits - the
  ...
  because tinytest backstops it (test-heteroscedastic.R:11-42: fTrue = 2x, s(x)
  bounded in (0.15,0.6) and (0.9,2.2)).  heteroscedastic.md:465-468 states gate
...
4 MAJOR sampler.hpp:575 (identically :605, :679) [A8].  Giving predict's
...
5 MAJOR moves.hpp:626-706, the move-validity predicates [M8, M9].
  ordinalRuleIsValid's descendant interval off by one on both sides (:635) and
  categoricalSubtreeIsValid's gauge dropping `directions == reachable` (:659)
...
6 MAJOR grow.hpp:222 [G5].  -log(numPredictors) for -log(numAvailable) -
...
7 MAJOR model.hpp:3314 [D8].  Dropping the lower tail from
...
8 MAJOR model.hpp:2185 [D4].  Dropping the `+ log 2` that
  ...
  grow.hpp:292) is caught and this is not: nothing measures an MH ratio moving
...
9 MAJOR model.hpp:3603 [D7].  Deleting LogisticResponse::setWeights' cold start
...
10 MINOR combiner.hpp:911 [B5].  formForestVetoWeights losing the near-zero
  multiplier snap survives, though testBCFZeroMultiplierSnap exists: it covers
  the response half (:894-897, all three caught), not the veto half - the one
...
R1 moves.hpp:280-281 - the birth-rejection restore of the parent's
  sumWeights/sumWeightedResponse is DEAD.  Tree::birth (tree.hpp:953) writes
...
R2 sampler.hpp:765 - `bool allValid = columnMaskOk;` is redundant:
  Chain::stateIsValid runs the same scratch build and the same
  columnMaskSubtreeIsValid per forest (chain.hpp:3229) and per variance tree
  (:3318), and *columnMaskRefused reads columnMaskOk directly, so `true` is
  behaviourally equivalent (A6, missed).  chain.hpp:3390 already calls
