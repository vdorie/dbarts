...
- **B1. `extract(fit, type = "trees", ...)` corrupts its own call.** Asking for the trees of one posterior
  ...
  R's raw `unused argument`. EVIDENCE `[[R/generics.R:363-382@b102e17c]]` rewrites `match.call()` onto
  `object$fit$getTrees` and strips only `object` ([[R/generics.R:379@b102e17c]]) and `type` ([[R/generics.R:380@b102e17c]]), so `sample` partial-matches
  `getTrees`'s `sampleNums` (`[[R/dbarts.R:1917@b102e17c]]`); `extract.rbart` ([[R/dbarts.R:1851-1866@b102e17c]]) repeats it. REPRO (3 trees, 5
  ...
  `[[man/bart.Rd:203-205@b102e17c]]` and `[[man/bart.Rd:219@b102e17c]]`. LEGS F3, E1, G2 (33 grid cells + 102 on the combineChains axis).
...
- **B2. `survivalProbabilities()` is dead on every hazard fit whose training predictors carry column
  names.** Hazard fits are matrix-interface only and `[[man/bart.Rd:75@b102e17c]]` says column names are honoured, so the
  named design is the normal spelling - and the broken one. EVIDENCE `[[R/bart.R:2421-2424@b102e17c]]` (training) and
  `[[R/bart.R:2432-2435@b102e17c]]` (matrix `newdata`) build `cbind(<named covariates>, rep(seq_len(K), each=n))`, leaving the
  ...
  `[[survivalProbabilities.Rd:45-47@b102e17c]],[[survivalProbabilities.Rd:87-96@b102e17c]]`, `[[bart.Rd:328@b102e17c]]`. WHY NO TEST: `[[inst/tinytest/test-hazard.R:12@b102e17c]]`
...
- **B3-B7. Five reach gaps.** Each was re-planted by this pass, alone, in a staged copy; R-side mutations
  ...
  - B3 `[[R/dbarts.R:1025@b102e17c]]` `$growFromRoot(n.sweeps)` can discard `n.sweeps` [mutation-A 1].
  - B4 `[[combiner.hpp:906-913@b102e17c]]` the BCF near-zero multiplier snap can go [mutation-A 2, and mutation-B B5, the
    same site seen from the C++ side]. B5 `[[combiner.hpp:1818-1825@b102e17c]]` the multinomial veto's active-row mask
  ...
  - B6 `[[chain.hpp:1518@b102e17c]]` the latent refresh's vector-leaf cache invalidation can go - R GREEN and C++
    SURVIVED (planted twice); every Linear/GP-leaf fixture in tests/cpp is gaussian so the branch is never
    entered, while `[[facade.hpp:816@b102e17c]]` has no family gate and no tinytest pairs a vector leaf with a latent
    family [mutation-B 1].
  - B7 `facade.hpp` 5 of 7 forwarding defects survive (`[[facade.hpp:429@b102e17c]]` `filledSavedDraws` -> `savedTreeCapacity`;
    `[[facade.hpp:455@b102e17c]]` `setResponse(y,!updateScale)`; `[[facade.hpp:515@b102e17c]]` `savedSlotForDraw` -> identity; `[[facade.hpp:621@b102e17c]]` `setForestWeights`
    on forest 0 regardless; `[[facade.hpp:520@b102e17c]]` `savedTree` on forest 0 regardless). Every other test drives `Sampler<L>`
...
- **M1. `dbartsData()` mis-reports EVERY multi-column response; four surfaces inherit it.**
  `[[R/data.R:1179-1181@b102e17c]],[[R/data.R:1238-1241@b102e17c]]` compare `NROW(formula)` against `NROW(codeResponse(data)$y)` and
  ...
  lines.
- **M2. `bart()`'s by-name family redirect covers 4 of 10 tokens; `[[man/bart.Rd:174@b102e17c]]` asserts all ten and
  ...
  (`[[R/bart.R:2587-2592@b102e17c]]`) is exactly those four and is checked against the RAW token before `bart2`'s alias
  fold, so `"hurdle.lognormal"` redirects and its documented alias `"twopart"` (`[[dbarts.Rd:111@b102e17c]]`,
  `[[bart2.Rd:237@b102e17c]]`) does not. LEGS F9, E2, E3, reading-R 3.3. CONFIRMED. AGENT-FIX for the `twopart` half (1
...
- **M3. `dbartsSpec()` and `dbarts()` resolve `family="multinomial"` differently for the same input; two
  ...
  `[[dbartsSpec.Rd:40@b102e17c]]` ("a family can never resolve two ways") is falsified; `[[dbartsSpec.Rd:48@b102e17c]]` calls `"multinomial"`
  unavailable as describing "more than one sampler" - false, and the token IS in its own vocabulary
  (verified: auto, gaussian, probit, logistic, aft, multinomial, ordinal, nbinom). `[[dbartsSpec.Rd:25@b102e17c]]` is right. LEGS F1,
  E7. CONFIRMED. AGENT-FIX (Rd).
- **M4. `[[man/bart.Rd:153@b102e17c]]` states the `combinechains=TRUE` shape backwards.** REPRO (2 chains x 6 draws x 60
  obs): TRUE -> `yhat.train` 12x60 (collapsed matrix), FALSE -> 2x6x60; identical on `bart()`. Its own
  `\value` at `[[man/bart.Rd:277@b102e17c]]` and `[[bart2.Rd:164@b102e17c]]` are correct. LEG E5. CONFIRMED. AGENT-FIX (Rd).
- **M5. `[[man/bart.Rd:165@b102e17c]]` transposes the `proposalprobs` defaults** ("birth_death, change, and swap ...
  ...
  `[[bart2.Rd:200@b102e17c]]` writes correctly. LEG E6. CONFIRMED. AGENT-FIX (Rd).
- **M6. `$n.chains` is absent from every fit that keeps a sampler, which `[[bart.Rd:315-317@b102e17c]]` and
  `[[bart2.Rd:164@b102e17c]]` promise unconditionally.** `[[R/bart.R:390-394@b102e17c]]`: `if (keepSampler) result$fit <- fit else
...
- **M7. A saved `bartHurdle` fit cannot `predict` after reload and no Rd names the recipe.** REPRO:
  `names(fH)` is `call, family, y, occupancy, positive` - no `$fit` - so `[[bart.Rd:248@b102e17c]]`'s
  `bartFit$fit$storeState()` does not apply, and `[[bart.Rd:248@b102e17c]]` names only multinomial/ordinal/nbinom. After
  ...
  replay. LEG G9. CONFIRMED. AGENT-FIX (extend `[[bart.Rd:248@b102e17c]]`).
- **M8. `[[dbartsSampler-class.Rd:328@b102e17c]]` "Reading it forces the sampler's *current* state" is false, and the
  ...
  sampler`; with `storeState()` first it is ACCEPTED. The behavior follows the documented opt-in at `[[dbartsSampler-class.Rd:116@b102e17c]]`;
  `[[dbartsSampler-class.Rd:328@b102e17c]]` is the wrong sentence. LEG G10. CONFIRMED. AGENT-FIX (Rd).
- **M9. `dbartsDrawLatents()` refuses its own formal default when written out.** REPRO:
  ...
  -> `'sigma' applies only to family "aft" and "student", not "probit"`. `[[R/augmentation.R:65@b102e17c]]` declares
  `sigma = 1`, `[[R/augmentation.R:79-81@b102e17c]]` guards on `!missing(sigma)`. LEG E13. CONFIRMED. AGENT-FIX (default NULL, or guard
...
- **M11. Three engine divergences where one rule has two implementations that answer differently.** Verified
  by reading here, exactly as the engine leg states. `[[sampler.hpp:891@b102e17c]]` installs a LIVE-sourced donor's
  variance trees with NO positivity check where the slot-sourced arm at `[[sampler.hpp:919-923@b102e17c]]` applies one and says why,
  and `rebuildVarianceForest` (`[[chain.hpp:4356-4368@b102e17c]]`) then scatters a non-positive leaf into a divisor
  (reachable from a `.Call` with a hand-built state). `[[chain.hpp:2421@b102e17c]]` (`recoverTreeParameters`) and `[[chain.hpp:2453@b102e17c]]`
  (`applyNewData`) hard-code `forests_[0]` where siblings at `[[chain.hpp:2329@b102e17c]],[[chain.hpp:2401@b102e17c]],[[chain.hpp:2526@b102e17c]],[[chain.hpp:2540@b102e17c]]` loop `forests_`, so a
  whole-data replacement on a BCF/multinomial chain leaves forests 1..K-1 on the old grid - guarded two
  layers away by `refuseMultiForestMutation`, no engine assert. `[[combiner.hpp:986@b102e17c]]` routes `drawGlue` on
  `shippedShape()` (`[[combiner.hpp:1474@b102e17c]]`), which tests basis widths and canonicality but NOT the half-Cauchy flag; the
  shipped path refreshes only `prior[0].variance` (`[[combiner.hpp:1171@b102e17c]]`) where the general path loops every forest with
  `halfCauchyScale > 0` (`[[combiner.hpp:1213@b102e17c]]`), so two admissible specs get two different MODELS. LEGS reading-engine D2,
...
- **M12. Three shipped comments say a feature is "Not yet exposed through the R surface" when R reaches all
  three today.** VERIFIED: `[[chain.hpp:88@b102e17c]]` (monotone) vs `[[R/model.R:99@b102e17c]]`; `[[chain.hpp:141-142@b102e17c]]` (interaction
  constraints) vs `[[R/spec.R:111@b102e17c]]`; `[[chain.hpp:166@b102e17c]]` (variance forest) vs `[[R/spec.R:113@b102e17c]]`. LEG reading-engine C1
...
- **M13. Two CI gates are mis-wired so a green check means less than it looks, and five declared gates have
  ...
  stop at the merge to main; `[[rchk.yaml:57-59@b102e17c]]` fails only on a NON-EMPTY `grep -E '[PB]|[UP]' ... || true`,
  so an OOM-killed analyzer leaving no `.bcheck` is a GREEN job - the cure is already recorded at
  `[[docs/plans/release-candidate-review.md:1170@b102e17c]]`; `lint.yaml` and `pkgdown.yaml` declare a BARE
...
- A4 `[[model.hpp:3415@b102e17c]]` ordinal `updateCutpoints` freezes the last free cutpoint (at K=3, every free cutpoint)
  -> R GREEN, C++ KILLED (`ordinal gamma_2 posterior variance (actual 0, expected 0.1334)`).
  `[[test-ordinal.R:356@b102e17c]]`'s only gamma_2 assertion is a mean-within-0.35 band the frozen cold start sits
  inside. FIX `sd(cutpoints[,2]) > 0.02`.
- A5 `[[grow.hpp:186@b102e17c]]` `growth <= 0.0` -> `< 0.0` kills the depth/availability veto -> R GREEN, C++ KILLED (`a
  growth-vetoed node draws nothing`). `test-grow-from-root.R` is 11 of 18 smoke.
- A6-A9 four R-side refusals/constants with no behavioral cover, all R GREEN: `[[R/data.R:1495@b102e17c]]` precision
  threshold `1e-10` -> `1e-30` (both warn-cases have `diff(range(y))` EXACTLY 0, so the constant the file
  exists to defend is never exercised); `[[R/data.R:627@b102e17c]]` the `family="ordinal"` minimum-category refusal
  deleted; `[[R/xbart.R:608@b102e17c]]` the mcr loss ignoring case weights (the rmse arm IS covered); `[[R/rbart.R:270@b102e17c]]` the
  "survival (aft) fits do not support 'weights'" refusal deleted.
- A10-A12 three more, all R GREEN: `[[R/dbarts.R:1929@b102e17c]]` `getTrees(current=TRUE)` ignored, reading the saved
  store, though `test-empty-leaf-veto-weights.R`'s entire structural oracle is `getTrees(current=TRUE)`;
  `[[R/generics.R:2364@b102e17c]]` `print.bart` always printing family "gaussian", where `[[test-plot-generics.R:103-104@b102e17c]]`
  are `expect_true(is.character(capture.output(print(x))))`, which CANNOT FAIL; and (MINOR) `[[R/data.R:735@b102e17c]]`
  predict-path basis finiteness deleted.
- B3 `[[chain.hpp:4207@b102e17c]]` the variance forest binds `y + meanFits` for `y - meanFits` -> C++ SURVIVED (the only
  value-level variance assertion uses data with NO mean function). MAJOR not BLOCKER: tinytest backstops it.
- B4 `[[sampler.hpp:575@b102e17c]]` predict writes at the CAPACITY stride, reads at the draw count -> C++ SURVIVED;
...
- B5 `[[moves.hpp:635@b102e17c]]` and `[[moves.hpp:659@b102e17c]]`, the descendant-interval and categorical-gauge rule-validity predicates ->
  C++ SURVIVED (both); neither has a direct caller in tests/cpp.
- B6 `[[grow.hpp:222@b102e17c]]` `-log(numPredictors)` for `-log(numAvailable)` - identical at a root, wrong below it ->
  C++ SURVIVED; all three grow-from-root law tests measure ROOT rules only.
- B7 `[[model.hpp:3314@b102e17c]]` the ordinal log-likelihood loses its lower tail -> C++ SURVIVED. This is the EXPORTED
  per-observation loglik channel of every ordinal fit.
- B8 `[[model.hpp:2185@b102e17c]]` the `+ log 2` a missing-bearing ordinal column owes -> C++ SURVIVED; it cancels on a
  same-variable redraw, which is why the grow-side twin IS caught.
- B9 `[[model.hpp:3603@b102e17c]]` `LogisticResponse::setWeights`' cold start of INACTIVE rows deleted -> C++ SURVIVED -
...
- **VD-E. Which fit classes carry which generics, and what does an absent one do?** DECIDES `plot` on
  ...
  loo/WAIC channel `[[bart.Rd:201@b102e17c]]` documents unscoped) [G8]; `extract(type="trees")` on a keepSampler-only fit
  (verified: an 11x4 frame of the CURRENT trees, no `sample` column, no warning, where `[[bart.Rd:257@b102e17c]]` scopes
  the feature to keepTrees while `[[plotTree.Rd:39-42@b102e17c]]` documents exactly that fallback for plotTree) [G6]; and
...
- **VD-A. May every entry SPELL every family its siblings spell, and does a refusal echo the token TYPED or
  ...
  `bart(keepevery=-1)` refused as `n.thin`, an argument `bart()` lacks). `[[man/bart.Rd:174@b102e17c]]` asserts the
  redirects as done; `feature-matrix.md` [f1] says the opposite and the code follows the design doc.
  ALTERNATIVES (a) write the redirects on all four entries, ~40 R lines, and [[bart.Rd:174@b102e17c]] becomes true; (b)
  rewrite [[bart.Rd:174@b102e17c]] to match the design doc, 3 Rd lines, narrow vocabularies kept. RECOMMEND (a) for
...
- **VD-D. Do the own-class generics carry the bart-family argument vocabulary, or refuse it by name?**
  ...
  `[[bart2.Rd:38-49@b102e17c]]` is silent, so nothing constrains the call.
...
- **VD-H. Deletions and seams.** DECIDES `R/bartcore.R`'s low-level handle API (31 functions, leg-counted;
  ...
  file is a COMMENT at `[[R/model.R:434@b102e17c]]`, and `bartcorePredictPerForest` has none anywhere);
  `adoptPointer`/`reapplyForestWeights` (0 `man/` hits each); `dataSlotOrNULL` (`[[R/data.R:11-21@b102e17c]]`), whose own
  comment claims every internal read of `counts`/`offset.category`/`offset.category.test` goes through it -
  verified FALSE (5 bare `@counts` reads elsewhere in `R/`, a bare `@offset.category` at `[[R/generics.R:992@b102e17c]]`) -
  ...
  S1, per-observation virtual dispatch at `[[facade.hpp:694-703@b102e17c]]` against `[[core-generalization.md:69-76@b102e17c]]`'s
...
- **VD-G. Should the four C++ `default:` arms on a `ResponseFamily` switch go, so a 7th enumerator is a
  compile error rather than a silent gaussian?** DECIDES `[[chain.hpp:766@b102e17c]]` (K-forest ctor ->
  GaussianResponse), `[[chain.hpp:5033@b102e17c]]` (latentScaleAnchor -> scaledResponseSd),
  `[[R_interface_bartcore.cpp:2298@b102e17c]]` (defaultNodeScale -> 0.5), `[[R_interface_bartcore.cpp:2842@b102e17c]]` (validateResponseSupport -> NO
  validation), plus the open-coded chain at `[[R_interface_bartcore.cpp:6254@b102e17c]]` - all five verified present. The mutation leg's
  7th-enumerator probe found exactly three sites warn today; `[[R_interface_bartcore.cpp:2842@b102e17c]]`'s own comment enumerates the harm it
...
- **VD-F. Undocumented option vocabularies: keep-and-document, or delete?** DECIDES `monotone`'s `"inc"`,
  ...
  `[[R/model.R:559-562@b102e17c]]` enumerates only the documented set) [E12]; `makeind(x, all=)` (verified inert -
  `body(makeind)` is `ignored <- all; makeModelMatrixFromDataFrame(x, TRUE)` - but `[[man/makeind.Rd:26@b102e17c]]` DOES
...
- V `extract(type="trees")` omits the `chain` column on a single-chain fit where `[[bart.Rd:262@b102e17c]]` lists it
...
- V `setForestBasis` on an amplitude-free sampler says `forest index out of range` where
  `[[dbartsSampler-class.Rd:189@b102e17c]]` promises the `setForestWeights` wording. [M4]
- V `[[dbartsSampler-class.Rd:308@b102e17c]]` offers "the return of `storeState`" to `setState`, but it returns NULL
...
- V `names(fit)` lists NULL-valued `yhat.test`, `s.train`, `s.test`, so `nm %in% names(fit)` misleads. [M10]
  `[[bart.Rd:248@b102e17c]]` understates which generics stop on a stateless fit. [M6]
- V `defaultNodeScale` has no default arm, so `dbarts:::defaultNodeScale("hazard")` and `("student")` return
  ...
  [E19]
- V `resolveFamily` and `augmentationFamily` really do have disjoint vocabularies, but `[[R/augmentation.R:7@b102e17c]]`
...
- Reading-cost classes, all DEFER, fully enumerated in their leg files: reading-R's 14 duplication clusters;
  ...
  `[[R/diagnostics.R:4@b102e17c]]`'s non-existent `R/zzz.R`; and the support-lib residue D1, D7, U5, U6, U7 - only U7's
  doc line (`--with-xint-size`, at `[[kernel-vocabulary.md:26@b102e17c]]`, absent from `[[configure.ac:21@b102e17c]]`) is AGENT-FIX.
...
- S2 Rd- and comment-only: M3 (`[[dbartsSpec.Rd:40@b102e17c]],[[dbartsSpec.Rd:48@b102e17c]]`), M4 (`[[bart.Rd:153@b102e17c]]`), M5 (`[[bart.Rd:165@b102e17c]]`), M7
  (`[[bart.Rd:248@b102e17c]]`), M8 (`[[dbartsSampler-class.Rd:328@b102e17c]]`), minors M4-gen/M5-gen/M7-gen, U7's
  `[[kernel-vocabulary.md:26@b102e17c]]` line, and M12's three "Not yet exposed" comments. Sonnet. No draws.
...
- S6 GROWS by one line, M16: `test-sampler-state-emptyLeafVeto.R` either rescales its fixture until it
  separates or drops its regression claim and points at `[[tests/cpp/test_moves.cpp:1212-1218@b102e17c]]`, which this
...
- NOT RE-PLANTED: mutation-B's equivalent-mutant claim R1 (`[[moves.hpp:280@b102e17c]]`, the dead birth-rejection
...
- **B8. `fitted()` and `residuals()` on an `rbart` fit SEGFAULT the R session when the random-effect array
  has no dimnames.** `[[R/generics.R:2040-2046@b102e17c]]` (`fitted.rbart`, `type = "ev"`) reads `ranefNames <-
  ...
  array`). STRONGER THAN THE LEG REPORTED: leg D reached it only by mutating `[[R/bart.R:40@b102e17c]]` and called it
...
- **VD-I. `predict`'s `n.threads`: wire it, or drop the formal and say so on `bart.Rd` too?** DECIDES
  ...
  `n.threads` occurs exactly ONCE in the R5 `predict` method (`[[R/dbarts.R:1079@b102e17c]]`) - in its own formal list -
  and the body's only `.Call` is `.Call(C_dbarts_bartcore_predict, ptr, x.test, offset.test)` against
  `DEF_FUNC("dbarts_bartcore_predict", bartcore_predict, 3)` (`[[src/R_interface.cpp:224@b102e17c]]`). `predict.bart`
  coerces `as.integer(n.threads)[1L]` (`[[R/generics.R:294@b102e17c]]`) and hands it to that method, which discards it.
  ...
  `[[man/dbartsSampler-class.Rd:152-157@b102e17c]]` ALREADY discloses it - "Currently has no effect: `run` and `predict`
  both execute serially regardless of the value passed here ... reserved for a future per-call override."
  That Rd is honest and AGREES with the code. `man/bart.Rd` is the one that does not: `[[man/dbartsSampler-class.Rd:49@b102e17c]]` puts `n.threads`
  in `predict.bart`'s `\usage` while its only `\arguments` item, the shared `nthread, n.threads` at
  `[[man/dbartsSampler-class.Rd:149-151@b102e17c]]`, describes a live thread count ("Integer specifying how many threads to use") written for the
  ...
  `[[dbartsSampler-class.Rd:152-157@b102e17c]]`'s disclosure into `bart.Rd`'s item, and make the coercion refuse a
...
- **CONFIRMED whole-suite, MAJOR stands (the planted defect survives all 167 files).**
  - **M14. `plot.pdbart` can render only the FIRST requested predictor.** `[[R/plot.R:219@b102e17c]]`'s `for (i in xind)`
    narrowed to `xind[1L]` -> SURVIVED. `[[test-pdbart.R:88-90@b102e17c]]` states the claim in a comment ("the plot
  ...
  - **M15. The 3+-level factor refusal in the single-forest entries can be deleted.** `[[R/data.R:536-548@b102e17c]]` ->
    SURVIVED. `[[test-factor-response.R:85-103@b102e17c]]` is the file that exists to hold it - five `expect_error`s
    naming "multinomial" - and every one still passes, because the SECOND `stop()` at `[[R/data.R:549@b102e17c]]` (the
  ...
  - **M16. `test-sampler-state-emptyLeafVeto.R` is not the regression its header says it is.** Line 10 reads
    "Regression for the -1e7 -> -HUGE_VAL fix."; reverting `[[moves.hpp:120-123@b102e17c]]`'s `-HUGE_VAL` to the pre-fix
    ...
    `[[tests/cpp/test_moves.cpp:1212-1218@b102e17c]]` - and this pass proved it by planting the same mutation there:
    ...
    four assertions (`[[tests/cpp/test_moves.cpp:43@b102e17c]]`, `[[tests/cpp/test_moves.cpp:48@b102e17c]]`, `[[tests/cpp/test_moves.cpp:52@b102e17c]]`, `[[tests/cpp/test_moves.cpp:56@b102e17c]]`) separate, or drop the claim and point at tests/cpp.
  - **M17. `samplePriorPredictive`'s sigma prior draw can be replaced by the point estimate.**
    `[[R/dbarts.R:836@b102e17c]]`, `sqrt(df * sigest^2 * rawScale / rchisq(n, df))` -> `rep_len(sigest, n)` -> SURVIVED.
    `[[test-prior-predictive.R:104-111@b102e17c]]`'s (d) block asserts only `var(ppd.pinned) >= var(pinned1)`, which
  ...
  - **M18. `test-sampler-setData.R`'s ONE assertion is self-cancelling.** Quantizing the replacement data
    onto a single cut per column (`[[R/bartcore.R:441@b102e17c]]`) -> SURVIVED: it compares `sd(rowMeans(samples1$train) -
...
- `[[R/plotTree.R:9-35@b102e17c]]` decodeCategoricalSplits' PADDING BRANCH IS DEAD CODE: skipping the decode and padding
  with "R" instead of "L" both leave the three categorical files green, including
  `[[test-data-categorical-declared.R:53@b102e17c]]`'s `nchar(directions) == 4L` on a factor whose 4th level is never
...
- `[[R/model.R:1554@b102e17c]]` `resolvePriorScale`'s `prior.sd * k` conversion is unreached: every sampler in
  `test-calibration-prior-draws.R` spells `scale =`, never `sd =`. AGENT-FIX. [C 9]
- `[[R/generics.R:125@b102e17c]]` pointwiseLogLikelihood's zero-weight NaN flag can report -Inf instead; the file's
...
- `[[R/bart.R:2709@b102e17c]]` the burn side of `n.burn %/% n.thin` is unpinned on both `bart()` and `bart2()`;
