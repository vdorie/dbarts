...
1. BLOCKER  [R/dbarts.R:1079](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/dbarts.R#L1079) and [R/generics.R:294](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/generics.R#L294)  `n.threads` is a SILENT NO-OP on predict.
   ...
   [src/R_interface.cpp:224](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/src/R_interface.cpp#L224)). predict.bart coerces `n.threads <- as.integer(n.threads)[1L]` and
   ...
   [inst/tinytest/test-generics-correctValues.R:16-31](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-generics-correctValues.R#L16-L31) with `n.threads = 2L` written in the call.
...
2. MAJOR  [R/plot.R:219](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/plot.R#L219)  plot.pdbart's `for (i in xind)` can be narrowed to `xind[1L]` -
   rendering only the first requested predictor - with all 51 files green (wide).
   [inst/tinytest/test-pdbart.R:88-91](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-pdbart.R#L88-L91) states the claim in a comment ("the plot method renders each requested
   ...
   SHOULD EXIST [inst/tinytest/test-pdbart.R:90](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-pdbart.R#L90): count the panels, e.g.
   ...
   agent-fix
3. MAJOR  [R/bart.R:1236-1237](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/bart.R#L1236-L1237)  `n.grow.sweeps`'s COUNT is unpinned: `sampler$growFromRoot(
   ...
   of the user's number. test-bart2-grow-from-root.R does gate the FEATURE ([R/bart.R:107](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/bart.R#L107)'s early-RMSE
   claim kills a mutation that skips growth entirely) but not the count.
   SHOULD EXIST test-bart2-grow-from-root.R after [R/bart.R:107](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/bart.R#L107): at one seed,
   `expect_false(isTRUE(all.equal(bart2(..., n.grow.sweeps = 1L)$yhat.train,
   bart2(..., n.grow.sweeps = 3L)$yhat.train)))`.  agent-fix
4. MAJOR  [R/data.R:536-548](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/data.R#L536-L548)  the 3+-level factor refusal in the single-forest entry points
   (`if (!splitMultinomialMessage) stop(caller, " does not fit a ", K, "-level ", ...)`) can
   be deleted with all 51 green (wide). [inst/tinytest/test-factor-response.R:85-103](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-factor-response.R#L85-L103) is the file that exists
...
5. MAJOR  [R/bartcore.R:407-414](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/bartcore.R#L407-L414)  `setResponse(updateScale = TRUE)`'s refusal under an
   amplitude-carrying or grouped sampler can be deleted with all 51 green (wide).
   [inst/tinytest/test-grouped-swap.R:64-74](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-grouped-swap.R#L64-L74) is written to hold exactly this ("updateScale = TRUE is refused
   ...
   SHOULD EXIST [inst/tinytest/test-grouped-swap.R:67](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-grouped-swap.R#L67): pin the R text that distinguishes the two,
...
6. MAJOR  test-generics-sequentialExecution.R (2 assertions, the leg's only GAP)  making
   ...
   green, while it IS caught elsewhere ([inst/tinytest/test-calibration-prior-draws.R:171-172](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-calibration-prior-draws.R#L171-L172)). The file's
   [inst/tinytest/test-calibration-prior-draws.R:42](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-calibration-prior-draws.R#L42) compares a bart2 fit against a hand-driven sampler loop, and BOTH sides call the
   mutated function, so the equality is common-mode blind to anything the two paths share.
   Its second assertion, [inst/tinytest/test-calibration-prior-draws.R:65](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-calibration-prior-draws.R#L65) `expect_inherits(sampler, "dbartsSampler")`, cannot fail short of
   an error - it is the "sequential runs don't overflow with fixed trees" claim, unpinned.
   SHOULD EXIST [inst/tinytest/test-generics-sequentialExecution.R:65](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-generics-sequentialExecution.R#L65): after the 6 sweeps past n.samples = 5,
   ...
   agent-fix
7. MAJOR  [R/data.R:769](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/data.R#L769)  `validateXYOffset` can return `offset = NULL` - silently discarding
   ...
   The same rewrite for [inst/tinytest/test-data-formula.R:14-55](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-data-formula.R#L14-L55).  agent-fix
8. MINOR  [R/plotTree.R:9-35](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/plotTree.R#L9-L35)  decodeCategoricalSplits' PADDING BRANCH IS DEAD CODE. Skipping
   ...
   [inst/tinytest/test-data-categorical-declared.R:53](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-data-categorical-declared.R#L53)'s `nchar(directions) == 4L` on a factor whose 4th
...
9. MINOR  [R/model.R:1554](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/model.R#L1554) (resolvePriorScale's `node.prior@prior.sd * node.hyperprior@k`)
   ...
   `node.prior = normal(k = fixedK, sd = priorSd)` measured against the same band as [R/model.R:57](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/model.R#L57).
   agent-fix
10. MINOR  [R/generics.R:125](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/generics.R#L125) (pointwiseLogLikelihood's zero-weight NaN flag) can report -Inf
    ...
    agent-fix
11. MINOR  [R/bart.R:2709](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/bart.R#L2709) (`control@n.burn <- control@n.burn %/% control@n.thin` on bart()'s
    path) can be deleted with test-control-valuesAreUsed.R green; its keepevery arm
    ([R/bart.R:40-55](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/R/bart.R#L40-L55)) pins only `nrow(yhat.train) == n.sims %/% keepevery`, the SAMPLE side. The burn
...
12. MINOR  Six reach gaps confined to this half - an out-of-file assertion is the sole
    ...
    test-data-formula.R and dies only at [inst/tinytest/test-bart-weights-parity.R:22](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-bart-weights-parity.R#L22) and
    [inst/tinytest/test-generics-posteriorPredictiveDistribution.R:90](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-generics-posteriorPredictiveDistribution.R#L90); mp3 (named split probabilities
    resolving to nothing) survives test-dart-mixed-columns.R and dies at
    [inst/tinytest/test-model-priors.R:86](https://github.com/vdorie/dbarts/blob/b102e17cc7226338e7c7b26f73eedae617c8026f/inst/tinytest/test-model-priors.R#L86); me2/ml1/gl1 (the node-prior designation refusals) survive
