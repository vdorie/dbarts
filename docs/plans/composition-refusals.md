...
cells in docs/design/feature-matrix.md's table 4 (:186 `grouped` row x `variance forest` column, :187 `hetero` row x
`grouped ranef` column, both `?` with note [f30]:727-731) because the matrix is read along both axes, and the note says
what the code says: they "still CONSTRUCT (CH:641 decorates before CH:742 builds the variance forest)". Both anchors hold
at this tip - src/bartcore/chain.hpp:641-646 wraps `response_` in `GroupedResponse`, chain.hpp:738-742 builds the variance
forest afterwards.

Where each half can be declared, taken from formals:

- Variance forest: `dbarts(variance = )` (R/dbarts.R:369, forwarded :726), `bart2(variance = )` (R/bart.R:687, which
  builds its own control and calls `dbarts()`), `dbartsSpec(variance = )` (R/spec.R:783, forwarded :878). All three funnel
  into `resolveSamplerSpec` (R/spec.R:102), which resolves the selector at :507 and attaches `bartcore.variance` to the
  control at :532. A LinkingTo consumer can attach that attribute itself; the header states the route at
  inst/include/dbarts/dbarts.h:836 ("is flat-creatable (its variance forest rides a control attribute)").
- Grouped random intercepts: NO public formal outside `rbart_vi(group.by = )` (R/rbart.R:17), which writes
  `bartcore.groups` at R/rbart.R:385 and is the only shipped writer ([f8], feature-matrix.md:307-309). The attribute IS
  the interface: `resolveSamplerSpec` only READS it (R/spec.R:449, :664) and the bridge reads it at creation
  (`applyGroupAttribute`, src/R_interface_bartcore.cpp:1905, called once at :3014). The flat API documents the same route
  in `dbarts_sampler_setResponse` (src/C_interface.cpp:630-632, "this entry's control carries whatever bartcore.groups
...
1. `dbarts(x, y, control = <control carrying bartcore.groups>, variance = ...)` - what
   benchmarks/R/composition-matrix.R's two `?` probes build (`extra:variance` on the `grouped` fixture at :664-669,
   `extra:groups` on the `hetero` fixture at :670-682, both through `do.call(dbarts, args)`). Resolution:
   `resolveSamplerSpec`.
2. `dbartsSpec(data, control = <same>, variance = ...)` (R/spec.R:772) - the exported spec surface, same
...
3. `new("dbartsSampler", control, model, data)` - the class is exported (NAMESPACE:24 `exportClasses(dbartsSampler)`) and
   its `initialize` (R/dbarts.R:905-923) calls `C_dbarts_bartcore_create` on the control it is handed, touching
   `resolveSamplerSpec` at no point. This is not a back door: it is the route `dbarts()` itself takes at R/dbarts.R:750.
   A caller holding a control with both attributes - one from `dbartsSpec()`, one added by hand - reaches the engine here.
4. `dbarts_sampler_create(control, model, data, family)` (src/C_interface.cpp:519-524), which calls
   `bartcore_bridge::createHolder` (src/R_interface_bartcore.cpp:2991) directly.
5. Re-creation from a stored control: `getPointer()` (R/dbarts.R:1895) and `setState()` (:1928) both call
   `C_dbarts_bartcore_create` with the R-side `control`, `model` and `data` after a session boundary invalidates the
   pointer. And the stored control can ACQUIRE an attribute: `$setControl`'s carry-forward loop (R/dbarts.R:1180-1196)
   ...
   `setControl` itself ignores the attributes, src/R_interface_bartcore.cpp:1901-1903 - which is exactly why the
...
R/rbart.R:9-52), so R's own wall raises "unused argument" - pinned at inst/tinytest/test-heteroscedastic.R:258-262;
...
- `buildVarianceForest` (chain.hpp:4176) ends with `sigmaIsFixed_ = true; sigma_ = 1.0;` (:4203-4204): under a variance
  forest sigma is not a parameter, and the residual scale lives in `s^2(x)` row by row - what `setSigma`'s own comment
  states, chain.hpp:1705-1712.
- Each sweep calls `response_->refreshLatents(rng_, combined, sigma_)` (chain.hpp:1535, and :2029 on the callback loop),
  so the grouped decorator is handed sigma = 1.0 forever.
- `GroupedResponse::refreshLatents` (model.hpp:4746-4756) passes that sigma, and `base_->workingWeights()` - the USER
  weights, not the heteroscedastic precisions - to `drawGroupEffects` (model.hpp:4668-4691), whose conditional is
  `prec_j = (sum_{i in j} w_i) / sigma^2 + 1/tau^2`, `mean_j = (sum_i w_i (z_i - F_i)) / (sigma^2 prec_j)`.
- The mean forest meanwhile backfits against `meanWeights_[i] = w_i / s^2(x_i)` (chain.hpp:1409-1416, formed at :4212),
  and the variance forest backfits against `y - meanFits` where `y` is the grouped working response `z - b`
  (chain.hpp:1536, :1555-1556).
...
`rel.scale = sd(y)` (R/rbart.R:389, model.hpp:4712-4716), a homoscedastic calibration a variance surface leaves undefined.
...
cells' verdicts under "? resolutions" (the runner at :706-733); expect `grouped varianceForest -> CONSTRUCTS` and
...
`rbart_vi`'s family token list, bridge backstop at src/R_interface_bartcore.cpp:3016-3025). Five entrances, two sites: the
...
(a) R, inside `resolveSamplerSpec`'s variance block (R/spec.R:508-527), after the Student-t refusal at :519-524 and
before the monotone one at :525-527 - the block's order is family, then residual law, then leaf constraint, and this is a
...
construction. Placement after the `family != "gaussian"` check at :509-515 means a grouped probit + `variance` still gets
...
(src/R_interface_bartcore.cpp:3031-3032), in the shape of the two group refusals ten lines above:
...
(R/dbarts.R:922, :1895, :1928 and src/C_interface.cpp:521 all call `C_dbarts_bartcore_create` / `createHolder`), so the
...
(:680-697), which is where the sibling refusal's memo lives. So this memo lands as a NEW section 17 of
...
all (R/rbart.R:9-52), which is why R's own unused-argument wall is what raises. feature-matrix [f30] has recorded the
...
Records edited in the same commit, so no doc reads as if the cells were still open: feature-matrix.md:186 and :187 become
`R spec.R:NNN` (the two `?` cells; NNN is the new stop's line), [f30]:727-731 loses its "still CONSTRUCT" sentence and
gains one naming the refusal and pointing at heteroscedastic.md section 17 by title, and :1010-1011's grouped narrative
("Composition with a variance forest constructs unrefused and untested ([f30])") is rewritten. benchmarks/R/
composition-matrix.R needs no edit: it probes only `S` and `?` cells (:706, :715-731), so the two drop out of the probe
...
|---|---|---|
| `predict.bart` | :296 | `object$fit$predict(newdata, offset, n.threads)` :386 (and `predictForest` :668, `predictBlend` :808, through the same R5 methods) |
| `predict.bartMultinomial` | :1216 | `validateXTest(newdata, object$fit$data@x)` :1240, then `$predict` :1267 (which validates again) |
| `predict.bartOrdinal` | :1502 | `validateXTest` :1524, then `bartcorePredict` :1534 |
| `predict.bartNegbin` | :1754 | `validateXTest` :1777, then `bartcorePredict` :1786 |
| `predict.bartHurdle` | :2147 | `hurdleParts` :1990 -> two `predict.bart` calls |
| `predict.rbart` | :2194 | `object$fit[[i]]$predict(newdata, offset, n.threads)` :2249, :2259, :2269 |

Plus `survivalProbabilities.bart` / `.rbart` (R/bart.R:2500, :2551), which build an expanded newdata and forward to
`predict`, and the R5 surface itself: `$predict` (R/dbarts.R:1084) and `$predictForests` (:1147) both open with
`validateXTest(x.test, data@x)` (:1088, :1155), as do `$setTestPredictorAndOffset` (:1586) and `$getTrees(newdata)`
(:2057). `bartcorePredict` (R/bartcore.R:1085) does NOT validate - it coerces to a double matrix and calls the bridge -
which is why the two families that use it validate in the generic instead.

So `validateXTest` (R/data.R:50-283) is the single R-side chokepoint for out-of-sample predictors: every S3 predict,
every R5 predict/test-surface method, `dbartsData(test = )` (R/data.R:1492), and the EXPORTED `makeTestModelMatrix`
(R/data.R:46, `export(makeind, makeModelMatrixFromDataFrame, makeTestModelMatrix)` at NAMESPACE:16, documented
man/makeind.Rd:12) all reach it. It returns with the test columns already reordered and renamed onto the training columns
(:265-279), so column j of its value is column j of `x.train`.

Three routes do not pass through it, and the design states them as the contract's edge rather than chasing them: the
per-column `$setTestPredictor(x, column)` form (R/bartcore.R:571-600 takes the `column` branch), the internal store-view
build (section 8's residue), and DIRECT SLOT ASSIGNMENT - `data@x.test <- ...` on a `dbartsData` object, which bartCause
does at fit time (bartCause R/responseFit.R:180-189). The contract this slice ships is therefore: the validated surfaces
...
- C++: `ColumnStore::hasMissing`, one `uint8_t` per column (src/bartcore/data.hpp:535, its invariant comment at :533-534),
  assigned by every full quantize (`quantizeDenseInto` :954, `quantizeCscColumnInto` :999, both
  `hasMissingOut[j] = anyMissing`) and by a view build (:1465). This is the record the ENGINE routes by, and it gates the
...
- R: `data@x` keeps its NAs (pinned, inst/tinytest/test-data-missing.R:20), and `validateXTest` already holds it as
  `x.train`.

Saved-fit consequence: NONE. The record is derived at build time from the stored data object, never written to state, so
`stateFormatVersion` stays 3 and `minReadableStateFormatVersion` stays 3 (src/R_interface_bartcore.cpp:6390, :6399); a
...
`hasMissing[j]` when a value becomes NA and never clears it (data.hpp:1755-1764, "the flag only clears on a full column
...
reachable: `updatePredictorPerObservationJointly` writes the mutated column back to `data@x` (R/updatePredictorPerObservationJointly.R:89)
while the engine's commit sets the flag through `setCell` (src/bartcore/sampler.hpp:1938). Recorded as residue; a
...
- PREDICT (both `predictFromSavedSample`, chain.hpp:2832, and `predictFromCurrentTrees`, :2857, which flattens the live
  trees on the fly) replays through `addFlatPredictions` -> `partitionFlatIndices` (tree.hpp:1727), which reads RAW
  values and branches on `isNA(value)` against `missingGoesLeft = (flat.flags & flatMissingGoesRight) == 0` (:1732). The
  flag is set when a tree is flattened only if the rule carries the bit (:1465), and `buildFromFlat` REFUSES to restore
  it on an NA-free column (:1558-1560). `codeFor` is not on this path at all.
- The RUN's own test-fit channel (a test set installed on the sampler by `dbartsData(test = )` or
  `$setTestPredictor`) quantizes instead: `ColumnStore::codeFor` (data.hpp:695-706) codes any NA to `naCode` for an
  ordinal column and to `missingCategoryCode(numCuts[j])` for a categorical one, and `Rule::sendsRight` (tree.hpp:155-162)
  returns `missingGoesRight()` for `naCode` while the categorical mask never sets the missing bit on an NA-free column
  (tree.hpp:479).

Either way the bit is 0, because it is drawn only where `data.hasMissing[variableIndex]` (tree.hpp:450, :609, :875,
:889): on a training-complete column the row goes LEFT at every split on that column, in every tree, in every draw.
`validateColumnValues` (src/R_interface_bartcore.cpp:2911-2917) passes the value through - `categoricalValueIsValid`
returns true for NA by construction (data.hpp:707-712). So both surfaces return numbers, silently, from a route the model
never learned. man/dbarts.Rd:117 promises the opposite ("in training and test data alike") and inst/NEWS.Rd:720-725
...
carried training NAs (the learned route - the case test-data-missing.R:43-57 and :97-100 pin), and every complete
...
- Hurdle composes TWO samplers over different row sets (R/bart.R:1130, "occupancy probit on 1{y > 0} (all n) and a
  gaussian"; `hurdleParts`, R/generics.R:1990, forwards `newdata` to each). The positive part trains on `{i : y_i > 0}`
...
- rbart's per-chain samplers (R/generics.R:2249-2269) share one data object, so all chains agree; the refusal fires on
  the first.

Other predict arguments are unaffected and stay complete-only where they already were: `offset` / `offset.test` are
refused with NAs at ingestion (R/data.R:1713-1716) and a length-1 `NA` on the R5 flat path still reads as "no offset"
(R/dbarts.R:1136-1141); `weights` feeds a posterior-predictive draw and never a route; `bases` is a caller-supplied basis
...
(a) R: one helper called at the tail of `validateXTest` (R/data.R, immediately before its `x.test` return at :283). That
...
(R/mixedMatrix.R:413-441), which already answers for a plain matrix, a `dbartsMixedMatrix` and a bare `dgCMatrix`:
    ...
    refuseTestMissingness <- function(x.test, x.train) {
      ...
      # the two sides carry the same p by now (:220 refuses otherwise), so one
...
column. `predict.bartMultinomial` validates twice - once in the generic (R/generics.R:1240) and once inside `$predict`
(R/dbarts.R:1088) - so its scan runs twice; that is a pre-existing double coding this slice inherits rather than
introduces, and an `anyNA` short-circuit is cheap enough to leave alone rather than restructure a predict path for.

`validateXTest`'s own messages say `'test'` at every existing site (:190, :220, :258), including when the caller is
...
- The internal store view (`ColumnStore::buildFromParent`, data.hpp:1392, built at src/R_interface_bartcore.cpp:3678) is
  how xbart's folds are cut. A view recomputes `hasMissing` from ITS gathered training rows (:1459-1465) while the test
  side "gates no draws, so it tracks no missingness" (:1050), so a held-out row can be NA on a column complete within its
  fold and route left. Refusing it would mean a data-dependent per-fold error at ingestion; out of scope.
- `$setTestPredictor(x, column)`'s per-column form (R/bartcore.R:571-600) and direct `data@x.test <-` assignment
...
- man/dbarts.Rd:116-118 `\item{missing}`, the one D8 names. Replace the clause "in training and test data alike" and add
...
- man/bart2.Rd:311-313 `\item{missing}` says "exactly as in \code{\link{dbarts}}" and needs one added sentence: the
  refusal on a training-complete column, since bart2 is where most users meet it.
- man/makeind.Rd, whose `makeTestModelMatrix(data, newdata)` entry (:12) is an exported caller of `validateXTest` and so
...
- man/xbart.Rd:99-101 defers wholly to `?dbarts` and needs no edit.
- inst/NEWS.Rd:720-725, the 1.0-0 NEW FEATURES entry that repeats "training and test data alike": same correction. It is
  unreleased text, so this is a fix, not a rewrite of history.
- Two new `\item`s in `\subsection{UPGRADING}` (inst/NEWS.Rd:5), one per commit: D6 (the composition is a validation
...
- D6: nothing breaks. No in-repo test constructs both halves - the four files that mention `bartcore.groups` and
  `variance` keep them in separate cells (test-fits-without-offset.R:179-207, test-calibration-prior-draws.R:219-271,
  test-bcf-creation.R, test-capi.R) - and test-heteroscedastic.R:222-262 already pins the rbart_vi wall.
- D8: ONE test must be rewritten. inst/tinytest/test-sparse-factor.R:798-806, under the "NO FALSE REFUSALS" banner at
  :783, builds `test.dense.na <- cbind(rnorm(3L), c(0, NA, 2))` over columns `x1`/`f` and asserts that
  `dbarts(dbartsData(train.bound, y.bound, test = test.dense.na), control = boundControl)` constructs. `train.bound`
  (:616, `sparseFrame(labels.bound, levels.small, "a")`) carries no NA in `f`, so the new rule refuses it. Its intent -
  ...
  at the END of the file, since the file is seeded at :610 and a new `rnorm` call would otherwise shift every draw after
  ...
  (test-data-missing.R:9-11 puts NAs in `x1` and `g`; `test.df <- df[1:20, c("x1","x2","g")]` at :45 inherits exactly
  those), so `dbarts(..., test = test.df)` at :52 and `sampler.keep$predict(test.df)` at :97 still pass. The implementer
...
unadjudicated with 'variance'" (:222) rather than opening a file:
...
D6, in inst/tinytest/test-capi.R beside the existing `capi_create` refusal probes (:118-150), reusing the grouped control
the file already builds at :776: that control plus a `bartcore.variance` attribute through `capi_create` raises
...
column stores missing as NaN (R/mixedMatrix.R:50-56), so both storages must be exercised.
...
- stan4bart, /Users/vdorie/Repositories/stan4bart branch `bartcore` (f9bca65). D6: CANNOT hit it - it resolves through
  `dbartsSpec` (R/stan4bart_fit.R:554-559) and explicitly refuses `variance` in `bart_args` (:544-548, "the parametric
  component draws the residual standard deviation"), and it never writes `bartcore.groups` (its only control attribute is
  `n.cuts`, :524). D8: it calls the FLAT `dbarts_sampler_predict` (src/init.cpp:349) with a dense source, so it is
  exactly the consumer the C++ backstop serves - but it forbids `na.action = na.pass` outright (R/stan4bart.R:41-42) and
...
- bartCause, /Users/vdorie/Repositories/bartCause branch `dbarts-1.0` (d825cfc). D6: reaches the grouped surface only
  through `rbart_vi` (R/responseFit.R:121, :210; R/treatmentFit.R:107), which has no `variance` formal - unreachable.
  D8: its predict calls (R/generics.R:143-176) pass `x.new` built from the user's confounders; it validates the treatment
  column for NA (:209) but not the rest, so a bartCause user with NA confounders on a training-complete column is now
  refused instead of silently answered - the intended change, and BART with NA predictors is new in 1.0, so nothing there
  can be relying on it. Its fit-time `data@x.test <-` assignment (R/responseFit.R:180-189) is outside the validated
...
- treatSens, branch `dbarts-1.0` (1db3d89). D6: creates through
  `dbarts_sampler_create` (src/bartTreatmentModel.cpp:63, src/sensitivityAnalysis.cpp:260) with neither attribute -
  unreachable. D8: it IS a test-surface user - `dbartsData(x, y, x.test)` (R/cibart.R:56) and `sampler$setTestPredictor`
  (R/treatSensBART.R:228, :245, :253) - so the R check runs on its data; its designs are simulated numeric confounders
...
- bairrtt, /Users/vdorie/Repositories/bairrtt branch `main` (6167423). D6: no dbarts composition surface at all. D8: it
  predicts through the R5 `$predict(frame)` six times (R/irt_causal_bart.R:568-713), which is on the checked route, but
  it refuses NA covariates at ingestion itself (`as_covariate_frame`, R/engine.R:303-305, "'x' must not contain NA") and
...
1. `composition-refusals`. R/spec.R (one refusal in the variance block); src/R_interface_bartcore.cpp (one refusal after
   ...
   cross-reference); docs/design/feature-matrix.md:186, :187, :727-731, :1010-1011; one NEWS UPGRADING item.
...
2. `predict-na-refusal`. R/data.R (two helpers plus one call at `validateXTest`'s tail); src/C_interface.cpp (one helper
   plus one call in `validateTestSource`); man/dbarts.Rd:116-118; man/bart2.Rd:311-313; man/makeind.Rd;
   inst/NEWS.Rd:720-725 plus one UPGRADING item; inst/tinytest/test-data-missing.R, test-data-missing-matrix.R,
...
70 anchors sit past src/R_interface_bartcore.cpp:3032 across 11 docs/design files (feature-matrix.md alone holds 53),
14 past R/spec.R:527 across 4, and 35 past R/data.R:283 and src/C_interface.cpp:257 across 11. The anchor pass runs LAST
...
1. D8 covers `dbartsData(test = )` and `$setTestPredictorAndOffset`, not `predict` alone, by placing the check in
   ...
   in the plan: test-sparse-factor.R:798-806's fixture is rewritten (section 10) and man/makeind.Rd gains a line
