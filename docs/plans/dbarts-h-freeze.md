...
`printTrees` gains `useLiveTrees` in getTrees' exact position, after `numTreeIndices` and before `forest`: [[dbarts.h:474-480@f162d075]] (list), [[dbarts.h:909-917@f162d075]] (prototype + doc),
...
Every stub calls `dbarts_stub_checkApiHash()` on its resolution branch ([[dbarts.h:610-621@f162d075]]) and that check is hash EQUALITY ([[dbarts.h:572-587@f162d075]]), so an additive MINOR would
hard-error every stubs consumer - the inverse of [[dbarts.h:29-33@f162d075]] and [[dbarts.h:144-148@f162d075]]. The version accessors ARE list entries ([[dbarts.h:415-416@f162d075]]) and are NOT stub-exempt, so the new check MUST
resolve them raw through `R_GetCCallable` as the current one resolves `dbarts_apiHash` ([[dbarts.h:577-578@f162d075]]); calling the stubs would re-enter it. Replace [[dbarts.h:572-587@f162d075]] with:
    ...
    static inline void dbarts_stub_checkApi(void) {
      ...
      /* today's hash-equality check and its [[dbarts.h:581-585@f162d075]] message, verbatim, with dbarts_apiHash
...
Two literals move together, in the header edit's commit: `DBARTS_C_API_HASH` ([[dbarts.h:153@f162d075]]) and the `dbarts_apiSignatureToken` assert ([[src/C_interface.cpp:461@f162d075]]); the
combined assert is [[src/C_interface.cpp:465-470@f162d075]]. `DBARTS_C_API_MAJOR`/`MINOR` DO NOT MOVE - no version constant increments before 1.0-0 ([[dbarts.h:111-113@f162d075]]). Neither assert prints its value,
...
consumer at [[inst/tinytest/test-capi.R:29-56@f162d075]] covers the clean path, so both macro modes compile. Plus section 5's hash pin edits. NEWS: one new `\item` in the 1.0-0 `\subsection{C API}`
([[inst/NEWS.Rd:956-1023@f162d075]], before the `\itemize` close at [[inst/NEWS.Rd:1022@f162d075]]) covering `dbarts_family`, `dbarts_sampler_family`, the four renames, `printEvery` as `size_t`,
...
alone pushes [[docs/design/public-surface.md:411@f162d075]] (`dbarts_sampler_setCallback` at [[dbarts.h:439@f162d075]], STRICT) onto `getLatents`. Run `Rscript tools/check-doc-freshness.R .` after
