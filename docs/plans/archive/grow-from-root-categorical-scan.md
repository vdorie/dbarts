...
window: after the multinomial counts/offset channel
  (docs/plans/archive/multinomial-counts-mutation.md), whose S4 re-records the
...
1. **The arithmetic-only renormalization stays CLOSED** (D1 of
   docs/plans/archive/grow-from-root-default-study.md; matched its pre-registered null
...
- `Chain::growForestFromRoot` is [src/bartcore/chain.hpp:1444](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1444), and its SHAPE changed: it now
  loops `forests_` and, under a combiner, calls `drawForestGlue` then
  `formForestResponse` then `composeForestWeights` ([src/bartcore/chain.hpp:1467-1477](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1467-L1477)), so
  `growTreeFromRoot` receives an already-composed `forestY`/`forestWeights`.
  The multinomial CATEGORY OFFSET therefore enters UPSTREAM, at [src/bartcore/chain.hpp:1473](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1473);
...
- `compactMaskPoolIfNeeded` is [src/bartcore/chain.hpp:1494-1495](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1494-L1495) on the grow path (and
  [src/bartcore/chain.hpp:1139-1140](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1139-L1140) on run's); still guarded by `data_.hasPooledCategorical`,
  so "Post-draw rule assembly" step 4's no-mark/no-truncate claim stands.
- The per-forest DART close-of-sweep block is [src/bartcore/chain.hpp:1518-1530](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1518-L1530), the shape
  "The DART consumer" already describes.
- `CGMTreePrior::ruleForVariableLogProbability` is [src/bartcore/model.hpp:2093](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/model.hpp#L2093);
  `drawCategoryPattern` [src/bartcore/model.hpp:2197](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/model.hpp#L2197), `drawCategoryPatternWide` [src/bartcore/model.hpp:2213](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/model.hpp#L2213).
...
- `$growFromRoot` ([R/dbarts.R:841](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/R/dbarts.R#L841)) now opens with
...
1. Delete the `logCut -= std::log(2.0)` line ([src/bartcore/grow.hpp:114](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/grow.hpp#L114)). Keep the
...
- Pre-existing hazard found in the census, unrelated and unreached here:
  `storeVarianceSavedTrees` and `rebuildVarianceForest` ([src/bartcore/chain.hpp:3577](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L3577) and
  [src/bartcore/chain.hpp:3612](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L3612) at 9f92074a, where the census's finding was re-confirmed live) call
