...
[src/bartcore/model.hpp:95-138](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/model.hpp#L95-L138). Each occupied bottom node value:

    mu_l ~ N(0, (scale/k)^2)   iid across leaves,     ([src/bartcore/model.hpp:95](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/model.hpp#L95), [src/bartcore/model.hpp:137-138](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/model.hpp#L137-L138))

with `scale = nodeScale/sqrt(numTrees)`. For the BCF prognostic forest
`buildBCFForest` sets `nodeScale = s = scaledResponseSd()`
([src/bartcore/chain.hpp:474](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L474), 1958-1969), `k = 1`, `updateK = false`
([src/bartcore/chain.hpp:1987](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1987), [src/bartcore/chain.hpp:1985](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1985)), so

    leafVar := (scale/k)^2 = s^2 / T_mu,   T_mu = # prognostic trees.

Define the leaf prior precision `P_leaf := (k/scale)^2 = 1/leafVar`
([src/bartcore/model.hpp:112](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/model.hpp#L112), [src/bartcore/model.hpp:129](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/model.hpp#L129)). Keep the general `(k/scale)^2` form in code so the
move stays correct if k ever varies; for shipped BCF `k = 1`.

The prognostic total is `mu(x_i) = sum_l mu_l 1[i in leaf l]`, stored as
`forests_[0].totalFits` ([src/bartcore/chain.hpp:2030](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L2030), [src/bartcore/chain.hpp:505-508](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L505-L508)); each tree's per-obs
leaf-value fits live in `forests_[0].treeFits` ([src/bartcore/chain.hpp:270](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L270), [src/bartcore/chain.hpp:615](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L615)).

Empty leaves are forced to value 0 and are NOT prior draws
([src/bartcore/chain.hpp:1884-1895](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1884-L1895), "a forced-zero empty leaf is not a draw from the
...
([src/bartcore/chain.hpp:1892-1894](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1892-L1894)) -- but that accumulation is gated on
...
drawGlue, [src/bartcore/chain.hpp:2042-2066](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L2042-L2066); audit block 6, correctness-audit.md.
Scale-mixture representation of the half-Cauchy:

    aVariance ~ IG(1/2, aPriorScale^2/2)      (prior on the auxiliary)
    a | aVariance ~ N(0, aVariance)            ([src/bartcore/chain.hpp:2049](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L2049) uses 1/aVariance as the a-prior precision)
    => marginally a ~ Cauchy(0, aPriorScale)   (Monte-Carlo verified, audit:348-350)

Conditional refresh after the a-draw ([src/bartcore/chain.hpp:2060-2065](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L2060-L2065)):

    aVariance | a ~ IG(1, (a^2 + aPriorScale^2)/2).

The conjugate a-draw itself (mu held fixed, [src/bartcore/chain.hpp:2048-2058](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L2048-L2058)):
...
([src/bartcore/chain.hpp:2026-2036](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L2026-L2036) combined fit; bcf.md). `b0,b1 ~ N(0, bPriorVariance)`
(default 1/2, [src/bartcore/chain.hpp:2068-2080](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L2068-L2080)). The tau forest and b0/b1 are NOT
touched by the move.

### 1.4 Sweep placement

run() loop [src/bartcore/chain.hpp:579-709](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L579-L709): forest loop (mu backfit, then tau backfit)
...
- **1e-9 multiplier floor** (formForestResponse, [src/bartcore/chain.hpp:2020](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L2020)): floors
...
- **updateA gating**: the move CHANGES a; if `bcf_->updateA == false` (a
  ...
  move on `bcf_->updateA` ([src/bartcore/chain.hpp:2048](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L2048)'s own guard). updateB is
...
Required:
- `bcf_->a *= 1.0/c`                        ([src/bartcore/chain.hpp:234](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L234), [src/bartcore/chain.hpp:324](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L324))
- `forests_[0].treeFits *= c`  over all `n*T_mu` entries ([src/bartcore/chain.hpp:270](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L270)).
  MANDATORY: the next sweep's residual roll reads each `treeFits[t]` slab
  once ([src/bartcore/chain.hpp:617-630](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L617-L630), `oldFits`) before overwriting it; a stale
...
- `forests_[0].totalFits *= c`  over `n` ([src/bartcore/chain.hpp:271](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L271)). Read by the roll
  ([src/bartcore/chain.hpp:619](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L619)) and by combinedFits/storeSample.

Required when this sweep is recorded AND test data present:
- `forests_[0].totalTestFits *= c` and `forests_[0].currTestFits *= c`
  (n_test each, [src/bartcore/chain.hpp:271-272](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L271-L272)). [FLAG] Under BCF the testFits results
  channel is NaN'd / the test surface refused (bcf-testfits-guard, LANDED
  2026-07-08, [TODO:180-189](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/TODO#L180-L189)), so these arrays are diagnostically dead, but
...
- The mu-forest SAVED tree records for this sweep's slot were flattened at
  [src/bartcore/chain.hpp:651](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L651) with the PRE-move leaf values. After the move, the reported
  ...
  in this slot's T_mu saved mu trees by c ([src/bartcore/chain.hpp:1127-1156](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1127-L1156) slot layout;
...
- `bcf_->aVariance`: the move conditions on it; it stays. It was drawn
  `| a0` at [src/bartcore/chain.hpp:2065](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L2065); leaving it conditioned on the pre-move a is a
...
[src/bartcore/chain.hpp:1774-1788](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1774-L1788)) and accumulate `M += v*v; L += 1`, skipping empty
nodes -- mirroring the k-accumulator ([src/bartcore/chain.hpp:1892-1894](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L1892-L1894)) but run
UNCONDITIONALLY (not gated on updateK). Cost O(L + total nodes).

Recording note: `storeSample` records `scale*(a*mu + b_z*tau)+shift`
([src/bartcore/chain.hpp:2100-2103](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L2100-L2103)) which is INVARIANT -> trainingFits, and results.k /
...
- `benchmarks/R/bcf-exact.R`, `bcf-exact-weak.R` MUST STAY EXACT. They match
  ...
  [benchmarks/R/bcf-exact.R:16-18](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/benchmarks/R/bcf-exact.R#L16-L18), [benchmarks/R/bcf-exact.R:33-38](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/benchmarks/R/bcf-exact.R#L33-L38)), all of which the posterior-preserving move
  ...
  "mixes well" already, [benchmarks/R/bcf-exact.R:34](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/benchmarks/R/bcf-exact.R#L34)).
- `benchmarks/R/equivalence.R`: its scenarios (makeScenarios, lines 60-285:
  ...
  TODO "anchor re-record" ([TODO:174](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/TODO#L174)) bites only a BCF-specific snapshot: if a
...
(A4e measured ~2500-6600 sweeps pre-move; [TODO:178](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/TODO#L178)). Controls (abs.a,
...
roll ([src/bartcore/chain.hpp:617-630](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L617-L630)) which reads each `treeFits[t]` once before
...
- 2026-07-10: derivation complete (read-only Opus deriver), prototype
  ...
  drawGlue ([src/bartcore/chain.hpp:651](https://github.com/vdorie/dbarts/blob/4c018187036ff83eddde8308f243ed6584268004/src/bartcore/chain.hpp#L651)) - the saved slot must be rescaled or the
