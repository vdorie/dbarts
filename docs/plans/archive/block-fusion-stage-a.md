...
([src/bartcore/tree.hpp:175-177](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L175-L177)). Every consumer reads ONLY that cache:
- move scoring: `logLikelihoodForBranch` -> `logIntegratedLikelihoodForNode`
  reads the node cache ([src/bartcore/moves.hpp:47-65](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/moves.hpp#L47-L65), [src/bartcore/model.hpp:146-154](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/model.hpp#L146-L154));
- leaf draw: `drawFromPosteriorForNode` reads the node cache ([src/bartcore/model.hpp:156-162](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/model.hpp#L156-L162));
- k accumulation + fit scatter: `sampleParametersAndSetFits` reads the drawn
  param ([src/bartcore/chain.hpp:2105-2136](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L2105-L2136)).
Only FOUR sites WRITE that cache: `setNodeAverages`/`computeLeafStats`
([src/bartcore/tree.hpp:492-521](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L492-L521)), `birth`'s two child `computeLeafStats` ([src/bartcore/tree.hpp:780-781](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L780-L781)),
`refreshSubtree`'s leaf `computeLeafStats` ([src/bartcore/tree.hpp:760](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L760)), and
`orphanChildren`'s additive merge ([src/bartcore/tree.hpp:796-800](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L796-L800)).
...
loop. `misc_computeIndexedSufficientStatisticsFast` ([src/misc/moments.c:334](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/misc/moments.c#L334)) and its
weighted/non-indexed siblings use a specific mod-5 remainder + stride-5
horizontal-sum accumulation tree ([src/misc/moments.c:315-332](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/misc/moments.c#L315-L332), 387-416). Any different
...
([src/bartcore/model.hpp:117-118](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/model.hpp#L117-L118)) needs `sumWeightedResponseSq`. Stage B DROPS Q (fact 1.2:
...
- DRAW. The draw loop stays `sampleParametersAndSetFits` ([src/bartcore/chain.hpp:2105-2136](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L2105-L2136))
...
- ROLL. At b=1 the residual roll is the existing per-tree fused O(n) pass
  ([src/bartcore/chain.hpp:731-744](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L731-L744)) and the block-exit `totalFits` rebuild ([src/bartcore/chain.hpp:779-787](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L779-L787))
...
- MERGE (death). `orphanChildren` ([src/bartcore/tree.hpp:796-800](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L796-L800)) already forms the parent
...
1. LEAF / RNG DRAW ORDER. Both `setNodeAverages` and the draw iterate leaves
   ...
   (`leftChild+1`), arena order ([src/bartcore/tree.hpp:282-286](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L282-L286)). The draw consumes one
...
3. ROOT SPECIAL CASE. `computeLeafStats` uses the NON-indexed kernel
   ...
   invalidNode` ([src/bartcore/tree.hpp:494-508](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L494-L508)), i.e. a stump/root leaf, reading y[0..n)
...
4. BIRTH CHILD ORDER. `birth` calls `partitionChildren` then
   `computeLeafStats(left)` then `computeLeafStats(right)` ([src/bartcore/tree.hpp:779-781](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L779-L781)),
...
5. DEATH MERGE ORDER. `orphanChildren`: parent = left + right ([src/bartcore/tree.hpp:796-800](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L796-L800)).
...
6. CHANGE / SWAP REFRESH ORDER. `refreshSubtree` does `partitionChildren` then
   recurses left, right, computing leaf stats in DFS order ([src/bartcore/tree.hpp:757-766](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L757-L766)).
   PIN: `AtomMap::refreshSubtree` mirrors the identical recursion order.
7. PARTITION PERMUTATION. `partitionChildren` ([src/bartcore/tree.hpp:672-739](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L672-L739)) is the
...
8. EMPTY-LEAF HANDLING. Empty leaf => param forced 0.0, NO draw consumed, NOT
   counted toward k ([src/bartcore/chain.hpp:2109-2120](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L2109-L2120)). An empty leaf has zero atoms. PIN:
...
9. k ACCUMULATION. `kSumSquaredParams += param*param` and `kNumLeaves += 1`
   over non-empty leaves in `fillBottom` order ([src/bartcore/chain.hpp:2117-2119](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L2117-L2119)). PIN:
...
10. SIGMA / SSR / LATENTS. `misc_computeSumOfSquaredResiduals` over totalFits
    ([src/bartcore/chain.hpp:908-910](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L908-L910)), `refreshLatents`, `drawSigma` - all once/sweep,
    OUTSIDE the block, over the totalFits rebuilt at block exit. PIN: untouched;
    the block-exit totalFits rebuild stays [src/bartcore/chain.hpp:779-787](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L779-L787).
11. BCF / GROUPED-RE FOREST RESPONSE. `formForestResponse` builds each forest's
    residual ([src/bartcore/chain.hpp:700-704](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L700-L704)). PIN: at b=1 the block is per tree within the
    already-formed forest residual; nothing changes. (Blocks are per-forest in
    Stage B; irrelevant at b=1.)
12. GROW-FROM-ROOT WARM START ([src/bartcore/chain.hpp:938-1048](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L938-L1048)) stays on the per-tree path
...
One `AtomMap` per `Forest` ([src/bartcore/chain.hpp:257-307](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L257-L307)), added next to `treeFits`,
`treeY`, `scratch`. The Forest is the natural owner: it already holds the
per-tree index buffers (`indexBuffer`, sliced n per tree, [src/bartcore/chain.hpp:452-455](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L452-L455)),
`treeFits`, and `MoveScratch`. Sized once at forest init (the `initialize`
site, [src/bartcore/chain.hpp:452-459](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/chain.hpp#L452-L459)) and reused across sweeps (persistence, design 4.5).
...
crossproduct cache ([src/bartcore/model.hpp:458-539](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/model.hpp#L458-L539)): cache A(c) + the atom topology, keyed by
the atom's ordered member list, and re-validate on lookup by `std::memcmp`
against `tree.indices[begin..end)` ([src/bartcore/model.hpp:509-511](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/model.hpp#L509-L511)). A structural move or a
...
per-move hook" ([src/bartcore/model.hpp:459-462](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/model.hpp#L459-L462)) - the hardest part (4.5) inherits a landed,
...
- WHAT: on birth of leaf L, `splitAtom` partitions L's atom by the tree's
  ...
  ([src/bartcore/tree.hpp:827-842](https://github.com/vdorie/dbarts/blob/3137e17a563eeb5879a248ab9bc71a4101101da1/src/bartcore/tree.hpp#L827-L842)) for change/swap prep in commit (v).
