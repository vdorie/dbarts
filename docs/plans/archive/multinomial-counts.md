...
- PG(n_i, psi) for INTEGER n_i is the sum of n_i iid PG(1, psi) draws; the only
  ...
  ([src/include/external/random.h:126](https://github.com/vdorie/dbarts/blob/6a48351bd533cfc8a652abb0f183ab48478b0247/src/include/external/random.h#L126); [src/external/random.c:581](https://github.com/vdorie/dbarts/blob/6a48351bd533cfc8a652abb0f183ab48478b0247/src/external/random.c#L581)). There is NO
  direct PG(n, .) sampler. LogisticResponse already sums a weight's worth of
  PG(1, psi) draws ([src/bartcore/model.hpp:2236-2242](https://github.com/vdorie/dbarts/blob/6a48351bd533cfc8a652abb0f183ab48478b0247/src/bartcore/model.hpp#L2236-L2242), `reps = lround(weights_[i])`); the count
...
- The combiner reads the response ONLY in two functions; combinedFits (651-665),
  ...
  - drawForestGlue(f) ([src/bartcore/combiner.hpp:620-630](https://github.com/vdorie/dbarts/blob/6a48351bd533cfc8a652abb0f183ab48478b0247/src/bartcore/combiner.hpp#L620-L630)): today one PG(1, .) draw per
  ...
  - formForestResponse(f) ([src/bartcore/combiner.hpp:635-647](https://github.com/vdorie/dbarts/blob/6a48351bd533cfc8a652abb0f183ab48478b0247/src/bartcore/combiner.hpp#L635-L647)): today
    `yif = labels_[i] == f ? 1.0 : 0.0; forestResponse_[i] = (yif - 0.5)/omega
    + margins_[i]`. Becomes `yif = counts_[f*n + i]; (yif - trials_[i]*0.5)/omega
    + margins_[i]`.
- MultinomialSpec ([src/bartcore/combiner.hpp:181-191](https://github.com/vdorie/dbarts/blob/6a48351bd533cfc8a652abb0f183ab48478b0247/src/bartcore/combiner.hpp#L181-L191)) carries `const int* labels`; the ctor
  (591-602) stores labels_, cold-starts omega_ at 1/4, sizes n x K scratch.
  Combiner members at 760-766.
- MultinomialResponse ([src/bartcore/model.hpp:2339-2382](https://github.com/vdorie/dbarts/blob/6a48351bd533cfc8a652abb0f183ab48478b0247/src/bartcore/model.hpp#L2339-L2382)) is unchanged - it owns no labels and
  no counts (the combiner does), and its seams are already vestigial no-ops.
- Chain multinomial ctor ([src/bartcore/chain.hpp:413-434](https://github.com/vdorie/dbarts/blob/6a48351bd533cfc8a652abb0f183ab48478b0247/src/bartcore/chain.hpp#L413-L434)) builds K forests + the combiner
  from the spec; createMultinomialSampler ([src/bartcore/facade.hpp:545-551](https://github.com/vdorie/dbarts/blob/6a48351bd533cfc8a652abb0f183ab48478b0247/src/bartcore/facade.hpp#L545-L551)) is the single
  ConstantGaussianLeaf instantiation.
- createMultinomialHolder ([src/R_interface_bartcore.cpp:1670-1748](https://github.com/vdorie/dbarts/blob/6a48351bd533cfc8a652abb0f183ab48478b0247/src/R_interface_bartcore.cpp#L1670-L1748)): parses labelsExpr
  ...
  ([R/bartcore.R:559-583](https://github.com/vdorie/dbarts/blob/6a48351bd533cfc8a652abb0f183ab48478b0247/R/bartcore.R#L559-L583)) is the internal wrapper.
...
C2. Public matrix interface (RNG-neutral surface). bart2(family = "multinomial")
   ...
   ([R/bart.R:472-499](https://github.com/vdorie/dbarts/blob/6a48351bd533cfc8a652abb0f183ab48478b0247/R/bart.R#L472-L499), beside the factor path); levels from colnames(Y) (or
