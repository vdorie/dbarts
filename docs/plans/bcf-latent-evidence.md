...
within each - four `(cell, z)` groups of 100, `n = 400` - no offset, and cell means `mu = (0.0, 0.3)`, `tau = (0.5,
0.8)`, so the index at the four groups is `(0.00, 0.50, 0.30, 0.80)` and `p` is `(0.500, 0.691, 0.618, 0.788)`, well
inside the unit interval, which is also what keeps `0 < s_g < n_g` at every group for the aggregated arm below. The
contrast is 0.3 in BOTH forests because that is what the 0.9 weight cap permits: at 100 rows per group the
split-versus-stump Bayes factor is `exp(O(n))`, so the four configurations' weights here are `(0.000, 0.014, 0.849,
0.138)`, and over 20 realizations of the design seed the largest runs 0.371-0.897, never breaching the cap - where a 0.4
contrast reaches 0.935 and an index range of -0.5 to 1.1 puts 0.948 on one configuration and zero on two of four. The
script recomputes those weights at its realized counts and refuses to run if the largest exceeds 0.9. At two cells each
...
leaf posterior is about 0.115 wide against that gate's 0.0034, so an absolute bound carries no fixed meaning across the
two - keeping 0.015 would leave a bound of about 12 se, not 4. At 100000 kept draws thinned by 10 (25000 by 5 in quick
mode) over three seeds the standard error lands near 6e-4 and the detection threshold near 2.4e-3, so the quadrature
must be good to about 1e-5. The refinement self-check therefore runs on the GLUE axis, where the error is: recompute the
...
or Polya-Gamma draw per row. Seven sampler configurations (three modes x two links plus the aggregated arm) at three
seeds put the full grid near 10 minutes and quick near 2, so quick is what CI runs inside exact-gates.yaml's shared
...
oracle: the gap in `E[a mu]` is 1.35e-2, 5.6x the detection threshold, while `E[tau]` moves 1.4e-3 and does not, so the
poison is named on `E[a mu]`. (iii) Build the oracle's leaf scales at the sample sd of the RANGE-SCALED binary response,
0.482, in place of the link's latent anchor: the factors are 2.08 under probit and 3.76 under logistic and every matched
quantity must fail. The anchor has to be named, because the other reading of "gaussian's anchor" - the engine's own, the
sd of the cold-start working response - is 1.04 and 0.94, a 4-6% prior error that sits under the threshold and would not
land. (iv) Score the AGGREGATED logistic arm against an oracle built at unit counts, two rows per group in place of the
...
flag: the veto conditions on positive weight, not the glue's value, so that swap moves the tree law in one replication
of 200. (iii) Draw a sigma in the generator as noise the fit cannot model: every latent functional must FLAG. Only (ii)
...
1. `benchmarks/R/bcf-latent-exact.R`: the design and its weight check, enumeration, block decomposition, adaptive GH at
   12 nodes, both glue axes on the tan substitution, the glue-axis refinement check, three modes x two links plus the
   aggregated logistic arm, batch-means z at `zBound = 4`; and its word in `.github/workflows/exact-gates.yaml`.
2. The independent derivation (Decision 3) and the four exact-gate poisons (Decision 4), recorded here; then three
...
1e-6, no configuration above 0.9 posterior weight; every SBC functional PASS but the two ill-posed glue ones.
