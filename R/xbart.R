                  ...
                  node.prior = NULL)
{
  ...
  if (method == "k-fold") {
    ...
    foldSizes <- rep.int(numObservations %/% n.test, n.test) +
      rep.int(c(1L, 0L), c(numObservations %% n.test, n.test - numObservations %% n.test))
    numTest <- 0L
  } else {
    n.test <- coerceOrError(n.test, "numeric")
    if (n.test > 1) n.test <- n.test / numObservations
    if (n.test <= 0 || n.test >= 1) stop("for random subsample crossvalidation, n.test must be in (0, 1)")
    numTest <- max(1L, min(numObservations - 1L, as.integer(round(n.test * numObservations))))
    foldSizes <- integer()
  }

  n.reps    <- coerceOrError(n.reps,    "integer")
  if (is.na(n.reps) || n.reps <= 0L) stop("'n.reps' must be a positive integer")
  ...
  cells <- expand.grid(iBase = seq_along(base), iPower = seq_along(power),
                       iK = seq_len(kLength), iTrees = seq_along(n.trees))
  numCells <- nrow(cells)

  spec <- namedList(
    control, model, data, method, foldSizes, numTest,
    n.samples = control@n.samples, n.burn, n.trees,
    kValues = if (kIsGrid) k else NA_real_, kIsGrid, power, base,
    cells, lossFunction
  )
  ...
  if (!is.na(seed)) {
    oldSeed <- if (exists(".Random.seed", globalenv())) get(".Random.seed", globalenv()) else NULL
    on.exit(
  ...
  if (verbose)
    cat("running ", numCells, " parameter combination", if (numCells > 1L) "s" else "",
        " x ", n.reps, " replication", if (n.reps > 1L) "s" else "",
        " on ", numChunks, " worker", if (numChunks > 1L) "s" else "", "\n", sep = "")

  if (numChunks == 1L) {
    chunkResults <- list(xbartRunChunk(spec, chunkIndices[[1L]], chunkSeeds[1L]))
  } else {
  ...
  linearIndex <- repIndex + n.reps * (
    (cells$iTrees[cellIndex] - 1L) + length(n.trees) * (
      (cells$iK[cellIndex] - 1L) + kLength * (
        (cells$iPower[cellIndex] - 1L) + length(power) * (cells$iBase[cellIndex] - 1L))))
  ...
  dimIncluded <- c(
    ...
    if (!kIsGrid) FALSE else if (drop) length(k) > 1L else TRUE,
  ...
  if (length(newDims) == 1L) {
    ...
    names(dimNames) <- c("rep", varNames[dimIncluded[2L:5L]],
                         if (numResults > 1L) "loss")
    for (varName in varNames[dimIncluded[2L:5L]]) {
      x <- get(varName)
      dimNames[[varName]] <- as.character(if (is.double(x)) signif(x, 2L) else x)
    }
...
xbartLossFunction <- function(loss, control)
{
  ...
  switch(loss,
    ...
    log = function(y.test, testSamples, weights) {
      ...
      if (is.null(weights)) -mean(logLikelihood)
      else -sum(weights * logLikelihood) / sum(weights)
    },
    mcr = function(y.test, testSamples, weights) {
      misclassified <- as.numeric(rowMeans(probFromLatent(testSamples)) > 0.5) != y.test
      if (is.null(weights)) mean(misclassified)
      else sum(weights * misclassified) / sum(weights)
    })
}
...
xbartRunChunk <- function(spec, repIndices, chunkSeed)
{
  ...
  cellModel <- function(cell) {
    ...
    if (spec$kIsGrid)
      result@node.hyperprior <- new("dbartsFixedHyperprior",
  ...
  for (i in seq_along(repIndices)) {
    if (spec$method == "k-fold") {
      ...
      for (fold in seq_along(spec$foldSizes)) {
        testRows <- sort(permutation[foldOffset + seq_len(spec$foldSizes[fold])])
