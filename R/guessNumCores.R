...
guessNumCores <- function(logical = FALSE) {
  if (is.na(logical)) {
    stop("logical cannot be NA")
  }
  if (length(logical) != 1) {
    stop("logical must be length 1")
  }

  cGuess <- .Call(C_dbarts_guessNumCores)
  if (logical == FALSE && !is.na(cGuess[1])) {
    return(cGuess[1])
  }
  if (logical == TRUE && !is.na(cGuess[2])) {
    return(cGuess[2])
  }
  ...
  for (i in seq_along(systems)) {
    if (length(grep(paste0("^", names(systems)[i]), R.version$os)) > 0) {
      for (command in systems[i]) {
        ...
        if (length(grep("^[1-9]", commandResult))) {
          return(as.integer(commandResult))
        }
      }
    }
  }
