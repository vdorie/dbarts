...
- y.train:
  ...
  link is fit. A factor with three or more levels is an error, whose
  message names both remedies:
  [`bart`](https://vdorie.github.io/dbarts/reference/bart.md)`(x, y, family = "multinomial")`
  for a multinomial fit, or `as.integer(y) - 1L` to keep the numbers
  dbarts 0.9-x fit from the level codes.
...
#> total seconds in loop: 0.148084
#> 
