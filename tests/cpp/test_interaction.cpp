...
void testSwapSiblingStrand(ext_rng* rng) {
  ...
  MoveScratch scratch;
  MoveContext ctx{store,          prior, 0.5, 1.0, 0.0, 0.5,
                  weights.data(), 2.0,   scratch};
...
void testChangeStrandInvariant(ext_rng* rng) {
  ...
  ConstantGaussianLeaf leaf{0.5};
  MoveScratch scratch;
  MoveContext ctx{store,          prior, 0.5, 0.1, 0.0, 0.5,
                  weights.data(), 2.0,   scratch};
...
void testExactPosterior(ext_rng* rng) {
  ...
  prior.power = power;
  MoveScratch scratch;
  MoveContext ctx{store, prior, 0.5, 0.1, 0.0, 0.5, nullptr, k, scratch};
