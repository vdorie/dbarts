...
ext_rng* ext_rng_createDefault(bool useNative)
{
  ...
  // generator, so validate those two fields individually. R packs further
  // method selectors into the higher digits of the encoded seed (sample.kind,
  // binom.kind, and whatever it adds next); those are deliberately ignored, as
  // a bound on the encoded value as a whole would reject seeds that are
...
#  endif
  ...
  if (algorithmType == EXT_RNG_ALGORITHM_USER_UNIFORM) free(state);
  if (result == NULL) return NULL;
...
#  endif
  ...
  if (stdNormalType == EXT_RNG_STANDARD_NORMAL_USER_NORM) free(normalState);
...
{
  if (algorithm > EXT_RNG_ALGORITHM_INVALID || algorithm < EXT_RNG_ALGORITHM_MERSENNE_TWISTER)
...
int ext_rng_setStandardNormalAlgorithm(ext_rng* generator, ext_rng_standardNormal_t standardNormalAlgorithm, const void* state)
{
  ...
  if (standardNormalAlgorithm == EXT_RNG_STANDARD_NORMAL_USER_NORM) {
    if (state != NULL) {
