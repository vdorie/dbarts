...
SEXP create(SEXP controlExpr, SEXP modelExpr, SEXP dataExpr) {
  ...
  if (std::strcmp(CHAR(STRING_ELT(classExpr, 0)), "dbartsControl") != 0)
    Rf_error(
      "'control' argument to dbarts_create not of class 'dbartsControl'"
    );

  classExpr = Rf_getAttrib(modelExpr, R_ClassSymbol);
  if (std::strcmp(CHAR(STRING_ELT(classExpr, 0)), "dbartsModel") != 0)
    Rf_error("'model' argument to dbarts_create not of class 'dbartsModel'");

  classExpr = Rf_getAttrib(dataExpr, R_ClassSymbol);
  if (std::strcmp(CHAR(STRING_ELT(classExpr, 0)), "dbartsData") != 0)
    Rf_error("'data' argument to dbarts_create not of class 'dbartsData'");
...
SEXP run(
  ...
  if (fit == NULL)
    Rf_error("dbarts_run called on NULL external pointer");
  ...
  numBurnIn = i_temp == NA_INTEGER ? fit->control.defaultNumBurnIn
                                   : static_cast<size_t>(i_temp);
  ...
  numThreads = i_temp == NA_INTEGER ? fit->control.numThreads
                                    : static_cast<size_t>(i_temp);
  ...
  numSamples = i_temp == NA_INTEGER ? fit->control.defaultNumSamples
                                    : static_cast<size_t>(i_temp);
  ...
  if (numSamples != 0 &&
      numTrainingSamples / numSamples != fit->data.numObservations)
  ...
  if (s_numTrainingSamples < 0 ||
      static_cast<size_t>(s_numTrainingSamples) != numTrainingSamples)
    Rf_error(
      "training sample array size cannot be represented by a signed integer on "
      "this architecture"
    );

  size_t numTestSamples = fit->data.numTestObservations * numSamples;
  if (numSamples != 0 &&
      numTestSamples / numSamples != fit->data.numTestObservations)
    Rf_error("test sample array size exceeds architecture's capacity");
  R_xlen_t s_numTestSamples = rc_asRLength(numTestSamples);
  if (s_numTestSamples < 0 ||
      static_cast<size_t>(s_numTestSamples) != numTestSamples)
    Rf_error(
      "test sample array size cannot be represented by a signed integer on "
      "this architecture"
    );
  ...
  SET_VECTOR_ELT(
    resultExpr,
    0,
    rc_newReal(rc_asRLength(bartResults->getNumSigmaSamples()))
  );
  SET_VECTOR_ELT(
    resultExpr,
    1,
    rc_newReal(rc_asRLength(bartResults->getNumTrainingSamples()))
  );
  if (fit->data.numTestObservations > 0)
    SET_VECTOR_ELT(
      resultExpr,
      2,
      rc_newReal(rc_asRLength(bartResults->getNumTestSamples()))
    );
  else
  ...
  SET_VECTOR_ELT(
    resultExpr,
    3,
    rc_newInteger(rc_asRLength(bartResults->getNumVariableCountSamples()))
  );
  ...
  if (fit->control.numChains > 1)
    rc_setDims(
      sigmaSamples,
      static_cast<int>(bartResults->numSamples),
      static_cast<int>(fit->control.numChains),
      -1
    );
  std::memcpy(
    REAL(sigmaSamples),
    const_cast<const double*>(bartResults->sigmaSamples),
    bartResults->getNumSigmaSamples() * sizeof(double)
  );
  ...
  if (fit->control.numChains <= 1)
    rc_setDims(
      trainingSamples,
      static_cast<int>(bartResults->numObservations),
      static_cast<int>(bartResults->numSamples),
      -1
    );
  else
  ...
  std::memcpy(
    REAL(trainingSamples),
    const_cast<const double*>(bartResults->trainingSamples),
    bartResults->getNumTrainingSamples() * sizeof(double)
  );
  ...
  if (fit->data.numTestObservations > 0) {
    ...
    if (fit->control.numChains <= 1)
      rc_setDims(
        testSamples,
        static_cast<int>(bartResults->numTestObservations),
        static_cast<int>(bartResults->numSamples),
        -1
      );
    else
    ...
    std::memcpy(
      REAL(testSamples),
      const_cast<const double*>(bartResults->testSamples),
      bartResults->getNumTestSamples() * sizeof(double)
    );
  }
  ...
  if (fit->control.numChains <= 1)
    rc_setDims(
      variableCountSamples,
      static_cast<int>(bartResults->numPredictors),
      static_cast<int>(bartResults->numSamples),
      -1
    );
  else
  ...
  for (size_t i = 0; i < length; ++i)
    variableCountStorage[i] =
      static_cast<int>(bartResults->variableCountSamples[i]);
  ...
  if (bartResults->kSamples != NULL) {
    ...
    std::memcpy(
      REAL(kSamples),
      const_cast<const double*>(bartResults->kSamples),
      bartResults->getNumSigmaSamples() * sizeof(double)
    );
  }
  ...
  rc_setNames(
    resultExpr,
    namesExpr = rc_newCharacter(bartResults->kSamples == NULL ? 4 : 5)
  );
...
SEXP sampleTreesFromPrior(SEXP fitExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_sampleTreesFromPrior called on NULL external pointer");
...
SEXP sampleNodeParametersFromPrior(SEXP fitExpr) {
  ...
  if (fit == NULL)
    Rf_error(
      "dbarts_sampleNodeParametersFromPrior called on NULL external pointer"
    );
...
SEXP setData(SEXP fitExpr, SEXP dataExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_setData called on NULL external pointer");

  SEXP classExpr = Rf_getAttrib(dataExpr, R_ClassSymbol);
  if (std::strcmp(CHAR(STRING_ELT(classExpr, 0)), "dbartsData") != 0)
    Rf_error("'data' argument to dbarts_setData not of class 'dbartsData'");
...
SEXP setControl(SEXP fitExpr, SEXP controlExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_setControl called on NULL external pointer");

  if (std::strcmp(
        CHAR(STRING_ELT(Rf_getAttrib(controlExpr, R_ClassSymbol), 0)),
        "dbartsControl"
      ) != 0)
    Rf_error(
      "'control' argument to dbarts_setControl not of class 'dbartsControl'"
    );
...
SEXP setModel(SEXP fitExpr, SEXP modelExpr) {
  ...
  if (std::strcmp(
        CHAR(STRING_ELT(Rf_getAttrib(modelExpr, R_ClassSymbol), 0)),
        "dbartsModel"
      ) != 0)
    Rf_error("'model' argument to dbarts_setModel not of class 'dbartsModel'");
...
SEXP predict(
  SEXP fitExpr,
  SEXP x_testExpr,
  SEXP offset_testExpr,
  SEXP numThreadsExpr
) {
  const BARTFit* fit = static_cast<const BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_predict called on NULL external pointer");

  const Control& control(fit->control);

  if (Rf_isNull(x_testExpr) || rc_isS4Null(x_testExpr))
    return R_NilValue;

  if (!Rf_isReal(x_testExpr))
    Rf_error("x.test must be of type real");

  rc_assertDimConstraints(
    x_testExpr,
    ...
    RC_NA,
    RC_VALUE | RC_EQ,
    static_cast<int>(fit->data.numPredictors),
    RC_END
  );
  ...
  if (!Rf_isNull(offset_testExpr)) {
    if (!Rf_isReal(offset_testExpr))
      Rf_error("offset.test must be of type real");
    if (rc_getLength(offset_testExpr) != 1 || !ISNA(REAL(offset_testExpr)[0])) {
      if (rc_getLength(offset_testExpr) != numTestObservations)
        Rf_error("length of offset.test must equal number of rows in x.test");
      testOffset = REAL(offset_testExpr);
    }
  }

  size_t numThreads = static_cast<size_t>(rc_getInt(
    numThreadsExpr,
    "number of threads",
    RC_LENGTH | RC_EQ,
    rc_asRLength(1),
  ...
  SEXP result = PROTECT(Rf_allocVector(
    REALSXP,
    numTestObservations * numSamples * control.numChains
  ));
  if (control.keepTrees) {
    if (fit->control.numChains <= 1)
      ...
      rc_setDims(
        ...
        static_cast<int>(numTestObservations),
        static_cast<int>(numSamples),
        static_cast<int>(control.numChains),
        -1
      );
  } else {
    if (fit->control.numChains > 1)
...
SEXP setResponse(SEXP fitExpr, SEXP y) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_setResponse called on NULL external pointer");

  rc_assertDoubleConstraints(
    y,
    "y",
    RC_LENGTH | RC_EQ,
    rc_asRLength(fit->data.numObservations),
    RC_END
  );
...
SEXP setOffset(SEXP fitExpr, SEXP offsetExpr, SEXP updateScaleExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_setOffset called on NULL external pointer");

  const double* offset = NULL;
  if (Rf_isReal(offsetExpr)) {
    offset = REAL(offsetExpr);
    if (rc_getLength(offsetExpr) != fit->data.numObservations)
      Rf_error("length of new offset does not match y");
  } else if (!Rf_isNull(offsetExpr) && !rc_isS4Null(offsetExpr)) {
    Rf_error("offset must be of type real or NULL");
  }

  bool updateScale = rc_getBool(
    updateScaleExpr,
    "updateScale",
    RC_DEFAULT | RC_VALUE,
    false,
...
SEXP setSigma(SEXP fitExpr, SEXP sigmaExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_setSigma called on NULL external pointer");

  const double* sigma = NULL;
  if (Rf_isReal(sigmaExpr)) {
    sigma = REAL(sigmaExpr);
    if (rc_getLength(sigmaExpr) != fit->control.numChains)
      Rf_error("length of new sigma does not match number of chains");
  } else {
...
SEXP setWeights(SEXP fitExpr, SEXP weights) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_setWeights called on NULL external pointer");
...
SEXP setPredictor(
  SEXP fitExpr,
  SEXP xExpr,
  SEXP forceUpdateExpr,
  SEXP updateCutPointsExpr
) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_setPredictor called on NULL external pointer");

  if (!Rf_isReal(xExpr))
    Rf_error("x must be of type real");

  bool forceUpdate =
    rc_getBool(forceUpdateExpr, "forceUpdate", RC_NA | RC_NO, RC_END);
  bool updateCutPoints =
    rc_getBool(updateCutPointsExpr, "updateCutPoints", RC_NA | RC_NO, RC_END);
  ...
  rc_assertDimConstraints(
    ...
    rc_asRLength(2),
    RC_VALUE | RC_EQ,
    static_cast<int>(fit->data.numObservations),
    RC_VALUE | RC_EQ,
    static_cast<int>(fit->data.numPredictors),
    RC_END
  );
...
SEXP updatePredictor(
  ...
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_updatePredictor called on NULL external pointer");
  ...
  if (Rf_isNull(colsExpr)) {
    ...
    if (!Rf_isInteger(colsExpr))
      Rf_error("columns must be of type integer");
    ...
    if (!Rf_isNull(dimsExpr)) {
      ...
      if (numDims != 1 && numDims != 2)
        Rf_error("x must be a vector or a matrix");
      if (numDims == 2)
        dims = INTEGER(dimsExpr);
    }

    if (rc_getLength(colsExpr) == 0)
      Rf_error("length of columns is 0");

    if (dims != NULL) {
      if (static_cast<size_t>(dims[0]) != fit->data.numObservations)
        Rf_error("number of rows of new x does not match y");
      if (static_cast<size_t>(dims[1]) != rc_getLength(colsExpr))
        Rf_error(
          "number of columns of new x does not match length of columns to "
          "replace"
        );
    } else {
      if (rc_getLength(xExpr) != fit->data.numObservations)
        Rf_error("length of new x does not match y");
    }
...
SEXP updatePredictorPerObservation(SEXP fitExpr, SEXP xExpr, SEXP colExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error(
      "dbarts_updatePredictorPerObservation called on NULL external pointer"
    );

  // RC_NA | RC_YES lets NA predictors through, as setPredictor does
  rc_assertDoubleConstraints(
    xExpr,
    "x",
    RC_LENGTH | RC_EQ,
  ...
  SEXP resultExpr = PROTECT(
    Rf_allocVector(LGLSXP, static_cast<R_xlen_t>(fit->data.numObservations))
  );
  int* result = LOGICAL(resultExpr);

  bool* installed = new bool[fit->data.numObservations];
  fit->updatePredictorPerObservation(REAL(xExpr), col, installed);
  for (size_t i = 0; i < fit->data.numObservations; ++i)
    result[i] = installed[i] ? TRUE : FALSE;
  delete[] installed;
...
SEXP updatePredictorPerObservationJointly(
  SEXP fitsExpr,
  SEXP xExpr,
  SEXP colsExpr
) {
  if (!Rf_isNewList(fitsExpr))
    Rf_error("fits must be a list of dbarts samplers");
  size_t numFits = rc_getLength(fitsExpr);
  if (numFits == 0)
    Rf_error("fits must contain at least one sampler");

  if (!Rf_isReal(xExpr))
    Rf_error("x must be of type real");
  rc_assertIntConstraints(
    colsExpr,
    "columns",
    RC_LENGTH | RC_EQ,
    rc_asRLength(numFits),
    RC_END
  );
  ...
  for (size_t j = 0; j < numFits; ++j) {
    BARTFit* fit = static_cast<BARTFit*>(
      R_ExternalPtrAddr(VECTOR_ELT(fitsExpr, static_cast<R_xlen_t>(j)))
    );
    if (fit == NULL) {
      misc_stackFree(cols);
      misc_stackFree(fits);
      Rf_error(
        "dbarts_updatePredictorPerObservationJointly called on NULL external "
        "pointer"
      );
    }
    fits[j] = fit;
    ...
    size_t col = static_cast<size_t>(colsInt[j] - 1);
    if (colsInt[j] < 1 || col >= fit->data.numPredictors) {
      int badCol = colsInt[j];
      misc_stackFree(cols);
      misc_stackFree(fits);
  ...
  SEXP resultExpr =
    PROTECT(Rf_allocVector(LGLSXP, static_cast<R_xlen_t>(numObservations)));
  int* result = LOGICAL(resultExpr);

  bool* installed = new bool[numObservations];
  BARTFit::updatePredictorPerObservationJointly(
    fits,
    numFits,
    REAL(xExpr),
    cols,
...
SEXP setCutPoints(SEXP fitExpr, SEXP cutPointsExpr, SEXP colsExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_setCutPoints called on NULL external pointer");

  if (!Rf_isNewList(cutPointsExpr))
    Rf_error("cutPoints must be of type list");
  ...
  if (rc_getLength(cutPointsExpr) != numCols)
    Rf_error(
      "length of cutPoints (" SIZE_T_SPECIFIER
      ") must equal length of columns (" SIZE_T_SPECIFIER ")",
      rc_getLength(cutPointsExpr),
      numCols
    );
  ...
  for (size_t i = 0; i < numCols; ++i) {
    SEXP cutPointsExpr_i = VECTOR_ELT(cutPointsExpr, static_cast<R_xlen_t>(i));
...
SEXP setTestPredictor(SEXP fitExpr, SEXP x_test) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_setTestPredictor called on NULL external pointer");
...
SEXP setTestOffset(SEXP fitExpr, SEXP offset_test) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_setTestOffset called on NULL external pointer");

  if (Rf_isNull(offset_test)) {
    fit->setTestOffset(NULL);
  } else {
    if (!Rf_isReal(offset_test))
      Rf_error("offset.test must be of type real");
    if (fit->data.numTestObservations != rc_getLength(offset_test))
      Rf_error("length of offset.test must equal number of rows in x.test");
...
SEXP setTestPredictorAndOffset(SEXP fitExpr, SEXP x_test, SEXP offset_test) {
  ...
  if (fit == NULL)
    Rf_error(
      "dbarts_setTestPredictorAndOffset called on NULL external pointer"
    );
  ...
  if (Rf_isNull(offset_test)) {
    ...
    if (rc_getLength(offset_test) == 1 && ISNA(REAL(offset_test)[0])) {
      ...
      if (rc_getLength(offset_test) != static_cast<size_t>(dims[0]))
        Rf_error("length of offset.test must equal number of rows in x.test");
      fit->setTestPredictorAndOffset(
        REAL(x_test),
        REAL(offset_test),
...
SEXP updateTestPredictor(SEXP fitExpr, SEXP x_test, SEXP colsExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_updateTestPredictor called on NULL external pointer");

  if (fit->data.x_test == NULL)
    Rf_error("test matrix must exist at object creation to be updated");
  ...
  if (dims != NULL) {
    if (static_cast<size_t>(dims[0]) != fit->data.numTestObservations)
      Rf_error("number of rows of new x does not match old x.test");
    if (static_cast<size_t>(dims[1]) != rc_getLength(colsExpr))
      Rf_error(
        "number of columns of new x does not match length of columns to replace"
      );
  } else {
    if (rc_getLength(x_test) != fit->data.numTestObservations)
      Rf_error("length of new x does not match old x.test");
  }
...
SEXP storeLatents(SEXP fitExpr, SEXP resultExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_storeLatents called on NULL external pointer");
  ...
  if (Rf_isNull(resultExpr)) {
    resultExpr =
      PROTECT(rc_newReal(fit->data.numObservations * fit->control.numChains));

    fit->storeLatents(REAL(resultExpr));

    UNPROTECT(1);
  } else {
    if (rc_getLength(resultExpr) <
        fit->data.numObservations * fit->control.numChains)
...
SEXP getSigmas(SEXP fitExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_getSigmas called on NULL external pointer");
  ...
  for (size_t chainNum = 0; chainNum < fit->control.numChains; ++chainNum) {
    result[chainNum] =
      fit->sharedScratch.dataScale.range * fit->state[chainNum].sigma;
  }
...
SEXP getSumsOfSquaredResiduals(SEXP fitExpr) {
  ...
  if (fit == NULL)
    Rf_error(
      "dbarts_getSumsOfSquaredResiduals called on NULL external pointer"
    );
...
SEXP startThreads(SEXP fitExpr, SEXP numThreadsExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_startThreads called on NULL external pointer");

  int i_temp = rc_getInt(
    numThreadsExpr,
    "number of threads",
    RC_LENGTH | RC_GEQ,
    rc_asRLength(1),
  ...
  size_t numThreads = i_temp == NA_INTEGER ? fit->control.numThreads
                                           : static_cast<size_t>(i_temp);
...
SEXP stopThreads(SEXP fitExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_stopThreads called on NULL external pointer");
...
SEXP createState(SEXP fitExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_createState called on NULL external pointer");
...
SEXP restoreState(SEXP fitExpr, SEXP stateExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_restoreState called on NULL external pointer");
...
SEXP storeState(SEXP fitExpr, SEXP stateExpr) {
  BARTFit* fit = static_cast<BARTFit*>(R_ExternalPtrAddr(fitExpr));
  if (fit == NULL)
    Rf_error("dbarts_storeState called on NULL external pointer");
...
SEXP printTrees(
  ...
  if (fitPtr == NULL)
    Rf_error("dbarts_printTrees called on NULL external pointer");
  BARTFit& fit(*fitPtr);
  ...
  size_t numChainIndices =
    Rf_isNull(chainIndicesExpr) ? numChains : rc_getLength(chainIndicesExpr);
  size_t numSampleIndices =
    Rf_isNull(sampleIndicesExpr) ? numSamples : rc_getLength(sampleIndicesExpr);
  size_t numTreeIndices =
    Rf_isNull(treeIndicesExpr) ? numTrees : rc_getLength(treeIndicesExpr);

  if (numChainIndices > numChains)
    Rf_error(
      SIZE_T_SPECIFIER " chains specified but only " SIZE_T_SPECIFIER
                       " in sampler",
      numChainIndices,
      numChains
    );
  if (numSampleIndices > numSamples)
    Rf_error(
      SIZE_T_SPECIFIER " samples specified but only " SIZE_T_SPECIFIER
                       " in sampler",
      numSampleIndices,
      numSamples
    );
  if (numTreeIndices > numTrees)
    Rf_error(
      SIZE_T_SPECIFIER " trees specified but only " SIZE_T_SPECIFIER
                       " in sampler",
      numTreeIndices,
      numTrees
    );
  ...
  if (Rf_isNull(chainIndicesExpr)) {
    ...
    for (size_t i = 0; i < numChainIndices; ++i)
      chainIndices[i] = static_cast<size_t>(i_chainIndices[i] - 1);
  }
  ...
  if (Rf_isNull(sampleIndicesExpr)) {
    ...
    for (size_t i = 0; i < numSampleIndices; ++i)
      sampleIndices[i] = static_cast<size_t>(i_sampleIndices[i] - 1);
  }
...
SEXP getTrees(
  ...
  if (numChains > 1) {
    SET_VECTOR_ELT(
      resultExpr,
      colNum,
      PROTECT(rc_newInteger(flattenedTrees.totalNumNodes))
    );
  ...
  SET_VECTOR_ELT(
    resultExpr,
    colNum,
    PROTECT(rc_newReal(flattenedTrees.totalNumNodes))
  );
