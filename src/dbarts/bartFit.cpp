...
namespace {
  ...
  // smallest integer cut index k such that value <= cutPoints[col][k]; equals
  // numCutsPerVariable[col] (out of range) when value is to the right of every
  // cut point
  xint_t findIntegerCut(const BARTFit& fit, std::size_t col, double value);
  void setXIntegerCutMap(BARTFit& fit);
  void
  setXIntegerCutMap(BARTFit& fit, const size_t* columns, size_t numColumns);
  void setXTestIntegerCutMap(
    const BARTFit& fit,
    const double* x_test,
  ...
  void setCutPointsFromQuantiles(
    BARTFit& fit,
    const double* x,
    uint32_t maxNumCuts,
  ...
  void sampleProbitLatentVariables(
    const BARTFit& fit,
    State& state,
    const double* fits,
    double* yRescaled
  ...
  void storeSamples(
    ...
    size_t chainNum,
    Results& results,
    const double* trainingSample,
    const double* testSample,
    double sigma,
    double k,
    const uint32_t* variableCounts,
    size_t simNum
  );
  void countVariableUses(
    const BARTFit& fit,
    const State& state,
    uint32_t* variableCounts
  );
  ...
  struct VectorFunctions {
    ...
    void (*subtractVectorsInPlace)(
      const double* restrict x,
      misc_size_t length,
      double* restrict y
    );
  };
  unsigned int getVectorFunctionsAndAlignment(
    const BARTFit& fit,
    size_t chainNum,
    VectorFunctions& vec
  );
...
namespace dbarts {
  ...
  void BARTFit::rebuildScratchFromState() {
    ...
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
      ...
      if (data.numTestObservations > 0) {
        ...
        misc_setVectorToConstant(
          chainScratch[chainNum].totalTestFits,
        ...
        for (size_t treeNum = 0; treeNum < control.numTrees; ++treeNum) {
          ...
          state[chainNum].trees[treeNum].setCurrentFitsFromParameters(
            *this,
            nodeParams,
            treeFits,
            testFits
          ...
          misc_addVectorsInPlace(
            const_cast<const double*>(testFits),
            data.numTestObservations,
            chainScratch[chainNum].totalTestFits
          );
  ...
  void BARTFit::setResponse(const double* newY) {
    if (!control.responseIsBinary) {
      ...
      for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum)
        sigmaUnscaled[chainNum] =
          state[chainNum].sigma * sharedScratch.dataScale.range;

      double priorUnscaled = model.sigmaSqPrior->getScale() *
                             sharedScratch.dataScale.range *
                             sharedScratch.dataScale.range;
      ...
      model.sigmaSqPrior->setScale(
        priorUnscaled /
        (sharedScratch.dataScale.range * sharedScratch.dataScale.range)
      );

      for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum)
        state[chainNum].sigma =
          sigmaUnscaled[chainNum] / sharedScratch.dataScale.range;
      ...
      for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum)
        sampleProbitLatentVariables(
          *this,
          state[chainNum],
          const_cast<const double*>(chainScratch[chainNum].totalFits),
          chainScratch[chainNum].probitLatents
        );
    }
  }
    ...
    if (control.responseIsBinary) {
      ...
      if (updateScale) {
        ...
        if (data.offset == newOffset && data.offset != NULL) {
          // offset update is in-place, don't have access to old value so
          // recreate
          std::memcpy(yRescaled, data.y, data.numObservations * sizeof(double));

          misc_subtractVectorsInPlace(
            data.offset,
            data.numObservations,
            yRescaled
          );
          ...
          // Y' = (y - min) / (max - min) - 0.5
          //    = y / (max - min) - min / (max - min) - 0.5 * (max - min) / (max
          //    - min) = y / (max - min) - 0.5 (max + min) / (max - min)
          /* misc_addScalarToVectorInPlace(   yRescaled, data.numObservations,
          -sharedScratch.dataScale.min);
          misc_scalarMultiplyVectorInPlace(yRescaled, data.numObservations, 1.0
          / sharedScratch.dataScale.range); misc_addScalarToVectorInPlace(
          yRescaled, data.numObservations, -0.5); */
          misc_scalarMultiplyVectorInPlace(
            yRescaled,
            data.numObservations,
            1.0 / sharedScratch.dataScale.range
          );
          misc_addScalarToVectorInPlace(
            yRescaled,
            data.numObservations,
            -0.5 * (sharedScratch.dataScale.max + sharedScratch.dataScale.min) /
              sharedScratch.dataScale.range
          );
        } else {
          // subtract old offset and add new one
          if (data.offset != NULL)
            misc_addVectorsInPlaceWithMultiplier(
              data.offset,
              data.numObservations,
              1.0 / sharedScratch.dataScale.range,
              yRescaled
  ...
  void BARTFit::setSigma(const double* newSigma) {
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum)
      state[chainNum].sigma =
        newSigma[chainNum] / sharedScratch.dataScale.range;
  }
...
void predictThreadFunction(std::size_t taskId, void* threadDataPtr) {
  PredictThreadData& threadData(
    *reinterpret_cast<PredictThreadData*>(threadDataPtr)
  );
  ...
  size_t numThreads =
    fit.threadManager != NULL ? misc_htm_getNumThreads(fit.threadManager) : 1;
  ...
  if (control.keepTrees) {
    ...
    for (size_t sampleNum = 0; sampleNum < fit.currentNumSamples; ++sampleNum) {
      ...
      for (size_t treeNum = 0; treeNum < control.numTrees; ++treeNum) {
        ...
        state.savedTrees[treeOffset]
          .getPredictions(fit, xt_test, numTestObservations, currTestFits);

        misc_addVectorsInPlace(
          const_cast<const double*>(currTestFits),
          numTestObservations,
          totalTestFits
        );
      }
      ...
      if (control.responseIsBinary) {
        std::memcpy(
          result_i,
          const_cast<const double*>(totalTestFits),
          numTestObservations * sizeof(double)
        );
      } else {
        misc_setVectorToConstant(
          result_i,
          numTestObservations,
          sharedScratch.dataScale.range * 0.5 + sharedScratch.dataScale.min
        );
        misc_addVectorsInPlaceWithMultiplier(
          const_cast<const double*>(totalTestFits),
          numTestObservations,
      ...
      if (testOffset != NULL)
        misc_addVectorsInPlace(testOffset, numTestObservations, result_i);
    }
  } else {
    ...
    if (testOffset != NULL)
      misc_addVectorsInPlace(testOffset, numTestObservations, result);
  }
...
namespace dbarts {
  ...
  void BARTFit::predict(
    ...
    misc_htm_manager_t originalThreadManager =
      const_cast<BARTFit*>(this)->threadManager;
    if (numThreads > 1 &&
        (threadManager == NULL ||
         misc_htm_getNumThreads(threadManager) != numThreads)) {
    ...
    if (numThreads <= 1) {
      ...
      PredictThreadData threadData =
        {this, {NULL}, numTestObservations, testOffset, 0, NULL};
      if (control.keepTrees)
        threadData.xt_test_d = xt_test_d;
      else
        threadData.xt_test_i = xt_test_i;

      for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
        threadData.chainNum = chainNum;
        threadData.result =
          result + chainNum * currentNumSamples * numTestObservations;
        predictThreadFunction(
      ...
      for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
        ...
        if (control.keepTrees)
          threadData[chainNum].xt_test_d = xt_test_d;
        else
          threadData[chainNum].xt_test_i = xt_test_i;
        ...
        threadData[chainNum].result =
          result + chainNum * currentNumSamples * numTestObservations;
        threadDataPtr[chainNum] =
          reinterpret_cast<void*>(&threadData[chainNum]);
      }
      misc_htm_runTopLevelTasks(
        threadManager,
        &predictThreadFunction,
        threadDataPtr,
        control.numChains
...
namespace {
  void forceUpdateTrees(BARTFit& fit) {
    ...
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
      ...
      for (size_t treeNum = 0; treeNum < control.numTrees; ++treeNum) {
        double* treeFits = const_cast<double*>(
          state[chainNum].treeFits + treeNum * state[chainNum].treeFitsStride
        );
        ...
        state[chainNum]
          .trees[treeNum]
          .setCurrentFitsFromParameters(fit, nodeParams, treeFits, NULL);
        misc_addVectorsInPlace(
          treeFits,
          data.numObservations,
          chainScratch[chainNum].totalFits
  ...
  bool updateTreesWithNewPredictor(BARTFit& fit) {
    ...
    for (size_t chainNum = 0; chainNum < control.numChains && allTreesAreValid;
         ++chainNum) {

      for (size_t treeNum = 0; treeNum < control.numTrees && allTreesAreValid;
           ++treeNum) {
        const double* treeFits =
          state[chainNum].treeFits + treeNum * state[chainNum].treeFitsStride;
    ...
    if (!allTreesAreValid)
      goto updateTreesWithNewPredictor_cleanup;

    // go back across bottoms and set predictions to those mus for obs now in
    // node
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
      for (size_t treeNum = 0; treeNum < control.numTrees; ++treeNum) {
        double* treeFits =
          state[chainNum].treeFits + treeNum * state[chainNum].treeFitsStride;
        const double* nodeParams =
          treeParams[treeNum + chainNum * control.numTrees];
  ...
  // Caches leaf counts to track observations moving in/out. While visiting
  // leaves to track counts, caches the leaf indices for each tree x
  // observation so it is on hand for testing what would happen if the
  // predictor changes.
  //
  // Routes each observation through the split structure rather than by reading
  // the node partitions, so it is correct regardless of how the trees were
  // last left.
  //
  // Handles multiple chains by combining their trees all into one.
  struct InPlacePredictorUpdater {
    ...
    xint_t* newXInt;              // just the vector, not the array
    size_t* observationLeafIndex; // totalNumTrees x numObservations
    size_t** leafCounts;          // totalNumTrees x numLeavesTree_t
    xint_t* x_temp;

    // these handle cache between observationWouldRemainValidIfPredictorUpdated
    // and commitToPredictorUpdateForObservation; one value of each for each
    // tree
    bool* pendingMove;
    size_t* pendingOldLeaf;
    size_t* pendingNewLeaf;
    ...
    InPlacePredictorUpdater(
      ...
      size_t column
        ...
        : fit(fit), column(column), newPredictor(newPredictor),
      ...
      const xint_t* xInt = fit.sharedScratch.x;

      newXInt = new xint_t[numObservations];
      for (size_t i = 0; i < numObservations; ++i)
        newXInt[i] = findIntegerCut(fit, column, newPredictor[i]);

      observationLeafIndex = new size_t[totalNumTrees * numObservations];
      leafCounts = new size_t*[totalNumTrees];
      x_temp = new xint_t[numPredictors];
      ...
      for (size_t chainNum = 0; chainNum < fit.control.numChains; ++chainNum) {
        for (size_t treeNum = 0; treeNum < fit.control.numTrees; ++treeNum) {
          ...
          size_t numBottomNodes =
            top.getAndEnumerateBottomVector()
              .size(); // sets enumerationIndex on each leaf
          ...
          for (size_t i = 0; i < numObservations; ++i) {
            for (size_t j = 0; j < numPredictors; ++j)
              x_temp[j] = xInt[i + j * numObservations];
            size_t leafIndex =
              top.findBottomNode(fit, x_temp)->enumerationIndex;
            observationLeafIndex[t * numObservations + i] = leafIndex;
            ++leafCounts[t][leafIndex];
          }
        }
      }
    ...
    ~InPlacePredictorUpdater() {
      ...
      delete[] leafCounts;
      delete[] observationLeafIndex;
      delete[] newXInt;
    }

    // Stages the per-tree leaf moves so a subsequent
    // commitToPredictorUpdateForObservation(i) can apply them;
    bool observationWouldRemainValidIfPredictorUpdated(size_t i) {
      const xint_t* xInt = fit.sharedScratch.x;
      for (size_t j = 0; j < numPredictors; ++j)
        x_temp[j] = xInt[i + j * numObservations];
      x_temp[column] = newXInt[i];
      ...
      for (size_t t = 0; t < totalNumTrees && valid; ++t) {
        ...
        size_t oldLeaf = observationLeafIndex[t * numObservations + i];
        size_t newLeaf = top.findBottomNode(fit, x_temp)->enumerationIndex;

        if (newLeaf == oldLeaf) {
          pendingMove[t] = false;
        } else if (leafCounts[t][oldLeaf] == 1) {
          valid = false; // i is the last occupant of its old leaf; moving it
                         // would empty the leaf
        } else {
          pendingMove[t] = true;
    ...
    void commitToPredictorUpdateForObservation(size_t i) {
      double* x = const_cast<double*>(fit.data.x);
      xint_t* xInt = const_cast<xint_t*>(fit.sharedScratch.x);
      x[i + column * numObservations] = newPredictor[i];
      xInt[i + column * numObservations] = newXInt[i];
      for (size_t t = 0; t < totalNumTrees; ++t) {
        if (pendingMove[t]) {
          --leafCounts[t][pendingOldLeaf[t]];
          ++leafCounts[t][pendingNewLeaf[t]];
          // don't actually need this, since we don't ever ready this value
          // observationLeafIndex[t * numObservations + i] = pendingNewLeaf[t];
        }
      }
    }
    ...
    if (!forceUpdate && updateCutPoints) {
      ...
      for (size_t j = 0; j < data.numPredictors; ++j) {
        ...
        std::memcpy(
          oldCutPoints[j],
          cutPoints[j],
          numCutsPerVariable[j] * sizeof(double)
        );
      }
    }
...
#  pragma GCC diagnostic pop
    ...
    if (forceUpdate) {
      ...
      if (data.numTestObservations > 0 && updateCutPoints)
        setXTestIntegerCutMap(
          *this,
          data.x_test,
          data.numTestObservations,
          const_cast<xint_t*>(sharedScratch.xt_test)
        );
      return true;
    }
    ...
    if (updateCutPoints) {
      for (size_t j = data.numPredictors; j > 0; --j)
        delete[] oldCutPoints[j - 1];
      delete[] oldCutPoints;
    }
  ...
  bool BARTFit::updatePredictor(
    ...
    if (!forceUpdate) {
      ...
      for (size_t j = 0; j < numColumns; ++j)
        std::memcpy(
          oldPredictor + j * data.numObservations,
          data.x + columns[j] * data.numObservations,
      ...
      if (updateCutPoints) {
        ...
        for (size_t j = 0; j < numColumns; ++j) {
          ...
          std::memcpy(
            oldCutPoints[j],
            cutPoints[columns[j]],
            numCutsPerVariable[columns[j]] * sizeof(double)
          );
        }
      }
    }
    ...
    for (size_t j = 0; j < numColumns; ++j) {
      ...
      std::memcpy(
        x + col * data.numObservations,
        newPredictor + j * data.numObservations,
        data.numObservations * sizeof(double)
      );
    }
    ...
    if (!treesAreValid) {
      for (size_t j = 0; j < numColumns; ++j) {
        ...
        if (updateCutPoints)
          std::memcpy(
            const_cast<double**>(cutPoints)[col],
            oldCutPoints[j],
            numCutsPerVariable[col] * sizeof(double)
          );
      }
      setXIntegerCutMap(*this, columns, numColumns);
  ...
  void BARTFit::updatePredictorPerObservation(
  ...
  void BARTFit::updatePredictorPerObservationJointly(
    ...
    for (size_t k = 0; k < numFits; ++k)
      updaters[k] =
        new InPlacePredictorUpdater(*fits[k], newColumn, columns[k]);

    size_t* scanOrder = new size_t[numObservations];
    ext_rng_drawPermutation(fits[0]->state[0].rng, scanOrder, numObservations);

    for (size_t i = 0; i < numObservations; ++i) {
      size_t j = scanOrder[i];
      bool valid = true;
      // breaks loop when !valid
      for (size_t k = 0; k < numFits && valid; ++k)
        valid = updaters[k]->observationWouldRemainValidIfPredictorUpdated(j);
      installed[j] = valid;
      if (valid)
        for (size_t k = 0; k < numFits; ++k)
  ...
  void BARTFit::setCutPoints(
    ...
    for (size_t j = 0; j < numColumns; ++j) {
      if (numCutPoints[j] >
          static_cast<uint32_t>(((1 >> sizeof(xint_t)) - 1))) {
        ext_throwError(
          "number of cut points for column %lu exceeds the max allowable; "
...
namespace {
  void updateTestFitsWithNewPredictor(
    const BARTFit& fit,
    ...
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
      ...
      for (size_t treeNum = 0; treeNum < control.numTrees; ++treeNum) {
        ...
        state[chainNum]
          .trees[treeNum]
          .setCurrentFitsFromParameters(fit, nodeParams, NULL, currTestFits);
...
namespace dbarts {
  ...
  // an invalid pointer address for testOffset is the object itself; when
  // invalid, it is not updated
  void BARTFit::setTestPredictorAndOffset(
    ...
    if (numTestObservations == 0 || x_test == NULL) {
      if (sharedScratch.xt_test != NULL) {
        delete[] sharedScratch.xt_test;
        sharedScratch.xt_test = NULL;
      }
      for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum)
        if (chainScratch[chainNum].totalTestFits != NULL) {
      ...
      if (numTestObservations != data.numTestObservations) {
        ...
        sharedScratch.xt_test =
          new xint_t[data.numTestObservations * data.numPredictors];
        for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum)
          chainScratch[chainNum].totalTestFits =
            new double[data.numTestObservations];
      }
      ...
      // misc_transposeMatrix(data.x_test, data.numTestObservations,
      // data.numPredictors, const_cast<double*>(sharedScratch.xt_test));

      if (testOffset != INVALID_ADDRESS)
...
#undef INVALID_ADDRESS
  ...
  void
  BARTFit::updateTestPredictor(const double* newTestPredictor, size_t column) {
    updateTestPredictors(newTestPredictor, &column, 1);
  }

  void BARTFit::updateTestPredictors(
    const double* newTestPredictor,
    const size_t* columns,
    size_t numColumns
  ) {
    ...
    for (size_t j = 0; j < numColumns; ++j) {
      size_t col = columns[j];
      std::memcpy(
        x_test + col * data.numTestObservations,
        newTestPredictor + j * data.numTestObservations,
        data.numTestObservations * sizeof(double)
      );

      for (size_t i = 0; i < data.numTestObservations; ++i)
        xt_test[i * data.numPredictors + col] = findIntegerCut(
          *this,
          col,
          x_test[i + col * data.numTestObservations]
        );
    }
  ...
  void BARTFit::storeLatents(double* target) const {
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum)
      std::memcpy(
        target + chainNum * data.numObservations,
        chainScratch[chainNum].probitLatents,
        data.numObservations * sizeof(double)
      );
  }
   ...
   *   cutPoints and numCutsPerVariable - compute from data.maxNumCuts and new
   * data
   *
   *   state.trees         - we have to go through these and prune any
   * now-invalid end nodes state.treeIndices   - resize and assign into trees
  ...
  void BARTFit::setData(const Data& newData) {
    ...
    unsigned int* oldTreeFitsAlignments =
      misc_stackAllocate(control.numChains, unsigned int);
    ...
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
      ...
      if (oldNumObservations != data.numObservations) {
        ...
        if (control.responseIsBinary) {
          ...
          chainScratch[chainNum].probitLatents =
            new double[data.numObservations];
        }

        state[chainNum].treeIndices =
          new size_t[data.numObservations * control.numTrees];
        ...
        if (state[chainNum].treeFitsAlignment == 0) {
          ...
          state[chainNum].treeFits =
            new double[state[chainNum].treeFitsStride * control.numTrees];
        } else {
          size_t remainder =
            data.numObservations %
            (state[chainNum].treeFitsAlignment / sizeof(double));
          state[chainNum].treeFitsStride =
            data.numObservations +
            (remainder == 0
               ? 0
               : (state[chainNum].treeFitsAlignment / sizeof(double) -
          ...
          if (misc_alignedAllocate(
                ...
                control.numTrees * state[chainNum].treeFitsStride *
                  sizeof(double)
              ) != 0)
    ...
    const double** oldCutPoints =
      misc_stackAllocate(data.numPredictors, const double*);
    for (size_t j = 0; j < data.numPredictors; ++j) {
      oldCutPoints[j] = cutPoints[j];
      // next assignments 'reset' the variables, so setCutPoints() ignores old
      // values
    ...
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
      for (size_t treeNum = 0; treeNum < control.numTrees; ++treeNum) {
        const double* oldTreeFits_i =
          oldTreeFits[chainNum] + treeNum * oldTreeFitsStrides[chainNum];

        // Use the bottom node enumeration to determine which fits to use.
        // The bottom nodes themselves keep an enumeration number, so that when
        // prunned we can still find the right one.
        state[chainNum].trees[treeNum].top.enumerateBottomNodes();

        // this allocates memory; predictions are of length equal to the number
        // of bottom nodes
        double* nodeParams =
          state[chainNum].trees[treeNum].recoverParametersFromFits(
            *this,
        ...
        // the mapping can end up with some end-nodes that no longer exist,
        // handles that internally
        state[chainNum].trees[treeNum].mapOldCutPointsOntoNew(
          *this,
          oldCutPoints,
          nodeParams
        );
        ...
        if (oldNumObservations != data.numObservations) {
          state[chainNum].trees[treeNum].top.observationIndices =
            state[chainNum].treeIndices + treeNum * data.numObservations;
          state[chainNum].trees[treeNum].top.numObservations =
            data.numObservations;
        }
  ...
  void BARTFit::setControl(const Control& newControl) {
    ...
    if (oldControl.numChains == newControl.numChains) {
      ...
      state = static_cast<State*>(
        ::operator new(newControl.numChains * sizeof(State))
      );
      ...
      for (size_t chainNum = resizeEnd; chainNum < newControl.numChains;
           ++chainNum) {
        new (state + chainNum) State(newControl, data);
        stateResized = true;
      }

      for (size_t chainNum = oldControl.numChains; chainNum > resizeEnd;
           --chainNum)
        oldState[chainNum - 1].invalidate(
          oldControl.numTrees,
          currentNumSamples
        );
    ...
    if (oldControl.numTrees != newControl.numTrees) {
      ...
      nodePrior.setScale(
        model.nodeScale / std::sqrt(static_cast<double>(newControl.numTrees))
      );
    }
    ...
    if (old_rng_algorithm != oldControl.rng_algorithm ||
        old_rng_standardNormal != oldControl.rng_standardNormal)
      destroyRNG(*this);

    control = newControl;
    // looks redundant, but the assignment taking place between impacts the
    // destroy/create
    if (old_rng_algorithm != oldControl.rng_algorithm ||
        old_rng_standardNormal != oldControl.rng_standardNormal)
      createRNG(*this);
  ...
  void BARTFit::setModel(const Model& newModel) {
    ...
    if (model.sigmaSqPrior->isFixed)
      setSigma(static_cast<FixedPrior*>(model.sigmaSqPrior)->getScale());
    if (model.kPrior->isFixed)
      setK(static_cast<FixedHyperprior*>(model.kPrior)->getK());
  }

  void BARTFit::printTrees(
...
namespace dbarts {
  FlattenedTrees::FlattenedTrees(size_t totalNumNodes)
      : totalNumNodes(totalNumNodes), chainNumber(NULL), sampleNumber(NULL),
        treeNumber(NULL), numObservations(NULL), variable(NULL), value(NULL) {
  ...
  BARTFit::BARTFit(Control control, Model model, Data data)
      : control(control), model(model), data(data), sharedScratch(),
        chainScratch(NULL), state(NULL), runningTime(0.0),
        currentNumSamples(control.defaultNumSamples), currentSampleNum(0),
        threadManager(NULL) {
    allocateMemory(*this);
  ...
  BARTFit::~BARTFit() {
    ...
    delete[] numCutsPerVariable;
    numCutsPerVariable = NULL;
    if (cutPoints != NULL) {
      for (size_t i = 0; i < data.numPredictors; ++i)
        delete[] cutPoints[i];
    }
  ...
  Results* BARTFit::runSampler() {
    ...
    Results* resultsPointer = new Results(
      data.numObservations,
      data.numPredictors,
      data.numTestObservations,
    ...
    size_t numBurnIn =
      control.defaultNumBurnIn -
      (control.defaultNumSamples == 0 && control.defaultNumBurnIn > 0 ? 1 : 0);
  ...
  Results*
  BARTFit::runSampler(size_t numBurnIn, size_t numThreads, size_t numSamples) {
    Results* resultsPointer = new Results(
      data.numObservations,
      data.numPredictors,
      data.numTestObservations,
  ...
  void BARTFit::sampleTreesFromPrior() {
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
      for (size_t treeNum = 0; treeNum < control.numTrees; ++treeNum) {
        // sample tree from prior should probably be re-written to be consistent
        // with the observed (and conditioned on) data
        state[chainNum].trees[treeNum].sampleStructureFromPrior(
          *this,
          state[chainNum].rng
        );
  ...
  void BARTFit::sampleNodeParametersFromPrior() {
    double* testFits = data.numTestObservations > 0
                         ? new double[data.numTestObservations]
                         : NULL;
    ...
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
      ...
      for (size_t treeNum = 0; treeNum < control.numTrees; ++treeNum) {
        ...
        state[chainNum]
          .trees[treeNum]
          .sampleParametersFromPrior(*this, chainNum, treeFits, testFits);
...
void samplerThreadFunction(std::size_t taskId, void* threadDataPtr) {
  SamplerThreadData* threadData(
    reinterpret_cast<SamplerThreadData*>(threadDataPtr)
  );
  ...
  double* currFits;
  if (alignment != 0) {
    misc_alignedAllocate(
      reinterpret_cast<void**>(&currFits),
      alignment,
      data.numObservations * sizeof(double)
    );
  } else {
  ...
  double* currTestFits =
    data.numTestObservations > 0 ? new double[data.numTestObservations] : NULL;

  uint32_t* variableCounts = misc_stackAllocate(data.numPredictors, uint32_t);

  size_t totalNumIterations =
    (numBurnIn + numSamples) * control.treeThinningRate;
  ...
  if (control.responseIsBinary) {
    ...
    std::memcpy(
      y,
      const_cast<const double*>(chainScratch.probitLatents),
      data.numObservations * sizeof(double)
    );
    if (data.offset != NULL)
  ...
  for (size_t k = 0; k < totalNumIterations; ++k) {
    ...
    if (control.verbose && !isThinningIteration &&
        (majorIterationNum + 1) % control.printEvery == 0) {
      if (control.numChains > 1)
        misc_htm_printf(
          fit.threadManager,
          "[" SIZE_T_SPECIFIER "] iteration: " SIZE_T_SPECIFIER
          " (of " SIZE_T_SPECIFIER ")\n",
          chainNum + 1,
          k + 1,
          totalNumIterations
        ...
        ext_printf(
          "iteration: " SIZE_T_SPECIFIER " (of " SIZE_T_SPECIFIER ")\n",
          k + 1,
          totalNumIterations
        );
    }
    ...
    for (size_t treeNum = 0; treeNum < control.numTrees; ++treeNum) {
      ...
      // is residual from every *other* tree, so what is left for this tree to
      // do
      std::memcpy(chainScratch.treeY, y, data.numObservations * sizeof(double));
      vec.subtractVectorsInPlace(
        const_cast<const double*>(chainScratch.totalFits),
        data.numObservations,
        chainScratch.treeY
      );
      vec.addVectorsInPlace(
        const_cast<const double*>(oldTreeFits),
        data.numObservations,
        chainScratch.treeY
      );
      ...
      state.trees[treeNum].sampleParametersAndSetFits(
        fit,
        chainNum,
        currFits,
        isThinningIteration ? NULL : currTestFits
      );
      ...
      vec.subtractVectorsInPlace(
        const_cast<const double*>(oldTreeFits),
        data.numObservations,
        chainScratch.totalFits
      );
      vec.addVectorsInPlace(
        const_cast<const double*>(currFits),
        data.numObservations,
        chainScratch.totalFits
      );
    ...
    if (control.keepTrees & !isBurningIn && !isThinningIteration) {
      size_t treeSampleNum =
        (fit.currentSampleNum + resultSampleNum) % fit.currentNumSamples;

      for (size_t treeNum = 0; treeNum < control.numTrees; ++treeNum)
        state.savedTrees[treeNum + treeSampleNum * control.numTrees]
          .copyStructureFrom(
            fit,
            state.trees[treeNum],
            const_cast<const double*>(
              state.treeFits + treeNum * state.treeFitsStride
            )
          );
    }
    ...
    if (!model.sigmaSqPrior->isFixed)
      state.sigma = std::sqrt(
        model.sigmaSqPrior
          ->drawFromPosterior(fit, chainNum, y, chainScratch.totalFits)
      );
    if (!model.kPrior->isFixed)
    ...
    if (!isThinningIteration) {
      ...
      if (control.callback != NULL) {
        ...
        control.callback(
          control.callbackData,
          fit,
          isBurningIn,
          results.trainingSamples +
            (resultSampleNum + chainStride) * data.numObservations,
          results.testSamples +
            (resultSampleNum + chainStride) * data.numTestObservations,
          results.sigmaSamples[resultSampleNum + chainStride],
          !model.kPrior->isFixed
            ? results.kSamples + resultSampleNum + chainStride
            : NULL
        );
      }
    }
...
namespace dbarts {
  ...
  void BARTFit::runSampler(
    ...
    if (numThreads <= 1) {
      ...
      if (control.verbose) {
        ...
        misc_htm_runTopLevelTasksWithOutput(
          threadManager,
          &samplerThreadFunction,
          threadDataPtr,
          control.numChains,
    ...
    if (control.keepTrees)
      currentSampleNum =
        (currentSampleNum + resultsPointer->numSamples) % currentNumSamples;
  ...
  void BARTFit::printInitialSummary() const {
    ...
    ext_printf(
      "number of chains: " SIZE_T_SPECIFIER
      ", default number of threads " SIZE_T_SPECIFIER "\n",
      control.numChains,
      control.numThreads
    );
    ...
    ext_printf(
      "\tpower and base for tree prior: %f %f\n",
      treePrior->power,
      treePrior->base
    );
    if (treePrior->splitProbabilities != NULL) {
      ext_printf(
        "\ttree split probabilities: %f",
    ...
    ext_printf(
      "\tuse quantiles for rule cut points: %s\n",
    ...
    ext_printf("data:\n");
    ext_printf(
      "\tnumber of training observations: " SIZE_T_SPECIFIER "\n",
      data.numObservations
    );
    ext_printf(
      "\tnumber of test observations: " SIZE_T_SPECIFIER "\n",
      data.numTestObservations
    );
    ext_printf(
      "\tnumber of explanatory variables: " SIZE_T_SPECIFIER "\n",
      data.numPredictors
    );
    if (!control.responseIsBinary)
      ext_printf(
        "\tinit sigma: %f, curr sigma: %f\n",
        data.sigmaEstimate,
        state[0].sigma * sharedScratch.dataScale.range
      );
    if (data.weights != NULL)
      ext_printf("\tusing observation weights\n");
    ext_printf("\n");
    ...
    if (control.printCutoffs > 0) {
      ...
      for (size_t j = 0; j < data.numPredictors; ++j) {
        ...
        size_t k;
        for (k = 0;
             k < numCutsPerVariable[j] - 1 && k < control.printCutoffs - 1;
             ++k) {
    ...
    if (data.offset != NULL ||
      ...
      if (data.offset != NULL) {
        ...
        for (size_t i = 1;
             i < (5 < data.numObservations ? 5 : data.numObservations);
             ++i)
...
namespace {
  ...
  unsigned int getVectorFunctionsAndAlignment(
    ...
    if (fit.chainScratch[chainNum].alignment ==
          fit.state[chainNum].treeFitsAlignment &&
  ...
  void printTerminalSummary(const BARTFit& fit) {
    ...
    for (size_t chainNum = 0; chainNum < fit.control.numChains; ++chainNum) {
      ...
      for (size_t treeNum = 0; treeNum < fit.control.numTrees; ++treeNum) {
        ext_printf(
          SIZE_T_SPECIFIER " ",
          fit.state[chainNum].trees[treeNum].getNumBottomNodes()
        );
        if ((linePrintCount++ + 1) % 20 == 0)
          ext_printf("\n");
      }
    ...
    uint32_t* variableCounts =
      misc_stackAllocate(fit.data.numPredictors, uint32_t);
  ...
  void allocateMemory(BARTFit& fit) {
    ...
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
      ...
      double* y = control.responseIsBinary
                    ? chainScratch[chainNum].probitLatents
                    : const_cast<double*>(sharedScratch.yRescaled);

      std::memcpy(
        chainScratch[chainNum].treeY,
        const_cast<const double*>(y),
        data.numObservations * sizeof(double)
      );
      // for (size_t i = 0; i < data.numObservations; ++i)
      // chainScratch[chainNum].treeY[i] = y[i];

      chainScratch[chainNum].totalTestFits =
        data.numTestObservations > 0 ? new double[data.numTestObservations]
                                     : NULL;
    ...
    // states
    fit.state =
      static_cast<State*>(::operator new(control.numChains * sizeof(State)));
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum)
  ...
  void setPrior(BARTFit& fit) {
    ...
    if (control.responseIsBinary) {
      ...
      for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
        state[chainNum].sigma =
          data.sigmaEstimate / sharedScratch.dataScale.range;
        state[chainNum].k =
          model.kPrior->isFixed
            ? static_cast<FixedHyperprior*>(model.kPrior)->getK()
            : 2;
      ...
      model.sigmaSqPrior->setScale(
        state[0].sigma * state[0].sigma * model.sigmaSqPrior->getScale()
      );
    }
  }
  ...
  void setInitialCutPoints(BARTFit& fit) {
    ...
    uint32_t* numCutsPerVariable =
      const_cast<uint32_t*>(fit.numCutsPerVariable);
  ...
  xint_t findIntegerCut(const BARTFit& fit, size_t col, double value) {
    // min cut such that value <= c_col,k; can possibly be out of range if value
    // is to the far right
    xint_t k = 0;
    while (k < fit.numCutsPerVariable[col] && value > fit.cutPoints[col][k])
      ++k;
    return k;
  }
  ...
  void setXIntegerCutMap(BARTFit& fit) {
    ...
    for (size_t j = 0; j < data.numPredictors; ++j)
      for (size_t i = 0; i < data.numObservations; ++i)
        x[i + j * data.numObservations] =
          findIntegerCut(fit, j, data.x[i + j * data.numObservations]);
  }
  ...
  setXIntegerCutMap(BARTFit& fit, const size_t* columns, size_t numColumns) {
    ...
    for (size_t j = 0; j < numColumns; ++j) {
      size_t col = columns[j];
      for (size_t i = 0; i < data.numObservations; ++i)
        x[i + col * data.numObservations] =
          findIntegerCut(fit, col, data.x[i + col * data.numObservations]);
    }
  }
  ...
  void setXTestIntegerCutMap(
    ...
    for (size_t j = 0; j < data.numPredictors; ++j)
      for (size_t i = 0; i < numTestObservations; ++i)
        xt_test[i * data.numPredictors + j] =
          findIntegerCut(fit, j, x_test[i + j * numTestObservations]);
  }
    ...
    for (size_t j = 0; j < numColumns; ++j) {
      ...
      for (size_t i = 0; i < numTestObservations; ++i)
        xt_test[i * data.numPredictors + col] =
          findIntegerCut(fit, col, x_test[i + col * numTestObservations]);
    }
  }
  ...
  void setCutPoints(BARTFit& fit, const size_t* columns, size_t numColumns) {
    ...
    if (control.useQuantiles) {
      if (data.maxNumCuts == NULL)
        ext_throwError("Num cuts cannot be NULL if useQuantiles is true.");

      // sets are inherently sorted, should be a binary tree back there
      // somewhere
      std::set<double> uniqueElements;
      ...
      for (size_t j = 0; j < numColumns; ++j) {
        ...
        setCutPointsFromQuantiles(
          fit,
          data.x + col * data.numObservations,
          data.maxNumCuts[col],
          numCutsPerVariable[col],
          cutPoints[col],
          uniqueElements,
          sortedElements
        );
  ...
  void setCutPointsFromQuantiles(
    ...
    if (numCutsPerVariable != static_cast<uint32_t>(-1)) {
      if (numCuts < numCutsPerVariable)
        ext_throwError(
          "number of induced cut points in new predictor less than previous: "
          "old splits would be invalid"
        );
      if (numCuts > numCutsPerVariable)
        ext_issueWarning(
          "number of induced cut points in new predictor greater than "
          "previous: ignoring extra quantiles"
        );
    } else {
  ...
  void setCutPointsUniformly(
    ...
    for (size_t k = 0; k < numCutsPerVariable; ++k)
      cutPoints[k] = xMin + (static_cast<double>(k + 1)) * xIncrement;
  }
  ...
  void createRNG(BARTFit& fit) {
    ...
    // if only one chain or one thread, can use environment's rng since
    // randomization calls will all be serial
    size_t numThreads = fit.threadManager != NULL
                          ? misc_htm_getNumThreads(fit.threadManager)
                          : control.numThreads;
    ...
    ext_rng_algorithm_t rng_algorithm =
      static_cast<ext_rng_algorithm_t>(control.rng_algorithm);
    ext_rng_standardNormal_t rng_standardNormal =
      static_cast<ext_rng_standardNormal_t>(control.rng_standardNormal);
    ...
    for (/* */; chainNum < control.numChains; ++chainNum) {
      ...
      if (rng_algorithm == EXT_RNG_ALGORITHM_INVALID) {
        if ((state[chainNum].rng = ext_rng_createDefault(useNativeRNG)) ==
            NULL) {
          errorMessage = "could not allocate rng";
          goto createRNG_cleanup;
        }
      } else {
        if ((state[chainNum].rng = ext_rng_create(rng_algorithm, NULL)) ==
            NULL) {
          errorMessage = "could not allocate rng";
          goto createRNG_cleanup;
        }
      ...
      if (rng_standardNormal != EXT_RNG_STANDARD_NORMAL_INVALID &&
          ...
          ext_rng_setStandardNormalAlgorithm(
            state[chainNum].rng,
            rng_standardNormal,
            NULL
          ) != 0) {
      ...
      if (useNativeRNG) {
        // If we are using the native generator, we only have to call set seed
        // once as we are running sequentially and the (single-threaded)
        // built-in generator will be used for everything.
        ...
        if (rng_algorithm != EXT_RNG_ALGORITHM_USER_UNIFORM &&
          ...
          uint_least32_t chainSeed = static_cast<uint_least32_t>(
            ext_rng_simulateUnsignedIntegerUniformInRange(
              seedGenerator,
              0,
              static_cast<uint_least32_t>(-1)
            )
          );
          if (ext_rng_setSeed(state[chainNum].rng, chainSeed) != 0) {
            errorMessage = "could not seed rng";
            goto createRNG_cleanup;
          }
          // if (ext_rng_setSeed(state[chainNum].rng,
          // static_cast<uint_least32_t>(ext_rng_simulateUnsignedIntegerUniformInRange(seedGenerator,
          // 0, static_cast<uint_least32_t>(-1)))) != 0) { errorMessage = "could
          // not seed rng"; goto createRNG_cleanup; }
        } else {
        ...
        if (chainNum > 0) {
          ...
          for (numSeedResets = 0; numSeedResets < static_cast<size_t>(-1);
               ++numSeedResets) {
            if (!ext_rng_seedsAreEqual(
                  state[chainNum].rng,
                  state[chainNum - 1].rng
                ))
              break;
          ...
          if (numSeedResets == static_cast<size_t>(-1))
            for (numSeedResets = 0; numSeedResets < static_cast<size_t>(-1);
                 ++numSeedResets) {
              if (!ext_rng_seedsAreEqual(
                    state[chainNum].rng,
          ...
          if (numSeedResets == static_cast<size_t>(-1)) {
            errorMessage = "could not obtain unique seed";
            goto createRNG_cleanup;
          }
        }
      }
...
namespace dbarts {
    ...
    for (size_t chainNum = 0; chainNum < control.numChains; ++chainNum) {
      ...
      if (normalState != NULL && normalState[chainNum] != NULL)
        ext_rng_setStandardNormalAlgorithm(
          state[chainNum].rng,
          static_cast<ext_rng_standardNormal_t>(control.rng_standardNormal),
          normalState[chainNum]
        );
    }
  }

  size_t BARTFit::startThreads() { return startThreads(control.numThreads); }
  ...
  size_t BARTFit::startThreads(size_t numThreads) {
    ...
    if (numThreads > 1 && misc_htm_create(&threadManager, numThreads) != 0) {
      ext_issueWarning(
        "Unable to create thread manager and start threads; reverting to "
        "single thread"
      );
      numThreads = 1;
    }
    ...
    if (numThreads > 1 && control.numChains > 1 &&
        ext_rng_usesNativeRNG(state[0].rng)) {
...
namespace {
  ...
  void destroyRNG(BARTFit& fit) {
    if (fit.control.rng_algorithm == RNG_ALGORITHM_USER_POINTER)
      return;
  ...
  void initializeLatents(BARTFit& fit) {
    ...
    // shouldn't be used, but will leave at reasonable values; if anyone cares,
    // should look at offset var for min/max/range
  ...
  void initializeLatents(BARTFit& fit, size_t chainNum) {
    ...
    // z = 2.0 * y - 1.0 - offset; so -1 if y == 0 and 1 if y == 1 when offset
    // == 0
#ifndef MATCH_BAYES_TREE
    misc_setVectorToConstant(z, data.numObservations, -1.0);
    // if (data.offset != NULL) misc_subtractVectorsInPlace(data.offset,
    // data.numObservations, z);
    ...
    if (data.offset != NULL)
      misc_subtractVectorsInPlace(data.offset, data.numObservations, z);
    misc_addVectorsInPlaceWithMultiplier(data.y, data.numObservations, 2.0, z);
#endif
  }
  ...
  void rescaleResponse(BARTFit& fit) {
    ...
    for (size_t i = 1; i < data.numObservations; ++i) {
      if (yRescaled[i] < sharedScratch.dataScale.min)
        sharedScratch.dataScale.min = yRescaled[i];
      if (yRescaled[i] > sharedScratch.dataScale.max)
        sharedScratch.dataScale.max = yRescaled[i];
    }
    sharedScratch.dataScale.range =
      sharedScratch.dataScale.max - sharedScratch.dataScale.min;
    if (sharedScratch.dataScale.max == sharedScratch.dataScale.min)
      sharedScratch.dataScale.range = 1.0;
  ...
  void sampleProbitLatentVariables(
    ...
    for (size_t i = 0; i < fit.data.numObservations; ++i) {
      ...
      double mean = fits[i];
      double offset = 0.0;
      if (fit.data.offset != NULL)
        offset = fit.data.offset[i];

      double z_new, sign;
      sign = 2.0 * fit.data.y[i] - 1.0;
      if (fit.data.weights == NULL) {
        // Y_i ~ N(mu+offset, sigma^2)+;
        z_new = sign * ext_rng_simulateLowerTruncatedNormalScale1(
                         state.rng,
                         sign * (mean + offset),
                         0.0
                       );
      } else {
        z_new = sign * ext_rng_simulateLowerTruncatedNormal(
                         state.rng,
                         sign * (mean + offset),
                         1.0 / std::sqrt(fit.data.weights[i]),
                         0.0
                       );
      }
      if (!std::isnan(z_new)) {
        ...
        // unable to simulate a new Z - we're likely so far in the tail of a
        // distribution that it's incredibly hard to obtain a valid value; as
        // such, we use an epsilon amount above or below 0
      ...
      double mean = fits[i];
      if (fit.data.offset != NULL)
        mean += fit.data.offset[i];

      double u = ext_rng_simulateContinuousUniform(state.rng);
      if (fit.data.y[i] > 0.0) {
        prob =
          u + (1.0 - u) * ext_cumulativeProbabilityOfNormal(0.0, mean, 1.0);
  ...
  void storeSamples(
    ...
    if (control.responseIsBinary) {
      if (control.keepTrainingFits) {
        double* trainingSamples = results.trainingSamples +
                                  (simNum + chainStride) * data.numObservations;
        std::memcpy(
          trainingSamples,
          trainingSample,
          data.numObservations * sizeof(double)
      ...
      if (data.numTestObservations > 0) {
        double* testSamples = results.testSamples +
                              (simNum + chainStride) * data.numTestObservations;
        std::memcpy(
          testSamples,
          testSample,
          data.numTestObservations * sizeof(double)
      ...
      results.sigmaSamples[simNum + chainStride] =
        sigma * sharedScratch.dataScale.range;
    }

    if (!model.kPrior->isFixed)
      results.kSamples[simNum + chainStride] = k;

    uint32_t* variableCountSamples =
      results.variableCountSamples +
      (simNum + chainStride) * data.numPredictors;
    for (size_t j = 0; j < data.numPredictors; ++j)
      variableCountSamples[j] = variableCounts[j];
  }
  ...
  double subtractTimes(struct timeval end, struct timeval start) {
    return (1.0e6 * (static_cast<double>(end.tv_sec - start.tv_sec)) +
