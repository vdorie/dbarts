...
void dbarts_destroyFit(BARTFit* fit) { delete fit; }
void dbarts_invalidateFit(BARTFit* fit) { fit->~BARTFit(); }
...
void dbarts_destroyControl(Control* control) { delete control; }
...
void dbarts_invalidateData(dbarts::Data* data) { invalidateData(*data); }
...
void dbarts_invalidateModel(dbarts::Model* model) { invalidateModel(*model); }
...
FlattenedTrees* dbarts_getTrees(
  const dbarts::BARTFit* fit,
  const std::size_t* chainIndices,
  std::size_t numChainIndices,
  const std::size_t* sampleIndices,
...
Results* dbarts_runSampler(BARTFit* fit) { return fit->runSampler(); }

Results* dbarts_runSamplerForIterations(
  BARTFit* fit,
...
Results* dbarts_runSamplerMultithreadedForIterations(
  BARTFit* fit,
  std::size_t numBurnIn,
...
void dbarts_sampleTreesFromPrior(BARTFit* fit) { fit->sampleTreesFromPrior(); }
...
int dbarts_updatePredictor(
  ...
  return fit
    ->updatePredictor(newPredictor, &column, 1, forceUpdate, updateCutPoints);
}
...
void dbarts_updatePredictorPerObservation(
  ...
  fit->updatePredictorPerObservation(newColumn, column, installedBool);
  for (std::size_t i = 0; i < fit->data.numObservations; ++i)
    installed[i] = installedBool[i] ? 1 : 0;
...
void dbarts_updatePredictorPerObservationJointly(
  ...
  if (numFits == 0)
    return;
  std::size_t numObservations = fits[0]->data.numObservations;
...
void dbarts_stopThreads(dbarts::BARTFit* fit) { fit->stopThreads(); }

CGMPrior* dbarts_createCGMPrior() { return new CGMPrior; }
CGMPrior* dbarts_createCGMPriorFromOptions(
  double base,
  double power,
...
void dbarts_destroyCGMPrior(CGMPrior* prior) { delete prior; }
...
void dbarts_invalidateCGMPrior(CGMPrior* prior) { prior->~CGMPrior(); }

NormalPrior* dbarts_createNormalPrior() { return new NormalPrior; }
NormalPrior* dbarts_createNormalPriorFromOptions(
  const Control* control,
  const Model* model
) {
  return new NormalPrior(*control, *model);
}
void dbarts_destroyNormalPrior(NormalPrior* prior) { delete prior; }
...
void dbarts_invalidateNormalPrior(NormalPrior* prior) { prior->~NormalPrior(); }

ChiHyperprior* dbarts_createChiHyperprior() { return new ChiHyperprior; }
ChiHyperprior*
...
void dbarts_destroyChiHyperprior(ChiHyperprior* prior) { delete prior; }
...
FixedHyperprior* dbarts_createFixedHyperprior() { return new FixedHyperprior; }
FixedHyperprior* dbarts_createFixedHyperpriorFromOptions(double k) {
  return new FixedHyperprior(k);
}
void dbarts_destroyFixedHyperprior(FixedHyperprior* prior) { delete prior; }
void dbarts_initializeFixedHyperpriorFromOptions(
  FixedHyperprior* prior,
  double k
) {
...
ChiSquaredPrior* dbarts_createChiSquaredPrior() { return new ChiSquaredPrior; }
ChiSquaredPrior* dbarts_createChiSquaredPriorFromOptions(
  double degreesOfFreedom,
  double quantile
) {
  return new ChiSquaredPrior(degreesOfFreedom, quantile);
}
void dbarts_destroyChiSquaredPrior(ChiSquaredPrior* prior) { delete prior; }
void dbarts_initializeChiSquaredPriorFromOptions(
  ChiSquaredPrior* prior,
  double degreesOfFreedom,
  double quantile
) {
